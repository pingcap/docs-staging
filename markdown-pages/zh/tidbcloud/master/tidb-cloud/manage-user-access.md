...
- 组织是一个顶级实体（例如公司或客户），用于管理你的 TiDB Cloud 账户（包括一个管理账户和任意数量的成员账户）、[项目](#projects)和[资源](/tidb-cloud/tidb-cloud-glossary.md#tidb-cloud-resource)。
- 项目是 TiDB Cloud 资源的容器。

    - 对于 TiDB Cloud Starter 和 Essential 实例，项目是一个可选的逻辑容器，这意味着你可以将这些实例分组到一个项目中，也可以将这些实例保留在组织级别。
    - 对于 TiDB Cloud Dedicated 集群，项目是基础设施绑定的且必需的，这意味着 TiDB Cloud Dedicated 集群必须分组到项目中进行管理。
- TiDB Cloud 中的资源可以是 TiDB X 实例（例如 TiDB Cloud Starter 或 TiDB Cloud Essential）或 TiDB Cloud Dedicated 集群。
...
- **TiDB X 项目**：此项目类型仅用于 TiDB X 实例（TiDB Cloud Starter 和 TiDB Cloud Essential）。它帮助你按项目管理 TiDB X 实例的 RBAC。在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上创建项目时，TiDB X 项目是默认的项目类型。
- **TiDB X 虚拟项目**：此项目是虚拟的，不提供任何管理功能。它充当不属于任何项目的 TiDB X 实例（TiDB Cloud Starter 和 TiDB Cloud Essential）的虚拟容器，以便这些实例可以通过 TiDB Cloud API 使用项目 ID 进行访问。每个组织都有一个唯一的虚拟项目 ID。你可以从 TiDB Cloud API 的 [List all accessible projects](https://docs.pingcap.com/tidbcloud/api/v1beta/#tag/Project/operation/ListProjects) 端点获取此 ID。
...
| 项目中的资源类型 | 仅 TiDB Cloud Dedicated 集群 | 仅 TiDB X 实例 | 仅 TiDB X 实例 |
| 项目是否可选 | ❌ <br/>（每个 TiDB Cloud Dedicated 集群必须属于一个 Dedicated 项目） | ✅ <br/>（你可以将 TiDB X 实例分组到 TiDB X 项目中，也可以将其保留在组织级别） | 未分配给任何项目的 TiDB X 实例会自动分组到 TiDB X 虚拟项目中。 |
| 项目设置 | ✅ | ❌ | ❌ |
| 基础设施绑定 | ✅ <br/>（强绑定） | ❌ | ❌ |
...
| 管理项目下所有 TiDB Cloud Starter 实例的 [消费限额](/tidb-cloud/manage-serverless-spend-limit.md)。 | ✅ | ❌ | ❌ | ❌ |
...
> **注意：**
>
> - 实例角色仅适用于 TiDB Cloud Starter 和 TiDB Cloud Essential。TiDB Cloud Dedicated 集群不支持实例角色。
