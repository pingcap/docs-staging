...
TiProxy 节点的规格和数量取决于 TiDB Cloud Dedicated 集群的 QPS 和网络带宽。网络带宽是客户端请求和 TiDB 响应带宽的总和。
...
在创建新集群时启用 TiProxy，请点击 TiProxy 开关，并选择 TiProxy 的规格和数量。

![Enable TiProxy](https://docs-download.pingcap.com/media/images/docs/tidb-cloud/tiproxy-enable-tiproxy.png)
...
3. 在 **Modify Cluster** 页面，点击 TiProxy 开关，并选择 TiProxy 的规格和数量。
...
- TiDB 节点规格至少为 4 vCPU。
- 组织中 TiProxy 节点的默认最大数量为 `10`。更多信息请参见 [Limitations and Quotas](/tidb-cloud/limitations-and-quotas.md)。
...
![Disable TiProxy](https://docs-download.pingcap.com/media/images/docs/tidb-cloud/tiproxy-disable-tiproxy.png)
...
![TiProxy Topology](https://docs-download.pingcap.com/media/images/docs/tidb-cloud/tiproxy-topology.png)
...
- **TiProxy Throughput**：每个 TiProxy 节点每秒传输的字节数。如果最大吞吐达到最大网络带宽，建议扩容 TiProxy。关于最大网络带宽的更多信息，请参见 [决定 TiProxy 节点的规格和数量](#决定-tiproxy-节点的规格和数量)。
- **TiProxy Sessions Migration Reasons**：每分钟发生的会话迁移次数及其原因。例如，当 TiDB 缩容且 TiProxy 将会话迁移到其他 TiDB 节点时，原因为 `status`。更多迁移原因请参见 [TiProxy Monitoring Metrics](https://docs.pingcap.com/tidb/stable/tiproxy-grafana#balance)。
...
![TiProxy Billing](https://docs-download.pingcap.com/media/images/docs/tidb-cloud/tiproxy-billing.png)

## 修改 TiProxy

> **注意**
>
> - 不支持直接修改 TiProxy 规格。建议你修改 TiProxy 节点数量。如果必须修改 TiProxy 规格，需要先在所有 TiDB 节点组中禁用 TiProxy，然后重新启用以选择不同规格。
...
![Modify TiProxy](https://docs-download.pingcap.com/media/images/docs/tidb-cloud/tiproxy-enable-tiproxy.png)

## 在多个 TiDB 节点组中管理 TiProxy

当你拥有多个 TiDB 节点组时，每个 TiDB 节点组都有其专属的 TiProxy 组。TiProxy 会将流量路由到同一 TiDB 节点组内的 TiDB 节点，以实现计算资源隔离。你可以在每个 TiDB 节点组中启用、禁用或修改 TiProxy。但所有 TiDB 节点组中的 TiProxy 规格必须保持一致。
