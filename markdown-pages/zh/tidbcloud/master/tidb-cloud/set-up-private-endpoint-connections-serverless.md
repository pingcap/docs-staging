...
4. 记下 **Service Name**、**Availability Zone ID** 和 **Region ID**。
    ...
    > **注意：**
    >
    > 每个 AWS 区域只需创建一个私有端点，该端点可供位于同一区域的所有 TiDB Cloud Starter 或 TiDB Cloud Essential 实例共享。
...
创建 AWS interface 端点后，必须将其添加到目标 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的 allowlist。
