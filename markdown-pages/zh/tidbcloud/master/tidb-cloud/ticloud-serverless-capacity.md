...
为 TiDB Cloud Essential 实例设置最大和最小请求容量单元（RCU）的容量。
...
以交互模式为 TiDB Cloud Essential 实例设置容量：

```shell
 ticloud serverless capacity
```

以非交互模式为 TiDB Cloud Essential 实例设置容量：
...
| --max-rcu int32         | 指定 TiDB Cloud Essential 实例的最大请求容量单元（RCU），最大可设置为 100000。 | 否       | 仅在非交互模式下生效。                             |
| --min-rcu int32         | 指定 TiDB Cloud Essential 实例的最小请求容量单元（RCU），最小需为 2000。 | 否       | 仅在非交互模式下生效。                             |
