...
- 在增量数据迁移过程中，如果目标数据库已存在待迁移表且有主键冲突，会报错并中断迁移。此时需确认 MySQL 源数据的准确性。如果数据无误，点击迁移任务的 **重启** 按钮，迁移任务会用 MySQL 源数据替换目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>中的冲突记录。
...
在迁移前，请检查你的数据源是否受支持，确保 MySQL 兼容数据库已开启 binary log，网络连通性正常，并为源数据库和目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>数据库授予所需权限。
...
要通过 DM 持续复制源 MySQL 兼容数据库的增量变更到目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>，需要在源数据库中进行如下配置以开启 binary log：
...
在创建迁移任务前，你需要规划并设置源 MySQL 实例、TiDB Cloud 数据迁移（DM）服务与目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>之间的网络连通性。
...
1. [配置 VPC peering 连接](/tidb-cloud/set-up-vpc-peering-connections.md)，将 MySQL 服务所在 VPC 与你的 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent> 互通。
...
在开始迁移前，你需要在源数据库和目标数据库分别设置合适的数据库用户，并授予所需权限。这些权限用于让 TiDB Cloud DM 读取 MySQL 数据、复制变更并安全写入目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>。由于迁移涉及现有数据的全量导出和增量变更的 binlog 复制，迁移用户需要超出只读权限的特定权限。
...
#### 授予目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>所需权限 {#grant-required-privileges-in-the-target-dedicated-cluster-essential-instance}

出于测试目的，你可以使用 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>的 `root` 账户。

对于生产工作负载，建议在目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>中为复制创建一个专用用户，并且仅授予必要的权限：
...
例如，你可以在目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>中执行以下 `GRANT` 语句来授予相应权限：
...
2. 点击目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>名称进入概览页，然后点击左侧导航栏的 **Data** > **Data Migration**。
...
3. 填写目标连接信息。
    ...
    - **User Name**：输入 TiDB Cloud 目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent>的用户名。
...
> **注意：**
>
> - 使用物理模式时，现有数据迁移完成前，不能为该 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent> 创建第二个迁移任务或导入任务。
> - 使用物理模式且迁移任务已启动后，**不要**在该 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent> 上启用 PITR（时间点恢复）或创建任何 changefeed，否则迁移任务会卡住。如需启用 PITR 或 changefeed，请使用逻辑模式迁移数据。
...
2. 点击目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent> 的名称进入概览页，然后点击左侧导航栏的 **Data** > **Data Migration**。
