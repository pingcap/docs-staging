...
每个组织在 TiDB Cloud Essential 实例上最多可创建 100 个迁移任务。如需更多迁移任务，请 [提交支持工单](/tidb-cloud/tidb-cloud-support.md)。
...
- 当你删除 TiDB Cloud Dedicated 集群时，该集群下的所有迁移任务会被自动删除且无法恢复。
...
在迁移前，请检查你的数据源是否受支持，确保 MySQL 兼容数据库已开启 binary log，网络连通性正常，并为源数据库和目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>数据库授予所需权限。
...
要通过 DM 持续复制源 MySQL 兼容数据库的增量变更到目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>，需要在源数据库中进行如下配置以开启 binary log：
...
| `binlog_transaction_compression` | `OFF`          | DM 不支持事务压缩 |
...
在创建迁移任务前，你需要规划并设置源 MySQL 实例、TiDB Cloud 数据迁移（DM）服务与目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>之间的网络连通性。
...
2. 在 [Amazon VPC 控制台](https://console.aws.amazon.com/vpc/)中，点击左侧导航栏的 **Endpoint Services**，然后点击 **Create endpoint service**。请配置以下设置：
    ...
    创建 endpoint service 后，复制服务名称以备后用。服务名称格式为 `com.amazonaws.vpce.<region>.vpce-svc-<id>`，例如 `com.amazonaws.vpce.us-east-1.vpce-svc-0123456789abcdef0`。
...
1. [配置 VPC peering 连接](/tidb-cloud/set-up-vpc-peering-connections.md)，将 MySQL 服务所在 VPC 与你的 TiDB Cloud Dedicated 集群互通。

2. 修改 MySQL 服务关联的安全组的入站规则。

    <CustomContent plan="dedicated">

    你必须将 [你的 TiDB Cloud Dedicated 集群所在区域的 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) 添加到入站规则，允许流量从你的 TiDB Cloud Dedicated 集群流向 MySQL 实例。

    </CustomContent>
    
    <CustomContent plan="essential">

    你必须将 [你的 TiDB Cloud Essential 实例所在区域的 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) 添加到入站规则，允许流量从你的 TiDB Cloud Essential 实例流向 MySQL 实例。
...
2. [配置 VPC peering 连接](/tidb-cloud/set-up-vpc-peering-connections.md)，将你的 MySQL 服务所在 VPC 与你的 TiDB Cloud Dedicated 集群互通。

3. 修改 MySQL 所在 VPC 的 ingress 防火墙规则。

    <CustomContent plan="dedicated">

    你必须将 [你的 TiDB Cloud Dedicated 集群所在区域的 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) 添加到 ingress 防火墙规则，允许流量从你的 TiDB Cloud Dedicated 集群流向 MySQL endpoint。

    </CustomContent>
    
    <CustomContent plan="essential">

    你必须将 [你的 TiDB Cloud Essential 实例所在区域的 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) 添加到 ingress 防火墙规则，允许流量从你的 TiDB Cloud Essential 实例流向 MySQL endpoint。
...
在开始迁移前，你需要在源数据库和目标数据库分别设置合适的数据库用户，并授予所需权限。这些权限用于让 TiDB Cloud DM 读取 MySQL 数据、复制变更并安全写入目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>。由于迁移涉及现有数据的全量导出和增量变更的 binlog 复制，迁移用户需要超出只读权限的特定权限。
...
| `LOCK TABLES` | 表 | 当源端是托管 MySQL 服务（如 Amazon RDS、Aurora、ApsaraDB RDS for MySQL、Azure Database for MySQL 或 Google Cloud SQL），且不允许使用 `FLUSH TABLES WITH READ LOCK` (FTWRL) 时，需要此权限。在这种情况下，DM 会回退使用 `LOCK TABLES` 以确保全量数据导出期间的一致性。对于可使用 FTWRL 的自管理 MySQL 实例，则不需要此权限。 |
...
-- 对于托管 MySQL 服务（如 Amazon RDS 和 Aurora），还需要授予 LOCK TABLES 权限：
...
出于测试目的，你可以使用 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的 `root` 账户。

对于生产工作负载，建议在目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>中为复制创建一个专用用户，并且仅授予必要的权限：

| 权限 | 范围 | 用途 |
|:----------|:------|:--------|
| `CREATE` | 数据库、表 | 在目标端创建 schema 对象 |
| `SELECT` | 表 | 在迁移期间校验数据 |
| `INSERT` | 表 | 写入迁移的数据 |
| `UPDATE` | 表 | 在增量数据迁移期间修改现有行 |
| `DELETE` | 表 | 在复制或更新期间删除行 |
| `ALTER`  | 表 | 在 schema 变更时修改表定义 |
| `DROP`   | 数据库、表 | 在 schema 同步期间删除对象 |
| `INDEX`  | 表 | 创建和修改索引 |
| `CREATE VIEW`  | 视图 | 创建迁移使用的视图 |

例如，你可以在目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>中执行以下 `GRANT` 语句来授予相应权限：
...
2. 点击目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>名称进入概览页，然后点击左侧导航栏的 **Data** > **Data Migration**。
...
2. 填写源连接信息。
    ...
    - **Connectivity method**：根据安全需求和云厂商选择数据源连接方式：
        ...
        - **Public**：适用于 TiDB Cloud Premium 支持的所有云厂商（推荐用于测试和概念验证迁移）。
    ...
    - 根据所选 **Connectivity method**，进行如下操作：
        ...
        - 选择 **Private Link** 时，填写以下信息：
            - **Endpoint Service Name**（数据源为 AWS 时可用）：填写为 RDS 或 Aurora 实例创建的 VPC endpoint service 名称（格式：`com.amazonaws.vpce.<region>.vpce-svc-<id>`，例如 `com.amazonaws.vpce.us-east-1.vpce-svc-0123456789abcdef0`）。
    ...
    - 根据所选 **Connectivity method**，进行如下操作：
        ...
        - 选择 **Public** 时，在 **Hostname or IP address** 字段填写数据源的主机名或 IP 地址。
        - 选择 **Private Link** 时，在 **Private Endpoint** 字段中选择一个已有的 private endpoint，或点击 **Create a Private Endpoint here** 创建一个。private endpoint 在 TiDB Cloud Premium 实例的 **Networking** > **Private Endpoint for External Services** 下统一管理。你可以在多个 Data Migration 任务和 changefeed 之间复用同一个 private endpoint。有关设置详情，请参见 [Private link or private endpoint](#private-link-or-private-endpoint)。
...
3. 填写目标连接信息。
    ...
    - **User Name**：输入 TiDB Cloud 目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="essential">TiDB Cloud Essential 实例</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的用户名。
...
> **注意：**
>
> - 使用物理模式时，现有数据迁移完成前，不能为该 TiDB Cloud Dedicated 集群创建第二个迁移任务或导入任务。
> - 使用物理模式且迁移任务已启动后，**不要**在该 TiDB Cloud Dedicated 集群上启用 PITR（时间点恢复）或创建任何 changefeed，否则迁移任务会停止。如需启用 PITR 或 changefeed，请使用逻辑模式迁移数据。
...
如需一次性将数据迁移到 TiDB Cloud Premium，请选择 **Full + Incremental**，以保证源库和目标库数据一致性。

对于 **现有数据迁移**，你可以使用 **物理模式** 或 **逻辑模式**：

- 默认模式为 **逻辑模式**。该模式将 MySQL 源数据库数据导出为 SQL 语句并在目标 TiDB Cloud Premium 实例上执行。在此模式下，迁移前目标表可以为空表或非空表，但性能低于物理模式。

- 对于大数据集，你可以选择 **物理模式**。该模式在目标 TiDB Cloud Premium 实例上使用 `IMPORT INTO` 以实现更快的数据导入。物理模式要求迁移前目标表为空。如果预检查检测到所选目标表非空，迁移任务会自动回退为逻辑模式。

> **注意：**
>
> - 使用物理模式时，现有数据迁移完成前，不能为该 TiDB Cloud Premium 实例创建第二个迁移任务或导入任务。
> - 使用物理模式且迁移任务已启动后，**不要**在该 TiDB Cloud Premium 实例上启用 PITR（时间点恢复）或创建任何 changefeed，否则迁移任务会停止。如需启用 PITR 或 changefeed，请使用逻辑模式迁移数据。
...
> **注意：**
>
> TiDB Cloud Premium 会自动管理迁移任务资源。worker pool 会根据活跃迁移任务的数量自动扩缩容，因此你无需选择规格或手动扩缩容。
...
2. 点击目标 TiDB Cloud Dedicated 集群的名称进入概览页，然后点击左侧导航栏的 **Data** > **Data Migration**。
