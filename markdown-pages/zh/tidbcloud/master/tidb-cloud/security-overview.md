...
你可以通过 TLS 加密所有通信，确保数据在传输过程中的机密性和完整性。网络访问控制确保只有授权来源能够访问集群或实例资源，从而提升整体安全性。

## 数据访问控制

<CustomContent plan="starter,essential,dedicated">

对于启用了客户自管加密密钥（CMEK）的 TiDB Cloud Dedicated 集群，TiDB Cloud 为静态数据和备份数据提供加密保护。
...
对于托管在 AWS 上的 TiDB Cloud Premium 实例，你可以启用双层数据加密，在默认的存储层加密之上增加数据库层加密。启用双层数据加密后，TiDB Cloud 会对 TiKV 存储的数据、changefeed 数据和备份数据进行加密，并且你可以选择使用客户自管加密密钥（CMEK）或服务自管加密密钥。

更多信息，请参见 [双层数据加密](/tidb-cloud/premium/dual-layer-data-encryption-premium.md)。
