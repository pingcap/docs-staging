...
本文介绍如何构建一个自动化工作流：创建 TiDB Cloud Starter 实例，收集 Hacker News RSS，将其存储到 TiDB，并发送简报邮件。

> **注意：**
>
> 除了 TiDB Cloud Starter 实例外，本文中的步骤同样适用于 TiDB Cloud Essential 实例。
...
### （可选）创建一个 TiDB Cloud Starter 实例 {#optional-create-a-starter-instance}

如果你还没有 TiDB Cloud Starter 实例，可以使用此节点创建一个。否则，可跳过此操作。

1. 导航到 **Workflows** 面板，然后点击 **Add workflow**。
2. 在新的工作流工作区中，点击右上角的 **+** 并选择 **All** 字段。
3. 搜索 `TiDB Cloud` 并将其拖到工作区。
4. 为 TiDB Cloud 节点输入凭证，即 TiDB Cloud API 密钥。
5. 在 **Project** 列表中，选择你的项目。
6. 在 **Operation** 列表中，选择 `Create Serverless Cluster`。
7. 在 **Cluster Name** 框中，输入 TiDB Cloud Starter 实例的名称。
8. 在 **Region** 列表中，选择一个区域。
9. 在 **Password** 框中，输入用于登录 TiDB Cloud Starter 实例的密码。
10. 点击 **Execute Node** 运行该节点。

> **注意：**
>
> 创建新的 TiDB Cloud Starter 实例需要几秒钟时间。
...
6. 选择 TiDB Cloud Starter 实例。如果你还未在列表中看到新实例，需要等待几分钟直到实例创建完成。
...
如果工作流按预期运行，你将收到 Hacker News 简报邮件。这些新闻内容会被记录到你的 TiDB Cloud Starter 实例中，无需担心丢失。
...
- **Create Serverless Cluster**：创建 TiDB Cloud Starter 实例。
- **Execute SQL**：在 TiDB 中执行 SQL 语句。
- **Delete**：删除 TiDB 中的行。
- **Insert**：向 TiDB 插入行。
- **Update**：修改（如：修改行） TiDB 中的行。
...
- **Cluster**：输入 TiDB Cloud Starter 实例的名称。
- **Region**：Region 名称。选择 TiDB Cloud Starter 实例将要部署的 Region。通常选择最靠近你的应用部署的 Region。
- **Password**：root 密码。为新的 TiDB Cloud Starter 实例设置密码。
...
- **Project**：TiDB Cloud 项目名称。
- **Operation**：该节点的操作。所有支持的操作请参考 [支持的操作](#supported-operations)。
- **Cluster**：TiDB Cloud Starter 实例的名称。你应选择一个已有实例。
- **Password**：TiDB Cloud Starter 实例的密码。
- **User**：TiDB Cloud Starter 实例的用户名。
- **Database**：数据库名称。
- **SQL**：要执行的 SQL 语句。
...
- **Project**：TiDB Cloud 项目名称。
- **Operation**：该节点的操作。所有支持的操作请参考 [支持的操作](#supported-operations)。
- **Cluster**：TiDB Cloud Starter 实例名称。你应选择一个已有实例。
- **Password**：TiDB Cloud Starter 实例的密码。
- **User**：TiDB Cloud Starter 实例的用户名。
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择，或使用 `Name` 模式手动输入表名。
- **Update Key**：决定数据库中哪些行被修改（如：修改行）的条目属性名。item 是从一个节点传递到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
- **Columns**：以逗号分隔的输入条目属性列表，这些属性将作为被修改（如：修改行）行的列。
