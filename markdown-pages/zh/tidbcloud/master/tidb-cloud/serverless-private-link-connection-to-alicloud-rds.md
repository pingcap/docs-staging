...
本文介绍如何使用 [阿里云 Endpoint Service Private Link 连接](/tidb-cloud/serverless-private-link-connection.md) 将 TiDB Cloud Essential 实例连接到 [阿里云 ApsaraDB RDS for MySQL](https://www.alibabacloud.com/en/product/apsaradb-for-rds-mysql) 实例。
...
- 你的 TiDB Cloud Essential 实例部署在阿里云上，并且处于活跃状态。请获取并保存以下信息以备后续使用：
...
- Region 匹配：实例必须与 TiDB Cloud Essential 实例处于同一阿里云 Region。
- 可用区（AZ）可用性：可用区需与 TiDB Cloud Essential 实例的可用区有重叠。
...
3. 前往 [NLB](https://slb.console.alibabacloud.com/nlb) 创建网络型负载均衡器，填写以下信息：
    ...
    - **Zone**：需与 TiDB Cloud Essential 实例的可用区有重叠
    - **IP Version**：选择 `IPv4`
