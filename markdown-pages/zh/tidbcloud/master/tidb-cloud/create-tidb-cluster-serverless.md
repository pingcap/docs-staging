...
6. 修改实例的容量配置。
    ...
    - **Starter** 方案：
        ...
        - 你可以设置 TiDB Cloud Starter 实例的消费上限。如果消费上限设置为 0，实例将保持免费。如果消费上限大于 0，则需要在创建 TiDB Cloud Starter 实例前添加信用卡。

        - 默认情况下，每个组织最多可以创建五个[免费 TiDB Cloud Starter 实例](/tidb-cloud/select-cluster-tier.md#starter)。如需创建更多 TiDB Cloud Starter 实例，必须添加信用卡并指定消费上限。

    - **Essential** 方案：

        - 你必须为 TiDB Cloud Essential 实例指定最小和最大 Request Capacity Units（RCUs）。

        - RCU 代表为你的工作负载分配的计算资源。TiDB Cloud 会根据需求在该范围内自动扩缩 TiDB Cloud Essential 实例。
...
TiDB Cloud Starter 或 Essential 实例创建完成后，请按照 [Connect to TiDB Cloud via Public Endpoint](/tidb-cloud/connect-via-standard-connection-serverless.md) 的指引为你的实例创建密码。

> **Note:**
>
> 如果你未设置密码，将无法连接到 TiDB Cloud Starter 或 Essential 实例。
