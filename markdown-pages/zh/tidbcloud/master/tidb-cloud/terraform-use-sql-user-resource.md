...
2. 创建一个 `sql_user.tf` 文件：
    ...
    - 对于 TiDB Cloud Starter 或 TiDB Cloud Essential 实例中的 SQL 用户，`user_name` 以及内置角色 `role_readonly` 和 `role_readwrite` 必须以用户前缀开头，你可以通过运行 `tidbcloud_serverless_cluster` 数据源获取用户前缀。
