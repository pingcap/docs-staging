---
title: 备份和恢复 TiDB Cloud Premium 数据
summary: 了解如何备份和恢复你的 TiDB Cloud Premium 实例。
aliases: ['/zh/tidbcloud/restore-deleted-tidb-cluster']
---

# 备份和恢复 TiDB Cloud Premium 数据

<CustomContent plan="premium">

本文档介绍如何在 TiDB Cloud Premium 实例上备份和恢复数据。TiDB Cloud Premium 同时支持自动备份和手动备份，并允许你根据需要将备份数据恢复到新的实例。

</CustomContent>

<CustomContent plan="byoc">

本文档介绍如何在 TiDB Cloud Premium 或 TiDB Cloud BYOC 实例上备份和恢复数据。TiDB Cloud Premium 和 TiDB Cloud BYOC 同时支持自动备份和手动备份，并允许你根据需要将备份数据恢复到新的实例。

</CustomContent>

备份文件可以来自以下来源：

- 活跃的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例
- 回收站中已删除 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的备份

> **Tip:**
>
> - 如需了解如何在 TiDB Cloud Dedicated 集群上备份和恢复数据，请参见 [备份和恢复 TiDB Cloud Dedicated 数据](/tidb-cloud/backup-and-restore.md)。
> - 如需了解如何在 TiDB Cloud Starter 或 TiDB Cloud Essential 实例上备份和恢复数据，请参见 [备份和恢复 TiDB Cloud Starter 或 Essential 数据](/tidb-cloud/backup-and-restore-serverless.md)。

## 查看 Backup 页面 {#view-the-backup-page}

1. 在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上，点击目标 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的名称，进入其实例概览页面。
...
TiDB Cloud Premium 为生产环境提供增强的自动备份能力。它结合高频快照和日志备份，以确保数据可靠性。

</CustomContent>

<CustomContent plan="byoc">

TiDB Cloud Premium 和 TiDB Cloud BYOC 为生产环境提供增强的自动备份能力。它们结合高频快照和日志备份，以确保数据可靠性。
...
1. 在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上，点击目标 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的名称。
...
如需删除 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的现有备份文件，请执行以下步骤：
...
除了自动备份外，TiDB Cloud Premium 还支持手动备份。手动备份可提供一个可控且有保障的恢复点。强烈建议你在执行高风险操作之前创建手动备份，例如系统升级、关键数据删除，或不可逆的 schema 或配置变更。

</CustomContent>

<CustomContent plan="byoc">

除了自动备份外，TiDB Cloud Premium 和 TiDB Cloud BYOC 还支持手动备份。手动备份可提供一个可控且有保障的恢复点。强烈建议你在执行高风险操作之前创建手动备份，例如系统升级、关键数据删除，或不可逆的 schema 或配置变更。
...
### 恢复到新的 TiDB Cloud Premium 实例 {#restore-to-a-new-instance}

如需将数据恢复到新的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例，请执行以下步骤：
...
5. 为恢复配置新的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例。请按照 <CustomContent plan="premium">[Create a TiDB Cloud Premium Instance](/tidb-cloud/premium/create-tidb-instance-premium.md)</CustomContent><CustomContent plan="byoc">[Create a TiDB Cloud BYOC Instance](/tidb-cloud/byoc/create-tidb-instance-byoc.md)</CustomContent> 中的步骤进行操作。
    ...
    对于 TiDB Cloud BYOC，请选择与备份位于同一云服务提供商和 region 的活跃资源池。恢复后的实例会继承所选资源池的高可用模式。你可以将同一备份恢复到单可用区或多可用区资源池。如果没有可用的合适资源池，`Organization Owner` 可以在恢复实例前创建一个。其他角色无法创建资源池。更多信息，请参见 [Create a Resource Pool](/tidb-cloud/byoc/create-resource-pool-byoc.md)。
...
如需从回收站恢复已删除的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例，请执行以下步骤：
...
2. 在 **Recycle Bin** 页面，点击 <CustomContent plan="premium">**Premium**</CustomContent><CustomContent plan="byoc">**BYOC**</CustomContent> 页签，进入 <CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent> 实例的回收站。

3. 找到你要恢复的 <CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent> 实例，然后点击 **>** 按钮以展开该实例的可用备份。
...
当前，你只能将托管在 AWS 上的 TiDB Cloud Dedicated 集群备份恢复到新的 TiDB Cloud Premium 实例。

如需恢复由 TiDB Cloud Dedicated 集群生成的备份，请执行以下步骤：
...
2. 在 **Select Backup** 页面，选择包含目标 TiDB Cloud Dedicated 集群的项目。选择该 TiDB Cloud Dedicated 集群，选择你要恢复的备份快照，然后点击 **Next**。

    > **Note:**
    >
    > - 确保包含该备份快照的 TiDB Cloud Dedicated 集群在所选项目中处于 **Active** 或 **Deleted** 状态。
    > - 该快照必须位于 TiDB Cloud Premium 支持的 region 中。如果该 region 不受支持，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md) 为 TiDB Cloud Premium 开通新的 region，或选择其他备份快照。
...
TiDB Cloud Premium 支持将云存储（例如 Amazon S3 和 Alibaba Cloud Object Storage Service (OSS)）中的备份恢复到新实例。此功能兼容从 TiDB Cloud Dedicated 集群或 TiDB Self-Managed 集群生成的备份。

</CustomContent>

<CustomContent plan="byoc">

TiDB Cloud Premium 和 TiDB Cloud BYOC 支持将云存储（例如 Amazon S3）中的备份恢复到新实例。此功能兼容从 TiDB Cloud Dedicated 集群或 TiDB Self-Managed 集群生成的备份。
...
4. 如果验证成功，将显示 **Restore to a New Instance** 页面。查看页面顶部显示的备份信息，然后按照 <CustomContent plan="premium">[Create a TiDB Cloud Premium Instance](/tidb-cloud/premium/create-tidb-instance-premium.md)</CustomContent><CustomContent plan="byoc">[Create a TiDB Cloud BYOC Instance](/tidb-cloud/byoc/create-tidb-instance-byoc.md)</CustomContent> 中的步骤将备份恢复到一个新的实例。
    ...
    对于 TiDB Cloud BYOC，请选择一个与目标云服务提供商和 region 匹配的活跃资源池。恢复后的实例会继承所选资源池的高可用模式。你可以将同一备份恢复到单可用区或多可用区资源池。如果没有可用的合适资源池，`Organization Owner` 可以在恢复实例之前创建一个。其他角色无法创建资源池。更多信息，请参见 [Create a Resource Pool](/tidb-cloud/byoc/create-resource-pool-byoc.md)。
...
2. 向 IAM user 授予权限。
    ...
    创建一个仅包含你的任务所需权限的 policy，并将其附加到 IAM user。要将数据恢复到 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例，请授予 `s3:GetObject`、`s3:GetBucketLocation` 和 `s3:ListBucket` 权限。
...
2. 创建一个具有所需权限的自定义 policy。更多信息，请参见 [Create custom policies](https://www.alibabacloud.com/help/en/ram/user-guide/create-a-custom-policy)。
    ...
    - 在 **Action** 部分，选择所需权限。要将备份恢复到 TiDB Cloud Premium 实例，请授予 `oss:ListObjects` 和 `oss:GetObject` 权限。
