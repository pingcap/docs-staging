...
本文档也适用于 TiDB Cloud BYOC。TiDB Cloud BYOC 的备份和恢复功能目前与 TiDB Cloud Premium 保持一致。

</CustomContent>

备份文件可以来自以下来源：

- 活跃的 TiDB Cloud Premium 实例
- 回收站中已删除 TiDB Cloud Premium 实例的备份
...
1. 在 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上，点击目标 TiDB Cloud Premium 实例的名称，进入其实例概览页面。
...
TiDB Cloud Premium 实例采用多层备份架构来保护你的数据，如下表所示：
...
- **备份周期**：TiDB Cloud Premium 实例会同时执行每小时和每日自动备份。
...
如需删除 TiDB Cloud Premium 实例的现有备份文件，请执行以下步骤：
...
如需将数据恢复到新的 TiDB Cloud Premium 实例，请执行以下步骤：
...
5. 为恢复配置新的 TiDB Cloud Premium 实例。步骤与[创建 TiDB Cloud Premium 实例](/tidb-cloud/premium/create-tidb-instance-premium.md)相同。
...
如需从回收站恢复已删除的 TiDB Cloud Premium 实例，请执行以下步骤：
...
<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent> 支持将云存储（例如 Amazon S3 和 Alibaba Cloud Object Storage Service (OSS)）中的备份恢复到新实例。此功能兼容从 TiDB Cloud Dedicated 集群或 TiDB Self-Managed 集群生成的备份。
...
4. 如果验证成功，将显示 **Restore to a New Instance** 页面。查看页面顶部显示的备份信息，然后按照 [Create a TiDB Cloud Premium Instance](/tidb-cloud/premium/create-tidb-instance-premium.md) 中的步骤将备份恢复到一个新的实例。
...
2. 向 IAM user 授予权限。
    ...
    创建一个仅包含你的任务所需权限的 policy，并将其附加到 IAM user。要将数据恢复到 TiDB Cloud Premium 实例，请授予 `s3:GetObject`、`s3:GetBucketLocation` 和 `s3:ListBucket` 权限。
