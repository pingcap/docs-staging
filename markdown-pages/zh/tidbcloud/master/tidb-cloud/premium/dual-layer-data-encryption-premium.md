---
title: 双层数据加密
summary: 了解如何为 TiDB Cloud Premium 实例启用和管理双层数据加密。
---

# 双层数据加密

本文档介绍如何为 TiDB Cloud Premium 实例启用和管理双层数据加密。
...
- 你的云服务提供商 KMS 密钥必须与 TiDB Cloud Premium 实例位于同一区域。因此，对于使用 CMEK 的备份，不支持跨区域恢复操作。
...
1. 在你的云服务提供商 KMS 中创建一个对称加密密钥。
    ...
    该密钥必须与计划创建的 TiDB Cloud Premium 实例位于**同一区域**。
