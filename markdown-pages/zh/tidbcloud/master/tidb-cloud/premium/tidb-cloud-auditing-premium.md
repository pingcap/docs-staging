...
- 你正在使用 TiDB Cloud Premium 实例。
    ...
    > - 对于 TiDB Cloud Essential，请参见 [TiDB Cloud Essential 的数据库审计日志（PREVIEW）](/tidb-cloud/essential-database-audit-logging.md)。
...
## 配置数据库审计日志设置 {#configure-database-audit-logging-settings}

为你的云服务提供商配置存储后，完成数据库审计日志设置步骤：

1. 设置日志文件轮转策略。

    你可以根据文件大小或时间间隔轮转审计日志文件。当任一条件满足时，TiDB Cloud 会生成新的审计日志文件。
...
> **Note:**
>
> 如果禁用日志脱敏，写入你的云存储的审计日志文件可能会包含敏感信息。由于存在潜在的安全风险，不建议使用此配置。
...
2. 在 **Add Filter Rule** 对话框中，配置以下项目：
    ...
    - **Filter Events**：选择要记录的事件。支持的过滤事件，请参见[审计过滤事件](#audit-filter-events)。
