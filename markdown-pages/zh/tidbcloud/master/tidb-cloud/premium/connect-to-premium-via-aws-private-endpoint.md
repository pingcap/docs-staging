...
本文档介绍如何通过 [AWS PrivateLink](https://aws.amazon.com/privatelink) 连接到你的 TiDB Cloud Premium 实例。
...
- 私有端点和你要连接的 TiDB Cloud Premium 实例必须位于同一区域。
...
要通过私有端点连接到你的 TiDB Cloud Premium 实例，请按以下步骤操作：

1. [选择一个 TiDB Cloud Premium 实例](#step-1-select-a-premium-instance)
...
5. [连接到你的 TiDB Cloud Premium 实例](#step-5-connect-to-your-premium-instance)

如果你有多个实例，则需要对每个希望通过 AWS PrivateLink 连接的实例重复这些步骤。

### 步骤 1. 选择一个 TiDB Cloud Premium 实例 {#step-1-select-a-premium-instance}

1. 在 TiDB Cloud 控制台的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上，点击目标 TiDB Cloud Premium 实例的名称，进入其概览页面。
...
> **Note:**
>
> 对于每个 TiDB Cloud Premium 实例，相应的端点服务会在实例创建后 3 到 4 分钟自动创建。
...
>
> - 如果你的服务跨越三个以上的 availability zone (AZ)，你会收到一条错误消息，提示 VPC endpoint service 不支持该子网所在的 AZ。当你所选区域中存在一个额外的 AZ，而该 AZ 不在你的 TiDB Cloud Premium 实例所在的 AZ 范围内时，就会出现此问题。在这种情况下，你可以联系 [PingCAP Technical Support](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
...
7. 在 **Subnets** 区域中，选择你的 TiDB Cloud Premium 实例所在的可用区。

    > **Tip:**
    >
    > 如果你的服务跨越三个以上的 availability zone (AZ)，你可能无法在 **Subnets** 区域中选择 AZ。当你所选区域中存在一个额外的 AZ，而该 AZ 不在你的 TiDB Cloud Premium 实例所在的 AZ 范围内时，就会出现此问题。在这种情况下，请联系 [PingCAP Technical Support](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
...
> **Tip:**
>
> 你可以在目标 TiDB Cloud Premium 实例的 **Networking** 页面查看和管理私有端点连接。要访问此页面，请在左侧导航栏中点击 **Settings** > **Networking**。
...
1. 进入你组织的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击目标 TiDB Cloud Premium 实例的名称，进入其概览页面。
