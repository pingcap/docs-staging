---
title: 通过 AWS PrivateLink 连接到 TiDB Cloud Premium
summary: 了解如何通过 AWS 的私有端点连接到你的 TiDB Cloud Premium 实例。
---

# 通过 AWS PrivateLink 连接到 TiDB Cloud Premium

本文档介绍如何通过 [AWS PrivateLink](https://aws.amazon.com/privatelink) 连接到你的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例。

> **Tip:**
>
> 如需了解如何通过 AWS PrivateLink 连接到 TiDB Cloud Starter 或 TiDB Cloud Essential 实例，请参见 [Connect to TiDB Cloud Starter or Essential via AWS PrivateLink](/tidb-cloud/set-up-private-endpoint-connections-serverless.md)。
...
- 私有端点和你要连接的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例必须位于**同一区域**。
...
要通过私有端点连接到你的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例，请按以下步骤操作：
...
5. [连接到你的 TiDB Cloud Premium <CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例](#step-5-connect-to-your-premium-instance)

如果你有多个实例，则需要对每个希望通过 AWS PrivateLink 连接的实例重复这些步骤。

### 第 1 步：打开私有端点连接对话框 {#step-1-open-the-private-endpoint-connection-dialog}

1. 在 TiDB Cloud 控制台的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面上，点击目标 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的名称，进入其实例概览页面。
...
> - 对于每个 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例，系统会在实例创建后 3 到 4 分钟自动创建对应的端点服务。
...
>
> - 如果你的服务跨越三个以上的可用区（AZ），你将收到一条错误消息，提示 VPC endpoint 服务不支持该子网所在的 AZ。出现此问题的原因是：在你所选 Region 中，除了你的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例所在的 AZ 之外，还存在一个额外的 AZ。在这种情况下，你可以联系 [PingCAP Technical Support](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
...
7. 在 **Subnets** 区域中，选择你的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例所在的可用区。

    > **Tip:**
    >
    > 如果你的服务跨越三个以上的可用区（AZ），你可能无法在 **Subnets** 区域中选择 AZ。这是因为在你所选的 Region 中，除了你的 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例所在的 AZ 之外，还存在一个额外的 AZ。在这种情况下，请联系 [PingCAP Technical Support](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
...
> **提示：**
>
> 你可以在目标 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的 **Networking** 页面查看和管理私有端点连接。要访问此页面，请在左侧导航栏中点击 **Settings** > **Networking**。
...
### 步骤 5. 连接到你的 TiDB Cloud Premium 实例 {#step-5-connect-to-your-premium-instance}
...
1. 进入你组织的 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击目标 TiDB Cloud Premium<CustomContent plan="byoc"> 或 TiDB Cloud BYOC</CustomContent> 实例的名称，进入其概览页面。
...
### 启用 private DNS 后，我为什么仍然无法通过私有端点连接到 TiDB Cloud Premium 实例？ {#i-cannot-connect-to-a-premium-instance-via-a-private-endpoint-after-enabling-private-dns-why}
