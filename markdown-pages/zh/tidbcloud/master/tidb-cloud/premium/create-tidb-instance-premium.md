---
title: 创建 TiDB Cloud Premium 实例
summary: 了解如何创建 TiDB Cloud Premium 实例。
---

# 创建 TiDB Cloud Premium 实例

本文档介绍如何在 [TiDB Cloud console](https://tidbcloud.com/) 中创建 TiDB Cloud Premium 实例。
...
如果你拥有 `Organization Owner` 角色，可以按如下方式创建 TiDB Cloud Premium 实例：
...
3. 输入 TiDB Cloud Premium 实例的名称，然后选择要托管实例的云服务提供商和区域。
4. （可选）如需将此 TiDB Cloud Premium 实例归入某个项目以便管理，请点击 **Group Your Instance in a Project**，然后为该实例选择目标项目。如果你的组织中还没有项目，可以点击 **Create a Project** 创建一个。
...
6. 对于 TiDB Cloud Premium 实例，仅启用区域级高可用，且不可配置。更多信息，请参见 [High Availability](/tidb-cloud/serverless-high-availability.md)。
