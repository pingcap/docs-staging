...
> **Note:**
>
> 目前，TiDB Cloud 提供了使用范围不同的两个 CLI：[`ti`](https://github.com/tidbcloud/ti-cli) 和 [`ticloud`](https://github.com/tidbcloud/tidbcloud-cli)。
>
...
`ticloud` CLI 是一个命令行接口，让你可以通过终端操作 TiDB Cloud。它可管理 TiDB Cloud Starter 和 Essential 实例、导入和导出数据，并支持其他 CLI 工作流。
...
请确保你已先[完成 TiDB Cloud CLI 环境的设置](/tidb-cloud/get-started-with-cli.md)。安装好 `ticloud` CLI 后，你就可以通过命令行管理 TiDB Cloud Starter 和 Essential 实例。
...
| serverless (alias: s) | create, delete, describe, list, update, spending-limit, region, shell | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例          |
| serverless branch     | create, delete, describe, list, shell                                 | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的分支          |
| serverless import     | cancel, describe, list, start                                         | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的导入任务      |
| serverless export     | create, describe, list, cancel, download                              | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的导出任务      |
| serverless sql-user   | create, list, delete, update                                          | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的 SQL 用户         |
| serverless audit-log  | config, describe, filter-rule (alias: filter), download                                    | 管理 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的数据库审计日志         |
