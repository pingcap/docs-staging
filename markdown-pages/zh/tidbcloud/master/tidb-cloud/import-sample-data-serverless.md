...
本文档介绍如何通过 UI 将示例数据（SQL 文件）导入到 TiDB Cloud Starter 或 TiDB Cloud Essential。所使用的示例数据为 Capital Bikeshare 的系统数据，根据 Capital Bikeshare 数据许可协议发布。在导入示例数据之前，你需要拥有一个 TiDB Cloud Starter 或 Essential 实例。
...
连接到 TiDB Cloud Starter 或 Essential 实例后，你可以在终端中运行一些查询来检查结果，例如：
