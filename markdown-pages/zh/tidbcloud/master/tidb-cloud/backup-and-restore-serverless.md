...
- **Point-in-Time Restore (PREVIEW)**：将你的 TiDB Cloud Essential 实例恢复到指定的时间点。
...
> **注意：**
>
> TiDB Cloud Starter 不支持从回收站恢复。

要从回收站恢复已删除的 TiDB Cloud Essential 实例，请执行以下步骤：
...
2. 在 **Recycle Bin** 页面上，点击 **Essential** 页签，进入 TiDB Cloud Essential 实例的回收站。

3. 找到你要恢复的 TiDB Cloud Essential 实例，然后点击 **>** 按钮以展开该实例的可用备份。

    >**注意：**
    >
    > 如果某个 TiDB Cloud Essential 实例**没有备份**，则已删除的实例不会显示在回收站中。

4. 在所需备份所在的行中，点击 **...**，然后选择 **Restore**。

5. 在 **Restore** 页面上，按照[恢复到新实例](#restore-to-a-new-instance)中的相同步骤，将备份恢复到新实例。
