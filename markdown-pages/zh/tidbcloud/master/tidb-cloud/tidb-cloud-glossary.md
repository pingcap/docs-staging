...
- 对于 TiDB X 实例（包括 TiDB Cloud Starter、Essential 和 Premium 实例），project 是可选的，这意味着你既可以将这些实例归入某个 project，也可以将这些实例保留在 organization 层级。
- 对于 TiDB Cloud Dedicated 集群，project 是必需的。

project 的功能因 project 类型而异。目前共有三种类型的 project：

- **TiDB Dedicated project**：此类 project 仅用于 TiDB Cloud Dedicated 集群。它帮助你按 project 分别管理 TiDB Cloud Dedicated 集群的设置，例如 RBAC、网络、维护、告警订阅和加密访问。
...
目前，只有以下类型的 TiDB Cloud resource 支持 Recycle Bin 功能：

- TiDB Cloud Essential 实例
- TiDB Cloud Premium 实例
...
对于 TiDB Cloud Essential 和 TiDB Cloud Premium，Request Capacity Unit（RCU）是用于表示 TiDB Cloud Essential 或 TiDB Cloud Premium 实例预配置计算能力的单位。1 个 RCU 提供固定数量的计算资源，可处理一定数量的 RU 每秒。你预配置的 RCU 数量决定了实例的基线性能和吞吐能力。不过，TiDB Cloud Essential 和 TiDB Cloud Premium 的 RCU 管理方式有所不同：

- TiDB Cloud Essential 会根据你的工作负载自动预配置 RCU。随着 QPS 增加，TiDB Cloud 会动态扩容已预配置的 RCU 以维持性能。更多信息，参见 [TiDB Cloud Essential Pricing Details](https://www.pingcap.com/tidb-cloud-essential-pricing-details/)。
- TiDB Cloud Premium 允许你为工作负载指定最大 RCU 数量（`RCU_max`）。TiDB Cloud 会根据实时需求，在 `0.25 * RCU_max` 到 `RCU_max` 的范围内自动扩缩容。更多信息，参见 [TiDB Cloud Premium 中的请求单位与容量](https://docs.pingcap.com/tidbcloud/architecture-concepts/?plan=premium#request-units-and-capacity-in-premium)。

### Request Unit (RU)

对于 TiDB Cloud Starter、Essential 和 Premium，Request Unit（RU）是用于表示单个请求消耗资源量的单位。一个请求消耗的 RU 数量取决于多种因素，如操作类型和检索或修改的数据量。但这些套餐的计费模式不同：
...
- TiDB Cloud Essential 按预配置的 [Request Capacity Unit (RCU)](#request-capacity-unit-rcu) 数量计费。1 个 RCU 提供固定数量的计算资源，可处理一定数量的每秒 RU。更多信息，参见 [TiDB Cloud Essential Pricing Details](https://www.pingcap.com/tidb-cloud-essential-pricing-details/)。
- TiDB Cloud Premium 按工作负载实际消耗的 Request Capacity Unit（RCU）计费。TiDB Cloud 会每分钟计算一次平均每秒 RU，并将该平均值用作计费的 [Request Capacity Units (RCUs)](#request-capacity-unit-rcu)。更多信息，参见 [TiDB Cloud Premium 中的请求单位与容量](https://docs.pingcap.com/tidbcloud/architecture-concepts/?plan=premium#request-units-and-capacity-in-premium)。
...
聚合来自事务型或分析型存储查询结果的计算节点。增加 TiDB node 数量可以提升 TiDB Cloud Dedicated 集群可处理的并发查询数。

### TiDB Cloud resource

TiDB Cloud resource 是可管理的 TiDB Cloud 部署单元。它可以是以下之一：

- TiDB X 实例（一种基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建的面向服务的 TiDB Cloud 产品），例如 TiDB Cloud Starter、Essential 或 Premium 实例
- TiDB Cloud Dedicated 集群
...
TiDB X 架构现已在 TiDB Cloud Starter、Essential 和 Premium 中提供。更多信息，参见 [Introducing TiDB X: A New Foundation for Distributed SQL in the Era of AI](https://www.pingcap.com/blog/introducing-tidb-x-a-new-foundation-distributed-sql-ai-era/) 和 [PingCAP Launches TiDB X and New AI Capabilities at SCaiLE Summit 2025](https://www.pingcap.com/press-release/pingcap-launches-tidb-x-new-ai-capabilities/)。

### TiDB X instance

TiDB X instance 是一种基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建的面向服务的 TiDB Cloud 产品。它不需要你管理或了解底层集群拓扑。

在 TiDB Cloud 的各类套餐中，TiDB Cloud Starter、Essential 和 Premium 使用 TiDB X architecture。因此，当提到 “TiDB X instance” 时，它指的是 TiDB Cloud Starter、Essential 或 Premium 实例。
