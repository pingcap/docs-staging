...
### TiDB Cloud 账户和 TiDB Cloud 资源 {#a-tidb-cloud-account-and-a-tidb-cloud-resource}

你需要拥有一个 TiDB Cloud 账户和一个 TiDB Cloud 资源。如果还没有，请参考以下文档进行创建：

- [创建一个 TiDB Cloud Starter 或 TiDB Cloud Essential 实例](/tidb-cloud/create-tidb-cluster-serverless.md)
- [创建一个 TiDB Cloud Dedicated 集群](/tidb-cloud/create-tidb-cluster.md)

一个 TiDB Cloud 资源可以连接多个 Netlify 站点。
...
对于 TiDB Cloud Starter 或 TiDB Cloud Essential 实例，你可以通过 [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 或 [TiDB Cloud 控制台](https://tidbcloud.com/) 获取连接字符串。
...
1. 以交互模式获取 TiDB Cloud Starter 或 TiDB Cloud Essential 实例的连接字符串：

    ```shell
    ticloud cluster connect-info
    ```

2. 按提示选择你的 TiDB Cloud Starter 或 TiDB Cloud Essential 实例、客户端和操作系统。注意本文档中使用的客户端为 `Prisma`。
...
1. 在 [TiDB Cloud 控制台](https://tidbcloud.com/) 中，进入 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，点击目标资源名称进入其概览页面，然后点击右上角的 **Connect**。在弹出的对话框中，你可以从连接字符串中获取以下连接参数。
...
3. 设置环境变量。为了让你自己的空间和 Netlify 空间都能连接到 TiDB Cloud，你需要将 `DATABASE_URL` 设置为在 [步骤 1](#step-1-get-the-example-project-and-the-connection-string) 获取到的连接字符串。
...
4. 在本地构建应用并将 schema 迁移到 TiDB Cloud 资源。
