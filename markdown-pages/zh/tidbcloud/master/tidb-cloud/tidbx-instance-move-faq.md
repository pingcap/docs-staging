...
summary: 了解为什么 TiDB Cloud 会提示你移动或转换你的 TiDB Cloud Starter 和 Essential 资源、项目迁移期间会发生哪些变化，以及可能需要执行哪些后续操作。
---

# TiDB X 实例的项目迁移常见问题

TiDB X 实例是面向服务的 TiDB Cloud 产品，基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建，包括 TiDB Cloud Starter 和 TiDB Cloud Essential 实例。

本常见问题解答说明了为什么 [TiDB Cloud console](https://tidbcloud.com/) 会提示你将 TiDB Cloud Starter 和 Essential 实例移动到 TiDB X 项目中、迁移过程中会发生哪些变化，以及你需要执行哪些后续操作。

## 为什么 TiDB Cloud console 会提示我移动我的 TiDB Cloud Starter 和 Essential 实例？ {#why-does-the-tidb-cloud-console-prompt-me-to-move-my-starter-and-essential-instances}
...
| 资源类型 | 仅 TiDB Cloud Dedicated 集群 | 仅 TiDB X 实例 | 仅 TiDB X 实例 |
| 项目是否可选 | ❌ <br/>(每个 TiDB Cloud Dedicated 集群都必须属于一个 dedicated project) | ✅ <br/>（你可以将 TiDB X 实例归入某个 TiDB X project，也可以将其保留在组织级别） | N/A <br/>(未归入任何 TiDB X project 的 TiDB X 实例会自动归入 TiDB X virtual project) |
| 项目设置 | ✅ | ❌ | ❌ |
| 基础设施绑定 | ✅ <br/>(强绑定) | ❌ | ❌ |
...
## 我需要移动我的 TiDB Cloud Starter 和 Essential 实例吗？ {#do-i-need-to-move-my-starter-and-essential-instances}
...
## 如果我的项目仅包含 TiDB Cloud Starter 和 Essential 实例，会发生什么？ {#what-happens-if-my-project-contains-only-starter-and-essential-instances}
...
## 如果我的项目同时包含 TiDB Cloud Dedicated 集群以及 TiDB Cloud Starter 或 Essential 实例，会发生什么？ {#what-happens-if-my-project-contains-both-dedicated-clusters-and-starter-or-essential-instances}
...
> **Note:**
>
> 迁移后，TiDB Cloud Dedicated 集群会保留在原始项目中。因此，迁移不会影响 TiDB Cloud Dedicated 集群。
...
如果你使用 TiDB Cloud API 管理实例，请参阅 [Project API Migration Guide for TiDB Cloud Starter and Essential](/tidb-cloud/tidbx-starter-essential-project-api-migration-guide.md) 以更新你的 API 调用。
