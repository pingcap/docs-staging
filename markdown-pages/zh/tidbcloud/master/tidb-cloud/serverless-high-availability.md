...
> - 对于 TiDB Cloud Premium 实例，仅启用区域高可用，且不可配置。
> - 对于托管在 AWS Tokyo (ap-northeast-1) Region 的 TiDB Cloud Essential 实例，默认启用区域高可用。你可以在创建 TiDB Cloud Essential 实例时根据需要将其更改为分区高可用。对于托管在其他 Region 的 TiDB Cloud Essential 实例，仅启用分区高可用，且不可配置。
...
当你创建启用区域高可用的 TiDB Cloud Premium 实例时，关键 OLTP（联机事务处理）工作负载组件（如 PD 和 TiKV）会跨多个可用区部署，以确保冗余复制并最大化可用性。在正常运行期间，Gateway、TiDB 和 TiFlash 计算/写 node 等组件会部署在主可用区。这些数据面组件通过虚拟机池提供基础设施冗余，从而最大程度减少故障转移时间和因同区部署带来的网络延时。
