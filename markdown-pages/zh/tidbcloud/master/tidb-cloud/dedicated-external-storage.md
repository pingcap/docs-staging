...
- [使用 AWS access key](#configure-amazon-s3-access-using-an-aws-access-key)：使用 IAM 用户的 access key 访问你的 Amazon S3 bucket。
...
2. 在 AWS 管理控制台中，为你的 Amazon S3 bucket 创建一个托管策略。
    ...
    5. 复制以下访问策略模板，并粘贴到策略文本框中。
        ...
        - 如果你启用了 AWS Key Management Service key (SSE-KMS) 并使用了客户自管密钥加密，请确保策略中包含以下配置。`"arn:aws:kms:ap-northeast-1:105880447796:key/c3046e91-fdfc-4f3a-acff-00597dd3801f"` 是该 bucket 的一个示例 KMS key。
...
### 使用 AWS access key 配置 Amazon S3 访问权限

推荐使用 IAM 用户（而不是 AWS 账号 root 用户）来创建 access key。
...
3. 创建 access key。详细步骤请参见 [为 IAM 用户创建 access key](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。

> **Note:**
>
> TiDB Cloud 不会存储你的 access key。建议在导入完成后[删除 access key](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。
...
6. 在 **Bucket details** 页面，点击 **OBJECTS** 标签页。
    ...
    如果你想使用文件夹的 gsutil URI，打开该文件夹，然后点击文件夹名称后面的复制按钮复制文件夹名称。之后需要在名称前加上 `gs://`，末尾加上 `/`，以获得正确的文件夹 URI。
...
要允许 TiDB Cloud Dedicated 访问你的 Azure Blob 容器，需要为该容器配置 Azure Blob 访问权限。你可以使用 account SAS token 配置容器访问：

1. 在 [Azure Storage account](https://portal.azure.com/#browse/Microsoft.Storage%2FStorageAccounts) 页面，点击包含目标容器的存储账号。

2. 在存储账号的导航栏中，点击 **Security + networking** > **Shared access signature**。
