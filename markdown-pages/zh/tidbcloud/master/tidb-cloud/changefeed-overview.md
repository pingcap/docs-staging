...
> **注意：**
>
> - 目前，TiDB Cloud 每个<CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>最多只允许创建 100 个 changefeed。
...
2. 点击目标<CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的名称，进入其概览页面，然后在左侧导航栏点击 **Data** > **Changefeed**。此时会显示 changefeed 页面。
...
1. 进入目标 TiDB Cloud Dedicated 集群的 [**Changefeed**](#view-the-changefeed-page) 页面。
...
1. 进入目标 TiDB Cloud Premium 实例的 [**Changefeed**](#view-the-changefeed-page) 页面。
2. 找到你要查询的 changefeed，在 **Action** 列点击 **...** > **View**。
...
> **注意：**
>
> - 若要为 TiDB Cloud Dedicated 集群扩展 changefeed，请确保该集群的所有 changefeed 均创建于 2023 年 3 月 28 日之后。
> - 如果 TiDB Cloud Dedicated 集群中存在 2023 年 3 月 28 日之前创建的 changefeed，则该集群的所有 changefeed（包括已有和新建的）均不支持扩展或缩减。
...
1. 进入目标 TiDB <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的 [**Changefeed**](#view-the-changefeed-page) 页面。
