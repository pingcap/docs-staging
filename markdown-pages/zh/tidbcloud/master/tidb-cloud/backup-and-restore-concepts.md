...
- [TiDB Cloud Premium <CustomContent plan="byoc">和 TiDB Cloud BYOC </CustomContent>实例的自动备份](/tidb-cloud/premium/backup-and-restore-premium.md#automatic-backups)
...
TiDB Cloud Premium<CustomContent plan="byoc">, TiDB Cloud BYOC,</CustomContent> 和 TiDB Cloud Dedicated 支持手动备份。手动备份可提供一个可控的恢复点，并会一直保留，直到你显式将其删除。手动备份不支持 PITR 或部分备份，并且每次恢复操作都会创建一个新的实例。

更多信息，请参见以下内容：

- [TiDB Cloud Premium<CustomContent plan="byoc"> 和 TiDB Cloud BYOC</CustomContent> 实例的手动备份](/tidb-cloud/premium/backup-and-restore-premium.md#manual-backups)
...
对于 TiDB Cloud Premium<CustomContent plan="byoc"> 和 TiDB Cloud BYOC</CustomContent> 实例，你可以将数据恢复到新的实例。你可以从自动备份、手动备份或受支持的外部云存储备份进行恢复。PITR 仅支持自动备份，不支持手动备份。
