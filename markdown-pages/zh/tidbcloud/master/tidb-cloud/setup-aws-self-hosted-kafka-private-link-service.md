...
3. 从你的 TiDB Cloud Dedicated 集群获取 Kafka 部署信息。
    ...
    1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，进入你的 TiDB Cloud Dedicated 集群的概览页面，然后点击左侧导航栏的 **Data** > **Changefeed**。
    2. 在概览页面，找到你的 TiDB Cloud Dedicated 集群所在的 Region。确保你的 Kafka 集群将部署在同一 Region。
...
3. 从你的 TiDB Cloud Premium 实例获取 Kafka 部署信息。
    ...
    1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，进入 TiDB Cloud Premium 实例的概览页面，然后点击左侧导航栏的 **Data** > **Changefeed**。
    2. 在概览页面，找到 TiDB Cloud Premium 实例所在的 Region。确保你的 Kafka 集群将部署在同一 Region。
...
确保你的 Kafka 集群部署在与 TiDB Cloud Dedicated 集群相同的 Region 和 AZ。如果有 broker 在不同 AZ，请将其迁移到正确的 AZ。

</CustomContent>
<CustomContent plan="premium">

确保你的 Kafka 集群部署在与 TiDB Cloud Premium 实例相同的 Region 和 AZ。如果有 broker 在不同 AZ，请将其迁移到正确的 AZ。
...
1. 回到 [TiDB Cloud 控制台](https://tidbcloud.com)，为 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent> 创建 changefeed，通过 **Private Link** 连接到 Kafka 集群。详细操作请参见 [Sink to Apache Kafka](/tidb-cloud/changefeed-sink-to-apache-kafka.md)。
