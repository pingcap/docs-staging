...
- 如果你选择 Private Link 或 Private Service Connect 作为网络连接方式，请确保 TiDB Cloud Dedicated 集群版本满足以下要求：

    - v6.5.x：v6.5.9 或更高版本
    - v7.1.x：v7.1.4 或更高版本
    - v7.5.x：v7.5.1 或更高版本
    - v8.1.x：支持所有 v8.1.x 及更高版本
- 如果你希望使用 Debezium 作为数据格式，请确保 TiDB Cloud Dedicated 集群版本为 v8.1.0 或更高。
- 关于 Kafka 消息的分区分布，请注意以下事项：

    - 如果你希望按主键或索引值分发 changelog 到带有指定索引名的 Kafka 分区，请确保 TiDB Cloud Dedicated 集群版本为 v7.5.0 或更高。
    - 如果你希望按列值分发 changelog 到 Kafka 分区，请确保 TiDB Cloud Dedicated 集群版本为 v7.5.0 或更高。
...
确保你的 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent> 能够连接到 Apache Kafka 服务。你可以选择以下任一连接方式：
...
1. 在 Apache Kafka 服务所在 VPC 与 TiDB Cloud Dedicated 集群之间 [建立 VPC Peering 连接](/tidb-cloud/set-up-vpc-peering-connections.md)。
2. 修改 Apache Kafka 服务关联的安全组的入站规则。

    你必须将 TiDB Cloud Dedicated 集群所在区域的 CIDR 添加到入站规则中。该 CIDR 可在 **VPC Peering** 页面找到。这样可以允许 TiDB Cloud Dedicated 集群的流量访问 Kafka broker。
...
2. 修改 Apache Kafka 所在 VPC 的 ingress 防火墙规则。
    ...
    你必须将 TiDB Cloud Dedicated 集群所在区域的 CIDR 添加到 ingress 防火墙规则中。该 CIDR 可在 **VPC Peering** 页面找到。这样可以允许 TiDB Cloud Dedicated 集群的流量访问 Kafka broker。
...
2. 进入目标 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的概览页面，然后点击左侧导航栏的 **Data** > **Changefeed**。
...
6. 如果你选择 **Avro** 作为数据格式，页面会显示一些 Avro 专属配置。你可以按如下方式填写：
    ...
    - 在 **Schema Registry** 区域，填写你的 schema registry endpoint。如果启用 **HTTP Authentication**，会显示用户名和密码字段，并自动填充为你的 <CustomContent plan="dedicated">TiDB Cloud Dedicated 集群</CustomContent><CustomContent plan="premium">TiDB Cloud Premium 实例</CustomContent>的 endpoint 和密码。
