...
TiDB X 是一种全新的分布式 SQL 架构，其在设计上将云原生对象存储作为 TiDB 的核心存储基础。目前该架构已在 TiDB Cloud Starter、Essential 和 Premium 提供支持，能够为 AI 时代的工作负载带来弹性扩展、可预测性能以及优化的总体拥有成本（TCO）。
...
![TiDB X Architecture](https://docs-download.pingcap.com/media/images/docs/tidb-x/tidb-x-architecture.png)
...
![Classic TiDB vs TiDB X architecture](https://docs-download.pingcap.com/media/images/docs/tidb-x/tidb-classic-vs-tidb-x-1.png)
...
TiDB X 使用 [Request Capacity Unit](/tidb-cloud/tidb-cloud-glossary.md#request-capacity-unit-rcu)（RCU）来衡量计算能力。一个 RCU 提供固定的计算资源，可处理一定数量的 SQL 请求。用户配置的 RCU 数量决定了 TiDB X 实例的基准性能和吞吐能力，同时可以设置上限以控制成本，并仍然享受弹性扩展带来的优势。
...
![Classic TiDB vs TiDB X](https://docs-download.pingcap.com/media/images/docs/tidb-x/tidb-classic-vs-tidb-x-2.png)
