...
- 对于 TiDB Cloud Starter 实例，或在 2026 年 7 月 1 日之前创建的 TiDB Cloud Essential 实例，请使用[**端点共享模型**](#connect-via-a-public-endpoint-endpoint-shared-model)。在此模型中，同一 Region 中的多个 TiDB Cloud Starter 和 Essential 实例可以共享一个公共端点。
- 对于从 2026 年 7 月 1 日开始创建的 TiDB Cloud Essential 实例，请使用[**端点独占模型**](#connect-via-a-public-endpoint-endpoint-exclusive-model)。在此模型中，每个 TiDB Cloud Essential 实例都使用自己的独立公共端点。此模型无需在连接时包含[账户前缀](/tidb-cloud/select-cluster-tier.md#user-name-prefix)，但你需要为每个 TiDB Cloud Essential 实例重复执行设置步骤。
...
要使用共享模型通过公共端点连接到 TiDB Cloud Starter 或 TiDB Cloud Essential 实例，请按照以下步骤操作：
...
4. TiDB Cloud 支持为你的 TiDB Cloud Starter 实例创建 [branches](https://docs.pingcap.com/tidbcloud/branch-overview/?plan=starter)。创建分支后，你可以通过 **Branch** 下拉列表选择连接到某个分支。`main` 代表 TiDB Cloud Starter 实例本身。
...
> **注意：**
>
> 目前，端点独占模型仅适用于在特定 Region 中从 2026 年 7 月 1 日开始创建的 TiDB Cloud Essential 实例。如果你的实例不支持该模型，可以改用[端点共享模型](#connect-via-a-public-endpoint-endpoint-shared-model)。

在端点独占模型中，每个 TiDB Cloud Essential 实例都使用自己的独立公共端点。此模型无需在连接时包含[账户前缀](/tidb-cloud/select-cluster-tier.md#user-name-prefix)，但你需要为每个 TiDB Cloud Essential 实例重复执行设置步骤。

要使用独占模型通过公共端点连接到 TiDB Cloud Essential 实例，请按照以下步骤操作：
...
3. 在连接对话框中，从 **Connection Type** 下拉列表中选择 **Public**。
    ...
    - 如果你尚未为 TiDB Cloud Essential 实例配置 root 密码，请点击 **Set Root Password** 并进行设置。

4. 点击 **CA cert** 下载用于连接 TiDB Cloud Essential 实例的 TLS CA 证书。该 CA 证书默认支持 TLS 1.2。
