...
- TiDB X 实例，即一种面向服务的 TiDB Cloud 产品，基于 [TiDB X architecture](/tidb-cloud/tidb-x-architecture.md) 构建，例如 TiDB Cloud Starter, Essential, Premium<CustomContent plan="byoc">, 或 BYOC</CustomContent> 实例
- 一个 TiDB Cloud Dedicated 集群

<CustomContent plan="byoc">

在 TiDB Cloud BYOC 中，资源池是一种组织级别的 BYOC 资源，为 BYOC 实例提供底层物理资源、网络和容量。资源池在 **BYOC Management** > **Resource Pools** 中进行管理，并且不与项目关联。更多信息，请参见 [Manage Resource Pools](/tidb-cloud/byoc/manage-resource-pools-byoc.md)。
...
- 对于 TiDB Cloud Dedicated 集群，项目是必需的。
...
3. 根据你要为哪种类型的 TiDB Cloud 资源创建项目，执行以下操作之一：
    ...
    - 如果该项目是为 TiDB X 实例创建的，点击 **Confirm**。
        ...
        > **Note:**
        >
        > 对于 TiDB Cloud Premium 实例，加密是按实例而不是按项目配置的。创建实例后，你可以启用 [Dual-Layer Data Encryption](/tidb-cloud/premium/dual-layer-data-encryption-premium.md)，在默认的存储层加密之上增加数据库层加密。
        ...
        > **Note:**
        >
        > - 对于 TiDB Cloud BYOC 实例，项目是可选的。数据平面运行在你自己的云账户中，并且 BYOC 环境级别设置会在 BYOC onboarding 期间完成配置。
        > - 对于 TiDB Cloud Premium 实例，加密是按实例而不是按项目配置的。创建实例后，你可以启用 [Dual-Layer Data Encryption](/tidb-cloud/premium/dual-layer-data-encryption-premium.md)，在默认的存储层加密之上增加数据库层加密。

        </CustomContent>

    - 如果该项目是为 TiDB Cloud Dedicated 集群创建的，选择 **Create for Dedicated Cluster** 选项，为该项目配置 [Customer-Managed Encryption Keys (CMEK)](/tidb-cloud/tidb-cloud-encrypt-cmek-aws.md) 和 [maintenance window](/tidb-cloud/configure-maintenance-window.md)，然后点击 **Confirm**。
...
2. 在项目视图中，找到目标项目，然后按如下方式进行管理：
    ...
    - 对于 TiDB Dedicated 项目，你还可以点击目标项目所在行中的 <MDSvgIcon name="icon-project-settings" /> 图标，按项目管理 TiDB Cloud Dedicated 集群的设置，例如网络、维护、告警订阅和加密访问。
