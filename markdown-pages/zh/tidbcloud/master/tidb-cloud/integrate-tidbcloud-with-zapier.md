...
- 一个 [TiDB Cloud 账户](https://tidbcloud.com/signup) 以及在 TiDB Cloud 上的 TiDB Cloud Starter 实例。更多详情请参见 [TiDB Cloud 快速入门](https://docs.pingcap.com/tidbcloud/tidb-cloud-quickstart#step-1-create-a-starter-instance)。
...
3. 设置动作
    ...
    在此步骤，你需要指定 TiDB Cloud Starter 实例中的某个表来存储事件数据。如果你还没有表，可以在此步骤创建。

    1. 在下拉列表中选择项目名称和实例名称。你的 TiDB Cloud Starter 实例连接信息会自动显示。
    ...
    3. 在下拉列表中选择数据库。
        ...
        Zapier 会使用你输入的密码从 TiDB Cloud 查询数据库。如果在 TiDB Cloud Starter 实例中未找到数据库，请重新输入密码并刷新页面。
...
4. 测试动作
    ...
    点击 **Test action**，即可在表中创建一行新数据。如果你检查 TiDB Cloud Starter 实例，可以发现数据已成功写入。
...
|---|---|---|
| Find Cluster | 查找已存在的 TiDB Cloud Starter 实例或 TiDB Cloud Dedicated 集群。 | None |
| Create Cluster | 创建新集群。仅支持创建 TiDB Cloud Starter 实例。 | None |
| Find Database | 查找已存在的数据库。 | 一个 TiDB Cloud Starter 实例 |
| Create Database | 创建新数据库。 | 一个 TiDB Cloud Starter 实例 |
| Find Table | 查找已存在的表。 | 一个 TiDB Cloud Starter 实例和一个数据库 |
| Create Table | 创建新表。 | 一个 TiDB Cloud Starter 实例和一个数据库 |
| Create Row | 创建新行。 | 一个 TiDB Cloud Starter 实例、一个数据库和一个表 |
| Update Row | 更新已存在的行。 | 一个 TiDB Cloud Starter 实例、一个数据库和一个表 |
| Find Row | 通过查找列在表中查找行。 | 一个 TiDB Cloud Starter 实例、一个数据库和一个表 |
| Find Row (Custom Query) | 通过你提供的自定义查询在表中查找行。 | 一个 TiDB Cloud Starter 实例、一个数据库和一个表 |
