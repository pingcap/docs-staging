...
- 如果你的 Parquet 文件位于 Amazon S3，请为你的 TiDB Cloud Starter 或 Essential 实例[配置 Amazon S3 访问](/tidb-cloud/configure-external-storage-access.md#configure-amazon-s3-access)。

    你可以使用 AWS 访问密钥或 Role ARN 访问你的 bucket。完成后，请记录访问密钥（包括访问密钥 ID 和密钥）或 Role ARN 值，后续在 [第 4 步](#step-4-import-parquet-files) 中会用到。

- 如果你的 Parquet 文件位于 GCS，请为你的 TiDB Cloud Starter 或 Essential 实例[配置 GCS 访问](/tidb-cloud/configure-external-storage-access.md#configure-gcs-access)。

- 如果你的 Parquet 文件位于 Azure Blob Storage，请为你的 TiDB Cloud Starter 或 Essential 实例[配置 Azure Blob Storage 访问](/tidb-cloud/configure-external-storage-access.md#configure-azure-blob-storage-access)。

- 如果你的 Parquet 文件位于阿里云对象存储服务（OSS），请为你的 TiDB Cloud Starter 或 Essential 实例[配置阿里云对象存储服务（OSS）访问](/tidb-cloud/configure-external-storage-access.md#configure-alibaba-cloud-object-storage-service-oss-access)。
...
1. 打开目标 TiDB Cloud Starter 或 Essential 实例的 **Import** 页面。
    ...
    2. 点击目标 TiDB Cloud Starter 或 Essential 实例名称进入其概览页面，然后在左侧导航栏点击 **Data** > **Import**。
