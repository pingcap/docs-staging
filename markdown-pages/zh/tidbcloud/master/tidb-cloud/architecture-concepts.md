...
- **Resource pool**：定义一个或多个 TiDB Cloud BYOC 实例所依赖的底层物理资源、网络和容量边界。每个资源池都有其各自的容量配置、资源池 CIDR、高可用模式以及 AWS 资源标签。
...
你的应用会从应用环境连接到 TiDB Cloud BYOC 实例。根据你的 BYOC 配置，连接可以使用私有访问或受控的公网访问。对于私有访问，BYOC 支持 VPC peering 及其他私有连接选项。
