...
使用此函数时，需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}` 和 `${tidb_db_name}` 替换为你的 TiDB 集群的实际值。 TiDB Cloud Serverless 需要安全连接，因此需要将 `${use_ssl}` 设置为 `true`。
...
- 通过 [Go-MySQL-Driver 的文档](https://github.com/go-sql-driver/mysql/blob/master/README.md) 了解更多用法。
