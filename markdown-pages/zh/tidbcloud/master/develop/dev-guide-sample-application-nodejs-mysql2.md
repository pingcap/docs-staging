...
- (推荐) 参考 [Creating a TiDB Cloud Serverless cluster](/develop/dev-guide-build-cluster-in-cloud.md) 创建你自己的 TiDB Cloud 集群。
...
6. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的参数对应替换到 `{}` 占位符中：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Serverless，使用公共端点连接时，**必须**启用 TLS 连接，通过 `TIDB_ENABLE_SSL` 设置为 `true`。
