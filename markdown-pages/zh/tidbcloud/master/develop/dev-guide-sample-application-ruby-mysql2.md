...
> **注意：**
>
> 本教程适用于 TiDB Cloud Serverless、TiDB Cloud Dedicated 和 TiDB 自托管版本。
...
5. 编辑 `.env` 文件，设置环境变量，并用连接对话框中的参数替换 `{}` 占位符：
   ...
   > **Note**
   >
   > 建议在使用公共端点连接 TiDB Cloud Dedicated 集群时启用 TLS。
   >
   > 要启用 TLS 连接，将 `DATABASE_ENABLE_SSL` 改为 `true`，并用 `DATABASE_SSL_CA` 指定从连接对话框下载的 CA 证书文件路径。
...
2. 编辑 `.env` 文件，设置环境变量，并用连接信息中的参数替换 `{}` 占位符：
...
> **Note**
>
> 对于 TiDB Cloud Serverless，使用公共端点时 **必须** 通过 `DATABASE_ENABLE_SSL` 启用 TLS，但你**不需要**通过 `DATABASE_SSL_CA` 指定 SSL CA 证书文件路径，因为 mysql2 gem 会按特定顺序搜索现有的 CA 证书，直到找到文件。
