...
> **Note:**
>
> 目前， TiDB Cloud Serverless 集群存在限制：如果在 5 分钟内没有活动连接，它们会关闭，导致所有连接断开。因此，在使用 SQLAlchemy 连接 TiDB Cloud Serverless 集群时，池连接可能会遇到 `OperationalError`，如 `Lost connection to MySQL server during query` 或 `MySQL Connection not available`。为避免此类错误，可以将 `pool_recycle` 参数设置为 `300`。更多信息请参见 SQLAlchemy 文档中的 [Dealing with Disconnects](https://docs.sqlalchemy.org/en/20/core/pooling.html#dealing-with-disconnects)。
...
4. 点击 **Generate Password** 生成随机密码。
    ...
    > **Tip:**
    > 
    > 如果之前创建过密码，可以使用原密码，也可以点击 **Reset Password** 生成新密码。
