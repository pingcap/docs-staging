...
如果你还没有 TiDB Cloud Starter 实例，可以按照 [TiDB Cloud 快速上手指南](https://docs.pingcap.com/tidbcloud/tidb-cloud-quickstart) 创建一个，该实例免费，且大约 30 秒即可创建完成。
...
4. 进入 [AWS Lambda 控制台](https://console.aws.amazon.com/lambda/home)，你可以看到刚刚上传的 Lambda。注意需要在窗口右上角选择正确的 region。
    ...
    ![lambda dashboard](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-lambda-dashboard.png)
...
1. 在 [AWS 管理控制台](https://console.aws.amazon.com) 中，前往 [Amazon AppFlow > Connectors](https://console.aws.amazon.com/appflow/home#/gallery)，然后点击 **Register new connector**。
    ...
    ![register connector](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-register-connector.png)

2. 在 **Register a new connector** 对话框中，选择你上传的 Lambda 函数，并使用连接器名称指定 connector label。

    ![register connector dialog](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-register-connector-dialog.png)
...
![create flow](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-create-flow.png)

### 设置 flow 名称

输入 flow 名称，然后点击 **Next**。

![name flow](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-name-flow.png)
...
1. 选择源名称。本文档以 **Salesforce** 作为示例源。
    ...
    ![salesforce source](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-salesforce-source.png)

    注册 Salesforce 后，Salesforce 会在你的平台中添加一些示例数据。接下来的步骤将以 **Account** 对象作为示例源对象。

    ![salesforce data](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-salesforce-data.png)

2. 点击 **Connect**。

    1. 在 **Connect to Salesforce** 对话框中，指定该连接的名称，然后点击 **Continue**。

        ![connect to salesforce](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-connect-to-salesforce.png)

    2. 点击 **Allow**，以确认 AWS 可以读取你的 Salesforce 数据。

        ![allow salesforce](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-allow-salesforce.png)
...
3. 在 **Destination details** 区域，选择 **TiDB-Connector** 作为目标端。此时会显示 **Connect** 按钮。
    ...
    ![tidb dest](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-tidb-dest.png)
...
6. 在 **Connect to TiDB-Connector** 对话框中，输入 TiDB Cloud Starter 实例的连接属性。对于 TiDB Cloud Starter，需要将 **TLS** 选项设置为 `Yes`，这样 TiDB 连接器会使用 TLS 连接。然后点击 **Connect**。
    ...
    ![tidb connection message](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-tidb-connection-message.png)

7. 现在你可以获取到在连接中指定的数据库里的所有表。从下拉列表中选择 **sf_account** 表。

    ![database](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-database.png)

    下图展示了将 Salesforce **Account** 对象的数据传输到 TiDB 中 `sf_account` 表的配置：

    ![complete flow](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-complete-flow.png)

8. 在 **Error handling** 区域，选择 **Stop the current flow run**。在 **Flow trigger** 区域，选择 **Run on demand** 触发类型，表示你需要手动运行 flow。然后点击 **Next**。

    ![complete step1](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-complete-step1.png)
...
- 设置映射规则时，你可以在左侧选择源字段名，在右侧选择目标字段名。然后点击 **Map fields**，即可设置一条规则。
    ...
    ![add mapping rule](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-add-mapping-rule.png)
...
- 本文示例需要以下映射规则（源字段名 -> 目标字段名）：
    ...
    ![mapping a rule](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-mapping-a-rule.png)

    ![show all mapping rules](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-show-all-mapping-rules.png)

### （可选）设置过滤器

如果你想为数据字段添加一些过滤条件，可以在此处设置。否则，跳过此步骤，并点击 **Next**。

![filters](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-filters.png)

### 确认并创建 flow

确认即将创建的 flow 的信息。如果一切无误，点击 **Create flow**。

![review](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-review.png)

## 第 3 步：运行 flow

在新建 flow 的页面右上角，点击 **Run flow**。

![run flow](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-run-flow.png)

下图展示了 flow 成功运行的示例：

![run success](https://docs-download.pingcap.com/media/images/docs-cn/develop/aws-appflow-step-run-success.png)
