...
> **注意：**
>
> 下载链接中的 `{version}` 为 TiDB 的版本号。例如，amd64 架构的 `v8.5.7` 版本的下载链接为 `https://download.pingcap.com/tidb-community-server-v8.5.7-linux-amd64.tar.gz`。
...
- `cluster-version` 集群的版本，用于控制某些 Feature 是否开启，处理兼容性问题。通常是集群正常运行的所有 TiKV 节点中的最低版本，需要回滚到更低的版本时才进行手动设置。
    ...
    设置 cluster version 为 8.5.7：

    ```bash
    config set cluster-version 8.5.7
    ```
