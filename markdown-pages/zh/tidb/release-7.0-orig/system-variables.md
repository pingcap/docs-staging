...
> **注意：**
> 
> 索引加速目前实现没有从中断中恢复的能力，如果创建索引过程中遇到一些不能恢复的问题，例如 DDL owner 切换、数据操作失败等，为了保证索引数据和原表数据的一致性，TiDB 会重新进行索引数据回填的操作，并且退回事务创建索引模式。此时，可能会产生加索引的性能退化现象。后续版本中，索引加速功能将增加从检查点恢复进度的能力。
