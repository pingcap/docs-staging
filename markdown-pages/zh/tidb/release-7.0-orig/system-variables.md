...
> **注意：**
> 
> 索引加速目前实现没有从中断中恢复的能力，如果创建索引过程中遇到一些不能恢复的问题，例如：DDL owner 切换，数据操作失败等等，为了保证索引数据和原表数据的一致，我们会重新进行索引数据回填的工作，并且退回事务创建索引模式。
> 因此可能会产生加索引性能退化现象，在后续版本我们会给快速加索引模式增加从中断断点的上一个 checkpoint 恢复的能力。
>
