...
- `data-dir`：指定 TiCDC 需要使用磁盘储存文件时使用的目录。目前 Unified Sorter 会使用该目录储存临时文件，请确保该目录所在设备可用空间充足。对于使用 TiUP 的用户，本选项可以通过配置 `cdc_servers` 小节中的 `data_dir` 来指定或默认使用 `global` 中 `data_dir` 路径。**自 TiDB v5.0.3 起有效.**
- `sort-dir`：指定排序引擎使用的临时文件目录。**自 TiDB v5.0.3 起已经无效，请不要使用**。
