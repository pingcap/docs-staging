...
- 取值范围：
    ...
    - 你还可以将变量值设为内存大小，取值范围为 `0` 以及 `[536870912, 9223372036854775807]`，单位为 Byte。支持带单位的内存格式 "KB|MB|GB|TB"，例如 `90GB`（数字和单位之间无空格）。`0` 值表示不设内存限制。
...
- 取值范围：`[128, 9223372036854775807]`，单位为 Byte。支持带单位的内存格式 "KB|MB|GB|TB"，例如 `130MiB`（数字和单位之间无空格）。
