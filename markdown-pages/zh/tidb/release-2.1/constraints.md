---
title: 约束
aliases: ['/docs-cn/v2.1/constraints/','/docs-cn/v2.1/reference/sql/constraints/']
---
...
在 TiDB 的事务中，默认会对唯一约束进行[惰性检查](/transaction-overview.md#惰性检查)。通过在事务提交时再进行批量检查，TiDB 能够减少网络开销、提升性能。例如：
...
在第二个示例中，唯一约束的检查推迟到事务提交时才进行。由于 `bill` 值已经存在，这一行为导致了重复键错误。

当 `tidb_constraint_check_in_place` 的值设置为 `1` 时，则会在执行语句时就对唯一约束进行检查。例如：
