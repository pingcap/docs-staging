...
- 登录 TiDB Dashboard 后，在左侧导航栏中点击**Top SQL**。
  ...
  ![Top SQL](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-access.png)
...
2. 选择一个你想要观察负载的具体 TiDB 或 TiKV 实例。
    ...
    ![选择实例](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-select-instance.png)
...
3. 观察 Top SQL 呈现的 Top 5 图表及表格。
    ...
    ![图表表格](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-chart.png)
...
4. 点击表格中的某一个 SQL 语句后，可以展开查看该语句不同执行计划的执行情况，例如 Call/sec（平均每秒请求数）、Scan Indexes/sec（平均每秒扫描索引数）等。
    ...
    ![详情](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-details.png)
...
5. 基于这些初步线索，进一步在 [SQL 语句分析](/dashboard/dashboard-statement-list.md)或[慢查询](/dashboard/dashboard-slow-query.md)界面中了解该 SQL 语句开销大量 CPU 资源、或扫大量数据的详细原因。
    ...
    ![修改时间范围](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-change-timerange.png)

    如果图表中显示的数据已过时，你可以点击**刷新** (Refresh) 按钮，或在**刷新** (Refresh) 下拉列表中选择自动刷新。

    ![刷新](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-refresh.png)

6. 查看按表或者数据库维度的 CPU 资源使用情况，快速定位更高维度的资源使用情况。目前只支持查看 TiKV 实例。

    选择一个 TiKV 实例，然后选择 **By TABLE** 或者 **By DB**：

    ![选择聚合维度](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-select-agg-by.png)

    查看高维度的聚合结果：

    ![按 DB 维度聚合结果页面](https://download.pingcap.com/images/docs-cn/dashboard/top-sql-usage-agg-by-db-detail.png)
