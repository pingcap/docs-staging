---
title: SOUNDS LIKE
summary: 通过两个字符串的 Soundex 代码比较它们的发音。Soundex 是一种语音算法，会生成一个表示字符串发音的代码，从而可以根据发音而非拼写对字符串进行近似匹配。TiDB Cloud Lake 提供了 [SOUNDEX](/tidb-cloud-lake/sql/soundex.md) 函数，用于从字符串中获取 Soundex 代码。
---

# SOUNDS LIKE

通过两个字符串的 Soundex 代码比较它们的发音。Soundex 是一种语音算法，会生成一个表示字符串发音的代码，从而可以根据发音而非拼写对字符串进行近似匹配。TiDB Cloud Lake 提供了 [SOUNDEX](/tidb-cloud-lake/sql/soundex.md) 函数，用于从字符串中获取 Soundex 代码。
