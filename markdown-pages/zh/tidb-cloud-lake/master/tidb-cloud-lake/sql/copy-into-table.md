...
- 用户 / Internal / External stage：参见 [Stage 是什么？](/tidb-cloud-lake/guides/stage-overview.md)，了解 TiDB Cloud Lake 中的 stage。
- 在存储服务中创建的存储桶或容器。
...
> **注意：**
>
> 从 TiDB Cloud Lake `v1.2.890-nightly` 开始，`TEXT` 可以在 `FILE_FORMAT` 中用作 `TSV` 的别名。较旧的服务器可能会拒绝 `TYPE = TEXT`，因此为了兼容性，本页在语法和示例中仍使用 `TSV`。
...
`SCHEMA_EVOLUTION` 用于控制 TiDB Cloud Lake 在加载前如何对 stage 中的 NDJSON 文件进行采样。当目标表启用了 `ENABLE_SCHEMA_EVOLUTION = true` 时，请将其与 `FILE_FORMAT = (TYPE = NDJSON ...)` 一起使用。
...
如果省略 `SCHEMA_EVOLUTION`，TiDB Cloud Lake 会对这三个采样选项都使用 `AUTO`。当前 `AUTO` 的行为是最多采样 64 个文件、每个文件 1,000 条记录、总计 10,000 条记录。这些内部默认值可能会在未来版本中发生变化。如果你的加载过程对采样策略较为敏感，请显式设置 `SAMPLE_FILES`、`SAMPLE_RECORDS_PER_FILE` 和 `SAMPLE_TOTAL_RECORDS`。如果采样未覆盖某个在后续加载过程中出现的列，COPY 将失败，并报告这些额外的列名，以便你增大采样值。
...
以下示例展示了如何从不同类型的 stage 将数据加载到 TiDB Cloud Lake：
...
以下示例展示了如何从不同类型的外部数据源将数据加载到 TiDB Cloud Lake 中：
...
此示例连接到 Azure Blob Storage，并将 `data.csv` 中的数据加载到 TiDB Cloud Lake 中：
...
此示例将 Amazon S3 上一个经过 GZIP 压缩的 CSV 文件加载到 TiDB Cloud Lake 中：
...
将数据加载到 Variant 列时，TiDB Cloud Lake 会自动检查数据的有效性；如果存在任何无效数据，则会报错。例如，如果用户 stage 中有一个名为 `invalid_json_string.parquet` 的 Parquet 文件，其中包含无效的 JSON 数据，如下所示：
...
然后加载具有不同 schema 的 Parquet 文件。TiDB Cloud Lake 会自动添加新列，并使用 `NULL` 填充缺失值：
...
对于 NDJSON，`COPY INTO` 会使用默认采样值来推导缺失列。只有在你想覆盖 TiDB Cloud Lake 对 stage 中文件的采样方式时，才需要添加 `SCHEMA_EVOLUTION`：
...
TiDB Cloud Lake 会对 stage 中的 NDJSON 文件进行采样，将推导出的 `city` 和 `score` 等字段追加为可为空的列，然后再加载数据。
