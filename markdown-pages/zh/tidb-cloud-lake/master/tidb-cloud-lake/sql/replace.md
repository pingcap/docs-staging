...
- stage 文件：TiDB Cloud Lake 支持通过 `REPLACE INTO` 语句将 stage 文件中的数据替换到表中。这是通过 TiDB Cloud Lake 查询 [查询 Stage 文件](/tidb-cloud-lake/sql/stage.md) 并随后将查询结果写入表中来实现的。

> **Tip:**
>
> TiDB Cloud Lake 通过原子操作确保数据完整性。插入、修改、替换和删除操作要么全部成功，要么全部失败。
