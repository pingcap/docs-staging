...
summary: Workload group 通过为不同用户组分配 CPU、内存配额并限制并发查询数，在 TiDB Cloud Lake 中实现资源管理和查询并发控制。
---

# Workload Group

Workload group 通过为不同用户组分配 CPU、内存配额并限制并发查询数，在 TiDB Cloud Lake 中实现资源管理和查询并发控制。
