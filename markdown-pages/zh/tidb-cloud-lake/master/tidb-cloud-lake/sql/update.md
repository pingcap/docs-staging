...
> **Tip:**
>
> TiDB Cloud Lake 通过原子操作确保数据完整性。插入、修改、替换和删除要么全部成功，要么全部失败。
...
- 当 `error_on_nondeterministic_update` = `true`（默认值）时：如果某个目标行匹配到多个源行，且没有明确规则来选择使用哪个值，TiDB Cloud Lake 会返回错误。
