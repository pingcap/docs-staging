...
- 用户 / Internal / External stage：参阅 [Stage 是什么？](/tidb-cloud-lake/guides/stage-overview.md) 了解 TiDB Cloud Lake 中的 stage。
- 在存储服务中创建的存储桶或容器。
...
> - 当 `TYPE = LANCE` 且 `USE_RAW_PATH = FALSE` 时，TiDB Cloud Lake 会将查询 ID 追加到目标路径，并为每次导出创建一个单独的数据集根目录。
