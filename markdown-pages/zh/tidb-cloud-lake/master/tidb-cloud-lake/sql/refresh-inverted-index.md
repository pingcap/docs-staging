...
summary: "TiDB Cloud Lake 会在写入新数据时自动刷新 `SYNC` 模式的倒排索引。`REFRESH INVERTED INDEX` 主要用于回填在声明索引之前已存在的行。"
---

# REFRESH INVERTED INDEX

TiDB Cloud Lake 会在写入新数据时自动刷新 `SYNC` 模式的倒排索引。`REFRESH INVERTED INDEX` 主要用于回填在声明索引之前已存在的行。
