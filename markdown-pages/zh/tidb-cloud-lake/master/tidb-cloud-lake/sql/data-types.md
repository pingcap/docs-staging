---
title: 数据类型
summary: "TiDB Cloud Lake 将数据存储在强类型列中。本页概述支持的数据类型、自动/显式转换的工作方式，以及 NULL 或默认值的处理行为。"
---

# 数据类型

TiDB Cloud Lake 将数据存储在强类型列中。本页概述支持的数据类型、自动/显式转换的工作方式，以及 NULL 或默认值的处理行为。
...
TiDB Cloud Lake 会在定义明确的场景下执行自动转换：
...
除非声明为 `NOT NULL`，否则列允许 NULL 值。当在 INSERT 时省略 `NOT NULL` 列，TiDB Cloud Lake 会写入该类型对应的默认值：
