...
> **Note:**
>
> Worker 管理命令需要 cloud control。如果未配置 `cloud_control_grpc_server_address`，则在运行这些命令时，TiDB Cloud Lake 会返回 `CloudControlNotEnabled` 错误。
...
- 选项名称不区分大小写。TiDB Cloud Lake 会在规划期间将其规范化为小写。
