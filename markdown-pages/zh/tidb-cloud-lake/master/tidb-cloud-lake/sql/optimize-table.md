...
summary: 在 TiDB Cloud Lake 中优化表涉及压缩或清除历史数据，以节省存储空间并提升查询性能。
---

# OPTIMIZE TABLE

在 TiDB Cloud Lake 中优化表涉及压缩或清除历史数据，以节省存储空间并提升查询性能。

<details>
  <summary>为什么要优化？</summary>
    <div>TiDB Cloud Lake 使用 Parquet 格式将数据存储在表中，而 Parquet 格式按 block 组织。此外，TiDB Cloud Lake 支持 time travel 功能，其中每次对表进行修改的操作都会生成一个 Parquet 文件，用于捕获并反映对表所做的更改。</div><br/>

   <div>随着表随时间累积越来越多的 Parquet 文件，可能会导致性能问题并增加存储需求。为了优化表的性能，当历史 Parquet 文件不再需要时，可以将其删除。此类优化有助于提升查询性能，并减少表占用的存储空间。</div>
</details>

## TiDB Cloud Lake 数据存储：Snapshot、Segment 和 Block {#lake-data-storage-snapshot-segment-and-block}

Snapshot、segment 和 block 是 TiDB Cloud Lake 用于数据存储的概念。TiDB Cloud Lake 使用它们构建用于存储表数据的层次结构。

![Data storage structure](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/storage-structure.PNG)

TiDB Cloud Lake 会在数据修改后自动创建表 snapshot。snapshot 表示表 segment 元信息的一个版本。

在使用 TiDB Cloud Lake 时，当你通过 [AT](/tidb-cloud-lake/sql/at.md) 子句检索并查询表数据的历史版本时，最可能通过 snapshot ID 来访问某个 snapshot。
...
TiDB Cloud Lake 将实际的表数据保存在 Parquet 文件中，并将每个 Parquet 文件视为一个 block。如果你针对某个带有 snapshot ID 的 snapshot 执行 [FUSE_BLOCK](/tidb-cloud-lake/sql/fuse-block.md)，可以查看该 snapshot 引用了哪些 blocks。

TiDB Cloud Lake 会为每个数据库和表创建唯一 ID，用于存储 snapshot、segment 和 block 文件，并将它们保存在对象存储路径 `<bucket_name>/<tenant_id>/<db_id>/<table_id>/` 下。每个 snapshot、segment 和 block 文件都以 UUID（32 个字符的小写十六进制字符串）命名。
...
在 TiDB Cloud Lake 中，建议将理想的 block 大小控制为 100MB（未压缩）或 1,000,000 行，并让每个 segment 包含 1,000 个 blocks。为了最大化表优化效果，关键在于清楚了解何时以及如何应用各种优化技术，例如 [Segment Compaction](#segment-compaction) 和 [Block Compaction](#block-compaction)。

- 当使用 COPY INTO 或 REPLACE INTO 命令向包含 cluster key 的表写入数据时，TiDB Cloud Lake 会自动启动重新聚簇过程，以及 segment 和 block 的 compact 过程。
...
- 选项 LIMIT 用于设置要压缩的最大 segment 数量。在这种情况下，TiDB Cloud Lake 会选择并压缩最新的 segments。
...
- TiDB Cloud Lake 会在压缩过程完成后，自动对 clustered table 重新进行聚簇。
