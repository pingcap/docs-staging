---
title: 转换函数
summary: 本页按功能分类，全面概述了 TiDB Cloud Lake 中的转换函数，便于参考。
---

# 转换函数

本页按功能分类，全面概述了 TiDB Cloud Lake 中的转换函数，便于参考。
...
- 当从浮点数、小数或字符串转换为整数，或转换为带小数部分的小数时，TiDB Cloud Lake 会将值四舍五入到最接近的整数。这由设置 `numeric_cast_option` 决定（默认值为 `'rounding'`），该设置控制数值类型转换操作的行为。当 `numeric_cast_option` 显式设置为 `'truncating'` 时，TiDB Cloud Lake 会截断小数部分，丢弃所有小数值。
...
- TiDB Cloud Lake 还提供了多种函数，用于将表达式转换为不同的日期和时间格式。更多信息，请参见 [日期与时间函数](/tidb-cloud-lake/sql/date-time-functions.md)。