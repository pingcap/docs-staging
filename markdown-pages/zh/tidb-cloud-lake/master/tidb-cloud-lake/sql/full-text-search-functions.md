---
title: 全文搜索函数
summary: "TiDB Cloud Lake 的全文搜索函数可为使用倒排索引建立索引的半结构化 `VARIANT` 数据和纯文本列提供类似搜索引擎的过滤能力。它们非常适合与资产一同存储的 AI 生成元信息，例如来自自动驾驶视频帧的感知结果。"
---

# 全文搜索函数

TiDB Cloud Lake 的全文搜索函数可为使用倒排索引建立索引的半结构化 `VARIANT` 数据和纯文本列提供类似搜索引擎的过滤能力。它们非常适合与资产一同存储的 AI 生成元信息，例如来自自动驾驶视频帧的感知结果。

> **注意：**
>
> TiDB Cloud Lake 的搜索函数受 [Elasticsearch Full-Text Search Functions](https://www.elastic.co/guide/en/elasticsearch/reference/current/sql-functions-search.html) 启发。
