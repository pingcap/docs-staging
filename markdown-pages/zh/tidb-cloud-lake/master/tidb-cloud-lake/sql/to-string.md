---
title: TO_STRING
summary: 将值转换为 String 数据类型，或将 Date 值转换为特定的字符串格式。要在 TiDB Cloud Lake 中自定义日期和时间的格式，可以使用格式说明符。这些说明符可让你定义日期和时间值的所需格式。有关支持的说明符完整列表，请参见 Formatting Date and Time。
---

# TO_STRING

将值转换为 String 数据类型，或将 Date 值转换为特定的字符串格式。要在 TiDB Cloud Lake 中自定义日期和时间的格式，可以使用格式说明符。这些说明符可让你定义日期和时间值的所需格式。有关支持的说明符完整列表，请参见 [格式化日期和时间](/tidb-cloud-lake/sql/date-time.md#formatting-date-and-time)。
...
TiDB Cloud Lake 支持两种日期格式样式，可通过 `date_format_style` 设置进行选择：
