---
title: Interval
summary: INTERVAL 表示一个持续时间，可以写成自然语言文本（`'1 year 2 months'`、`'3 days ago'`），也可以写成以微秒为单位的整数。TiDB Cloud Lake 支持从千年到微秒的时间单位，并允许对 interval、日期和时间戳进行算术运算。
---
...
`INTERVAL` 表示一个持续时间，可以写成自然语言文本（`'1 year 2 months'`、`'3 days ago'`），也可以写成以微秒为单位的整数。TiDB Cloud Lake 支持从千年到微秒的时间单位，并允许对 interval、日期和时间戳进行算术运算。
