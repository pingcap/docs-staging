...
summary: "从 TiDB Cloud Lake 中删除现有的行访问策略。在删除策略之前，请先将其从所有引用该策略的表中解绑。"
---

# DROP ROW ACCESS POLICY

从 TiDB Cloud Lake 中删除现有的行访问策略。在删除策略之前，请先将其从所有引用该策略的表中解绑。
...
你必须具有全局 `APPLY ROW ACCESS POLICY` 权限，或者对目标策略具有 APPLY/OWNERSHIP。策略被删除后，TiDB Cloud Lake 会自动从创建者角色回收 OWNERSHIP。
