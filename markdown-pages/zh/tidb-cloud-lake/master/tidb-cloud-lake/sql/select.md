...
- `SELECT` 语句还支持直接查询 stage 中的文件。语法和示例请参见[使用 TiDB Cloud Lake 高效进行数据转换](/tidb-cloud-lake/sql/stage.md)。
...
在 TiDB Cloud Lake 中，你可以使用 `AS` 关键字为列指定别名。这样可以在 SQL 语句和查询结果中为列提供更具描述性且更易理解的名称：

- TiDB Cloud Lake 建议在创建列别名时尽量避免使用特殊字符。不过，如果某些场景下确实需要使用特殊字符，则应将别名用反引号括起来，例如：`SELECT price AS \`$CA\` FROM ...`

- TiDB Cloud Lake 会自动将别名转换为小写。例如，如果你将某列的别名设为 *Total*，那么它在结果中会显示为 *total*。如果你希望保留大小写，请将别名用反引号括起来：`\`Total\``。
...
TiDB Cloud Lake 对 SQL 语法进行了扩展，允许查询以 `FROM <table>` 开始，而无需显式使用 `SELECT *`：
...
为了优化大结果集查询的性能，TiDB Cloud Lake 默认启用了 `lazy_read_threshold` 选项，默认值为 1,000。该选项专门用于包含 LIMIT 子句的查询。启用 `lazy_read_threshold` 后，当查询中指定的 LIMIT 数量小于或等于你设置的阈值时，将触发该优化。要禁用此选项，请将其设置为 0。
