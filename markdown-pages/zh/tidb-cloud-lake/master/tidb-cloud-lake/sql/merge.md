...
一条 MERGE 语句通常包含 MATCHED 和 / 或 NOT MATCHED 子句，用于指示 TiDB Cloud Lake 如何处理匹配和不匹配的场景。对于 MATCHED 子句，你可以选择对目标表执行 **UPDATE** 或 **DELETE** 操作。相反，对于 NOT MATCHED 子句，可用的操作是 **INSERT**。
...
如果一条 MERGE 语句包含多个 MATCHED 子句，则除最后一个子句外，每个子句都需要指定条件。这些条件决定了执行相应操作的判定标准。TiDB Cloud Lake 会按照指定顺序对这些条件进行求值。一旦某个条件满足，就会触发对应的操作，跳过其余的 MATCHED 子句，然后继续处理来源中的下一行。如果 MERGE 语句还包含多个 NOT MATCHED 子句，TiDB Cloud Lake 也会以类似方式处理它们。
