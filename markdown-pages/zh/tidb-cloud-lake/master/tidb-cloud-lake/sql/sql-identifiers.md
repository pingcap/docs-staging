---
title: SQL 标识符
summary: SQL 标识符是在 TiDB Cloud Lake 中用于不同元素的名称，例如表、视图和数据库。
---

# SQL 标识符

SQL 标识符是在 TiDB Cloud Lake 中用于不同元素的名称，例如表、视图和数据库。
...
默认情况下，TiDB Cloud Lake 会将不带引号的标识符以小写形式存储，而双引号标识符则按输入时的形式存储。换句话说，TiDB Cloud Lake 默认将数据库、表和列等对象名称视为大小写不敏感。如果你希望 TiDB Cloud Lake 将它们视为大小写敏感，请使用双引号。

> **注意：**
>
> 默认情况下，TiDB Cloud Lake 遵循 PostgreSQL 风格的标识符大小写规则：不带引号的标识符会折叠为小写，而双引号标识符会保留其精确大小写并且大小写敏感。此行为由以下两个设置控制：
>
...
以下示例演示了 TiDB Cloud Lake 在创建和列出数据库时如何处理标识符的大小写：
...
以下示例演示了 TiDB Cloud Lake 如何处理表名和列名的大小写，突出显示了其默认的大小写敏感行为，以及如何使用双引号区分大小写不同的标识符：
...
在 TiDB Cloud Lake 中，处理文本和日期等字符串项时，标准做法是将它们用单引号（'）括起来。
...
默认情况下，TiDB Cloud Lake 的 SQL 方言为 `PostgreSQL`：
