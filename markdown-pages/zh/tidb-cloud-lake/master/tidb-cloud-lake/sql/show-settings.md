---
title: SHOW SETTINGS
summary: "TiDB Cloud Lake 提供了多种系统设置，使你能够控制 TiDB Cloud Lake 的工作方式。此命令会显示可用系统设置的当前值、默认值以及设置级别。要修改某个设置，请使用 SET 或 UNSET 命令。"
---

# SHOW SETTINGS

TiDB Cloud Lake 提供了多种系统设置，使你能够控制 TiDB Cloud Lake 的工作方式。此命令会显示可用系统设置的当前值、默认值以及[设置级别](#setting-levels)。要修改某个设置，请使用 [SET](/tidb-cloud-lake/sql/set.md) 或 [UNSET](/tidb-cloud-lake/sql/unset.md) 命令。

- TiDB Cloud Lake 的某些行为无法通过系统设置进行更改；你在使用 TiDB Cloud Lake 时必须将这些行为考虑在内。例如：
    - TiDB Cloud Lake 会将字符串编码为 UTF-8 字符集。
    - TiDB Cloud Lake 对数组使用从 1 开始的编号约定。
- TiDB Cloud Lake 将系统设置存储在系统表 [system.settings](/tidb-cloud-lake/sql/system-settings.md) 中。
...
每个 TiDB Cloud Lake 设置都具有一个级别，可以是 Global、Default 或 Session。下表说明了各个级别之间的区别：
...
> **注意：**
>
> 由于 TiDB Cloud Lake 会不时修改系统设置，此示例可能不会显示最新结果。要查看 TiDB Cloud Lake 中最新的系统设置，请在你的 TiDB Cloud Lake 实例中执行 `SHOW SETTINGS;`。
