---
title: Geospatial
summary: "TiDB Cloud Lake 通过两种数据类型存储空间数据。"
---

# Geospatial

TiDB Cloud Lake 通过两种数据类型存储空间数据：
...
TiDB Cloud Lake 以 EWKB 持久化空间值，但提供多种输出格式。你可以设置 `geometry_output_format` 会话参数（默认值：`WKT`），或调用显式转换函数：
