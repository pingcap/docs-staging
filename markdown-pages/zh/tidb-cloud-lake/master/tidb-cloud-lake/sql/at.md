...
TiDB Cloud Lake 会在数据发生修改时自动创建快照，因此可以将快照视为过去某一时间点的数据视图。你可以通过 snapshot ID 或创建该快照时的时间戳来访问快照。关于如何获取 snapshot ID 和时间戳，请参见[获取 Snapshot ID 和时间戳](#obtaining-snapshot-id-and-timestamp)。

这是 TiDB Cloud Lake Time Travel 功能的一部分。该功能允许你在保留时间内（默认 24 小时）对数据的历史版本进行查询、备份和恢复。
