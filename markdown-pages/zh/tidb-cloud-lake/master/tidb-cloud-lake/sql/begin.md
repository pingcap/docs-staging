...
- TiDB Cloud Lake *不*支持嵌套事务，因此不匹配的事务语句会被忽略。
...
TiDB Cloud Lake 会自动为每个事务生成一个事务 ID。该 ID 使用户能够识别哪些语句属于同一个事务，从而便于排查问题。

如果你使用的是 TiDB Cloud Lake，可以在 **Monitor** > **SQL History** 中找到事务 ID：
