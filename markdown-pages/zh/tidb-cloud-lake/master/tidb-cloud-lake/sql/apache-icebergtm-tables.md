...
TiDB Cloud Lake 可以连接到 [Apache Iceberg™](https://iceberg.apache.org/) catalog，这样你无需将数据加载到 Fuse 表中即可查询 Iceberg 表。当所连接的 catalog 支持写操作时，你还可以创建并写入 Iceberg 表。
...
| 参数 | 必填？ | 说明 |
| --- | --- | --- |
| `<catalog_name>` | 是 | TiDB Cloud Lake 中的 catalog 名称。 |
...
| TiDB Cloud Lake 类型 | Apache Iceberg™ 类型 |
| --- | --- |
| BOOLEAN | Boolean |
| INT | Int |
| BIGINT | Long |
