...
TiDB Cloud Lake 中的 connection 是一种指定配置，用于封装与外部存储服务交互所需的详细信息。它作为一组集中且可复用的参数集合，例如访问凭证、端点 URL 和存储类型，从而便于 TiDB Cloud Lake 与各种存储服务集成。

Connection 可用于创建 external stage、external table 以及 attach table，为通过 TiDB Cloud Lake 管理和访问存储在外部存储服务中的数据提供了一种更简洁、模块化的方法。
...
[ATTACH TABLE](/tidb-cloud-lake/sql/attach-table.md) 页面提供了[示例](/tidb-cloud-lake/sql/attach-table.md#examples)，演示如何将 TiDB Cloud Lake 中的新表与 TiDB Cloud Lake 中的现有表连接，其中数据存储在名为 "lake-toronto" 的 Amazon S3 存储桶中。在每个示例中，步骤 3 都可以通过使用前面定义的名为 `toronto` 的 connection 进行简化：
