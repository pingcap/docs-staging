...
物化视图会将查询结果以物理方式存储。它定义在 `default` catalog 中的一张持久化 FUSE 表之上。在创建物化视图时，TiDB Cloud Lake 会对源表启用变更跟踪。

与逻辑视图不同，物化视图可以通过显式刷新来持久化其源表中的变更。即使物理存储落后于源表，读也能保持一致。在第一次刷新之前，TiDB Cloud Lake 会根据源表计算该定义。当源表存在尚未刷新的变更时，TiDB Cloud Lake 会使用 **read fix**：在读取时，将已持久化的物化视图数据与所需的源表增量数据进行联合体，并对该增量应用视图定义。因此，查询会返回当前结果，而不是过期的物化数据。
...
第一次刷新会将源数据物化。后续刷新会对仅追加的变更进行增量地处理。如果源表存在 `UPDATE`、`DELETE` 或 `TRUNCATE` 变更，TiDB Cloud Lake 会根据当前源状态重建物化视图，以确保结果正确。
