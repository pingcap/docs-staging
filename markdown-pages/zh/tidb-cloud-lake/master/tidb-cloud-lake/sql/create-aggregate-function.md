...
summary: 创建在 TiDB Cloud Lake 的 JavaScript 或 Python 运行时中运行的用户定义聚合函数（UDAF）。
---

# CREATE AGGREGATE FUNCTION

创建在 TiDB Cloud Lake 的 JavaScript 或 Python 运行时中运行的用户定义聚合函数（UDAF）。
...
| `STATE { <state_field_list> }` | TiDB Cloud Lake 在部分/最终聚合步骤之间存储的结构体定义（例如 `STATE { sum DOUBLE, count DOUBLE }`）。 |
