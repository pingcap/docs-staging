---
title: Fuse Engine 表
summary: "TiDB Cloud Lake 使用 Fuse Engine 作为其默认存储引擎，提供类似 Git 的数据管理系统。"
---
...
TiDB Cloud Lake 使用 Fuse Engine 作为其默认存储引擎，提供类似 Git 的数据管理系统，具备以下特性：
...
**说明：** 如果未显式指定引擎，TiDB Cloud Lake 会默认使用 Fuse Engine 创建表，这等同于 `ENGINE = FUSE`。
...
- **说明：** 指定表数据的保留小时数。最小值为 1 小时。最大值由 TiDB Cloud Lake 服务配置决定；如果未指定，则默认值为 2,160 小时（90 天 x 24 小时）。
...
- **说明：**
    ...
    控制在执行 `COPY INTO` 操作期间是否可以自动进行表结构演进。启用后（设置为 `True`），当负载 Parquet 文件且其 schema 中包含目标表中不存在的列时，TiDB Cloud Lake 会自动将缺失的列添加到表中。现有行中缺失的值会填充为 `NULL`。更多信息，请参见 [Schema Evolution](/tidb-cloud-lake/guides/schema-evolution.md)。
