...
与 CSV、TSV、NDJSON 或 PARQUET 不同，命名的 `LANCE` 格式只能与 `COPY INTO <location>` 一起复用。不支持将其用于 stage-table 读取或 `COPY INTO <table>`，因为 TiDB Cloud Lake 写入的是 Lance 数据集目录，而不是独立文件。
