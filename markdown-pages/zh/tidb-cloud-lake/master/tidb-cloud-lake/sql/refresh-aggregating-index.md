...
summary: "TiDB Cloud Lake 会在新数据摄取时自动以 `SYNC` 模式维护聚合索引。当你在一个已包含数据的表上新增索引时，运行 REFRESH AGGREGATING INDEX 以回填更早的行。"
---

# REFRESH AGGREGATING INDEX

TiDB Cloud Lake 会在新数据摄取时自动以 `SYNC` 模式维护聚合索引。当你在一个已包含数据的表上新增索引时，运行 `REFRESH AGGREGATING INDEX` 以回填更早的行。
