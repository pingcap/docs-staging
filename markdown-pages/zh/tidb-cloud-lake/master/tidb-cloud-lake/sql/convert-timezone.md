...
- 夏令时缺口可能会导致某些时间戳无效。开启 `enable_dst_hour_fix = 1`（会话或租户级别）后，TiDB Cloud Lake 会自动调整此类值。
...
在此会话中，时区配置为 Asia/Shanghai，且 `enable_dst_hour_fix = 1`。时间戳 `1947-04-15 00:00:00` 在该时区中实际上从未存在，因为时钟曾向前跳变，因此 TiDB Cloud Lake 会在返回 UTC 值之前先对其进行调整。
