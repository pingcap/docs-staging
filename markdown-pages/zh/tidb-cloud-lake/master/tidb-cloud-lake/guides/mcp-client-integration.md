...
[TiDB Cloud Lake MCP server](https://github.com/tidbcloud/lake-mcp) 通过 [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 将 AI 助手连接到 TiDB Cloud Lake。借助兼容 MCP 的工具，你可以使用自然语言指令探索数据库对象、检查表结构，并运行 SQL。
...
从你的 TiDB Cloud Lake 计算集群中获取主机、用户名、密码、数据库和计算集群名称。更多信息，请参见[连接到计算集群](/tidb-cloud-lake/guides/warehouse.md#connecting-to-a-warehouse)。
...
| `false` | 服务器允许已配置的 TiDB Cloud Lake 用户所具备权限允许的任何 SQL 操作。仅在使用受信任的工具和最小权限账户时使用此设置。 |
