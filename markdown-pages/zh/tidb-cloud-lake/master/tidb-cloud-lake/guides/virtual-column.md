...
1. **数据摄取** → TiDB Cloud Lake 分析 VARIANT 列中的 JSON 结构
2. **智能检测** → 系统识别被频繁访问的嵌套字段
3. **后台优化** → 自动创建虚拟列
4. **查询加速** → 查询自动使用优化后的路径

![Virtual Column Workflow](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/virtual-column.png)
