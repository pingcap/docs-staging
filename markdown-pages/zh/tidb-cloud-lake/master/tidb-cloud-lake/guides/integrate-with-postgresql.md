...
summary: 本页介绍如何创建一个 PostgreSQL 集成任务，将 PostgreSQL 数据库中的数据同步到 TiDB Cloud Lake。
---

# PostgreSQL 集成任务

本页介绍如何创建一个 PostgreSQL 集成任务，将 PostgreSQL 数据库中的数据同步到 TiDB Cloud Lake。PostgreSQL 任务支持全量 `Snapshot` 导入、持续的 `Change Data Capture (CDC)`，或两者结合使用。
...
- 目标 PostgreSQL 实例可从 TiDB Cloud Lake 访问
...
请确保 PostgreSQL 实例可从 TiDB Cloud Lake 访问。检查防火墙规则和安全组设置，允许 PostgreSQL 端口上的入站连接。
...
Snapshot 模式会对源表执行一次性全量读，并将所有数据加载到 TiDB Cloud Lake 中的目标表。

**Use cases:**

- 从 PostgreSQL 到 TiDB Cloud Lake 的初始数据迁移
- 周期性的全量数据刷新
...
- 实时数据复制
- 使 TiDB Cloud Lake 与业务 PostgreSQL 数据库保持同步
- 事件驱动的数据管道
...
主键指定了 CDC 期间用于 MERGE 操作的唯一标识列。当捕获到变更事件时，TiDB Cloud Lake 使用该键来判断是插入新行还是修改现有行。通常，这应当是源表的主键。
...
启用后（CDC 模式下默认启用），从 PostgreSQL WAL 捕获到的 DELETE 操作会应用到 TiDB Cloud Lake 中的目标表。禁用后，删除操作会被忽略，目标表将保留所有历史记录。这适用于希望保留完整审计轨迹的场景。
