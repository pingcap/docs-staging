---
title: 使用任务自动化数据加载
summary: 任务会封装 SQL，使 TiDB Cloud Lake 能够按调度或在满足条件时为你运行它。在使用 CREATE TASK 定义任务时，请注意以下可调参数。
---

# 使用任务自动化数据加载

任务会封装 SQL，使 TiDB Cloud Lake 能够按调度或在满足条件时为你运行它。在使用 [CREATE TASK](/tidb-cloud-lake/sql/create-task.md) 定义任务时，请注意以下可调参数。
