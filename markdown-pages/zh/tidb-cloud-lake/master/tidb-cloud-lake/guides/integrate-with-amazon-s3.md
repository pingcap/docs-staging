...
summary: Amazon S3 数据集成使你能够将 S3 存储桶中的文件导入到 TiDB Cloud Lake。它支持 CSV、Parquet 和 NDJSON 文件格式，并可选择一次性导入或自动轮询新文件的持续摄取。
---
...
| 字段               | 描述                                                        |
|---------------------|--------------------------------------------------------------------|
| **Warehouse**       | 选择用于运行导入的目标 TiDB Cloud Lake 计算集群 (Warehouse)          |
| **Target Database** | 选择 TiDB Cloud Lake 中的目标数据库                                  |
| **Target Table**    | TiDB Cloud Lake 中的表名                                             |
...
> **Tip:**
>
> 当新文件会定期添加到 S3 路径，并且你希望它们自动加载到 TiDB Cloud Lake 时，请启用 **Continuous Ingestion**。对于一次性导入，请禁用此选项。
