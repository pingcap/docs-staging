...
summary: LakeSQL 是专为 TiDB Cloud Lake 设计的命令行工具。它允许用户建立与 TiDB Cloud Lake 的连接，并直接在 CLI 窗口中执行查询。
---

# 使用 LakeSQL 连接到 TiDB Cloud Lake

[LakeSQL](https://github.com/tidbcloud/lakesql) 是专为 TiDB Cloud Lake 设计的命令行工具。它允许用户建立与 TiDB Cloud Lake 的连接，并直接在 CLI 窗口中执行查询。

LakeSQL 特别适合偏好命令行接口并且需要经常使用 TiDB Cloud Lake 的用户。借助 LakeSQL，用户可以轻松高效地管理数据库、表和数据，并便捷地执行各种查询和操作。
...
对于连接到 TiDB Cloud Lake，你可以使用默认的 `cloudapp` 用户，或者使用通过 [CREATE USER](/tidb-cloud-lake/sql/create-user.md) 命令创建的 SQL 用户。请注意，你用于登录 [TiDB Cloud Lake console](https://app.lake.tidbcloud.com) 的用户账户不能用于连接到 TiDB Cloud Lake。

## 使用 LakeSQL 连接 {#connecting-with-lakesql}

LakeSQL 支持连接到 TiDB Cloud Lake 实例。

### 使用 DSN 自定义连接 {#customize-connections-with-a-dsn}

DSN（Data Source Name）是一种简单而强大的方式，可让你在 LakeSQL 中使用单个 URI 风格的字符串来配置和管理 TiDB Cloud Lake 连接。通过这种方式，你可以将凭证和连接设置直接嵌入到环境中，从而简化连接过程。
...
| 常见 DSN 参数 | 描述                          |
|-----------------------|--------------------------------------|
| `tenant`              | 租户 ID，仅适用于 TiDB Cloud Lake。      |
| `warehouse`           | 计算集群名称，仅适用于 TiDB Cloud Lake。 |
...
| `presign`                   | 为数据加载启用 presign。可选值：`auto`、`detect`、`on`、`off`。默认值为 `auto`（仅对 TiDB Cloud Lake 启用）。 |
...
### 连接到 TiDB Cloud Lake {#connect-to-lake}

连接到 TiDB Cloud Lake 的最佳实践是从 TiDB Cloud Lake 获取你的 DSN，并将其导出为环境变量。要获取 DSN，请执行以下操作：

1. 登录 TiDB Cloud Lake，然后在 **Overview** 页面点击 **Connect**。
...
断开与 TiDB Cloud Lake 的连接并退出 LakeSQL。
