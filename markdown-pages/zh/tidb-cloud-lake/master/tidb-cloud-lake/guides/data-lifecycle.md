...
summary: "TiDB Cloud Lake 支持常见的数据定义语言（DDL）和数据操纵语言（DML）命令，让你能够轻松管理数据库。无论是组织、存储、查询、修改还是删除数据，TiDB Cloud Lake 都遵循你所熟悉的行业标准。"
---

# TiDB Cloud Lake 中的数据生命周期

TiDB Cloud Lake 支持常见的数据定义语言（DDL）和数据操纵语言（DML）命令，让你能够轻松管理数据库。无论是组织、存储、查询、修改还是删除数据，TiDB Cloud Lake 都遵循你所熟悉的行业标准。

## TiDB Cloud Lake 对象 {#lake-objects}

TiDB Cloud Lake 支持以下对象的创建和修改：
...
直接将数据添加到表中。TiDB Cloud Lake 还支持将外部文件中的数据导入到其表中。
...
当数据位于 TiDB Cloud Lake 中后，你可以根据需要对其进行修改、替换、合并或删除。
...
TiDB Cloud Lake 支持删除特定数据，也支持删除整个表和数据库。
