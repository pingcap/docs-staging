...
[Jupyter Notebook](https://jupyter.org/) 是一个用于运行代码、查询数据和创建可视化内容的交互式环境。你可以通过 [TiDB Cloud Lake dialect for SQLAlchemy](https://github.com/tidbcloud/lake-sqlalchemy) 将 notebook 连接到 TiDB Cloud Lake。
...
`tidbcloudlake-sqlalchemy` 包会安装 SQLAlchemy 以及所需的 TiDB Cloud Lake Python 驱动。
