...
[Apache Superset](https://superset.apache.org/) 是一个开源的数据探索和可视化平台。Superset 通过 [TiDB Cloud Lake dialect for SQLAlchemy](https://github.com/tidbcloud/lake-sqlalchemy) 连接到 TiDB Cloud Lake。
...
- Docker
- 一个 TiDB Cloud Lake 账户、数据库和计算集群
...
官方 Superset 镜像不包含 TiDB Cloud Lake SQLAlchemy 方言。请创建一个名为 `Dockerfile` 的文件，并写入以下内容：
...
`tidbcloudlake-sqlalchemy` 包会安装所需的 TiDB Cloud Lake Python 驱动作为依赖关系。
...
现在，你可以在 Superset 中基于 TiDB Cloud Lake 表创建数据集，并将其用于图表和仪表板。
