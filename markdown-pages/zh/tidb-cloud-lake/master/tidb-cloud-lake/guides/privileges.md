---
title: 权限
summary: 权限是执行某项操作的许可。用户必须具有特定权限，才能在 TiDB Cloud Lake 中执行特定操作。例如，查询表时，用户需要具有该表的 `SELECT` 权限。类似地，要读取 stage 中的数据集，用户必须具有 `READ` 权限。
---
...
| OWNERSHIP | 授予对 masking policy 的完全控制权。TiDB Cloud Lake 会将 OWNERSHIP 授予创建该 policy 的角色，并在 policy 被删除时自动（权限）回收该权限。 |
...
| OWNERSHIP | 授予对 row access policy 的完全控制权。TiDB Cloud Lake 会将 OWNERSHIP 授予创建者角色，并在 policy 被删除时自动（权限）回收该权限。 |
