...
summary: "TiDB Cloud Lake 提供了对直接从 stage 查询 Avro 文件的全面支持。这使你无需先将数据加载到表中，即可灵活地进行数据探索和转换。"
---
...
* **类型映射**：每种 Avro 类型都会映射为 TiDB Cloud Lake 中对应的 variant 类型。
...
* **Enum 类型**：Avro `ENUM` 类型会映射为 TiDB Cloud Lake 中的 `STRING` 值。