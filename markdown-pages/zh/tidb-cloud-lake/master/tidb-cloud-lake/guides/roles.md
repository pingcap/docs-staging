---
title: 角色
summary: "TiDB Cloud Lake 中的角色在简化权限管理方面发挥着关键作用。当多个用户需要相同的一组权限时，逐个授予权限会很繁琐。角色通过将一组权限分配给某个角色来提供解决方案，然后可以轻松地将该角色分配给多个用户。"
---

# 角色

TiDB Cloud Lake 中的角色在简化权限管理方面发挥着关键作用。当多个用户需要相同的一组权限时，逐个授予权限会很繁琐。角色通过将一组权限分配给某个角色来提供解决方案，然后可以轻松地将该角色分配给多个用户。
...
要在 TiDB Cloud Lake 中将 `account-admin` 角色分配给用户，请在邀请用户时选择该角色。你也可以在用户加入后再将该角色分配给他们。如果你使用的是 TiDB Cloud Lake Community Edition 或 Enterprise Edition，请先在部署期间配置一个 `account-admin` 用户，然后根据需要将该角色分配给其他用户。
...
- 如果你没有为用户显式设置默认角色，TiDB Cloud Lake 会默认使用内置角色 `public` 作为默认角色。

## 活动角色和次要角色 {#active-role-secondary-roles}

在 TiDB Cloud Lake 中，一个用户可以被授予多个角色。这些角色分为活动角色和次要角色：
...
除了标准内置角色之外，你还可以在 TiDB Cloud Lake 中创建一个名为 `billing` 的自定义角色，以专门满足财务人员的需求。`billing` 角色仅提供对计费相关信息的访问，确保财务人员能够查看必要的财务数据，而不会接触到其他业务相关页面。
