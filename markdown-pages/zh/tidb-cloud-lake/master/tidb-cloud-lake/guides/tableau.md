...
[Tableau](https://www.tableau.com/) 是一个可视化分析平台，正在改变我们使用数据解决问题的方式。你可以通过 Tableau 的 **Other Databases (JDBC)** 接口，使用 [lake-jdbc driver](https://github.com/tidbcloud/lake-jdbc) 连接到 TiDB Cloud Lake。

为获得最佳兼容性，建议使用 Tableau 2022.3 或更高版本。

## 教程：集成 TiDB Cloud Lake {#tutorial-integrating-with-lake}

本教程将指导你使用 `lake-jdbc` 将 Tableau Desktop 连接到 TiDB Cloud Lake。

### 步骤 1. 获取连接信息 {#step-1-obtain-connection-information}

获取你的 TiDB Cloud Lake 计算集群连接信息。更多详情，请参见[连接到计算集群](/tidb-cloud-lake/guides/warehouse.md#connecting-to-a-warehouse)。
...
### 步骤 3. 连接到 TiDB Cloud Lake {#step-3-connect-to-lake}

1. 启动 Tableau Desktop，并在侧边栏中选择 **Other Databases (JDBC)**。

    ![Other Databases (JDBC)](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/bi-tableau-1.png)

2. 在窗口中填写你的 TiDB Cloud Lake 连接信息，然后点击 **Sign In**。
    ...
    | Username  | 用于连接到 TiDB Cloud Lake 的 SQL 用户      | cloudapp                                                            |
