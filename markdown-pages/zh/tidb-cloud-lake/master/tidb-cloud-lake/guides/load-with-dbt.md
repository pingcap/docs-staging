...
[tidbcloudlake-dbt](https://github.com/tidbcloud/lake-dbt) 是由 TiDB Cloud Lake 开发的一个插件，主要目标是实现 dbt 与 TiDB Cloud Lake 的平滑集成。借助该插件，你可以使用 dbt 无缝执行数据建模、转换和清洗任务，并方便地将输出加载到 TiDB Cloud Lake 中。下表展示了 tidbcloudlake-dbt 插件对 dbt 常用功能的支持级别：
...
如果你刚开始接触 dbt，TiDB Cloud Lake 建议你先完成官方 dbt 教程：<https://github.com/dbt-labs/jaffle_shop>。开始之前，请按照[安装 tidbcloudlake-dbt](#install-tidbcloudlake-dbt)中的说明安装 dbt 和 tidbcloudlake-dbt。

本教程提供了一个名为 “jaffle_shop” 的示例 dbt 项目，帮助你通过实践熟悉 dbt 工具。通过在默认的全局 profile（`~/.dbt/profiles.yml`）中配置连接到你的 TiDB Cloud Lake 实例所需的信息，该项目会直接在你的 TiDB Cloud Lake 数据库中生成 dbt models 中定义的表和视图。以下是一个连接到 TiDB Cloud Lake 实例的 `profiles.yml` 文件示例：
