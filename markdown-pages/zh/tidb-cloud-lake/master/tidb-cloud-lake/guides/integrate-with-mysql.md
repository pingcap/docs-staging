...
summary: MySQL 数据集成支持将 MySQL 数据库中的数据实时同步到 TiDB Cloud Lake，支持全量 `Snapshot` 导入、持续 `Change Data Capture (CDC)`，或两者结合。
---

# MySQL Integration Task

本页介绍如何创建一个 MySQL 集成任务，将 MySQL 数据库中的数据同步到 TiDB Cloud Lake。MySQL 任务支持全量 `Snapshot` 导入、持续 `Change Data Capture (CDC)`，或两者结合。
...
- 目标 MySQL 实例可从 TiDB Cloud Lake 访问
...
确保 MySQL 实例可从 TiDB Cloud Lake 访问。检查防火墙规则和安全组，允许 MySQL 端口上的入站连接。
...
| 字段               | 描述                                                        |
|---------------------|--------------------------------------------------------------------|
| **Warehouse**       | 选择用于运行同步任务的目标 TiDB Cloud Lake 计算集群 (Warehouse)    |
| **Target Database** | 选择 TiDB Cloud Lake 中的目标数据库                             |
| **Target Table**    | TiDB Cloud Lake 中的表名（默认为源表名）     |
...
Snapshot 模式会对源表执行一次性全量读，并将所有数据导入到 TiDB Cloud Lake 中的目标表。

**Use cases:**

- 从 MySQL 到 TiDB Cloud Lake 的初始数据迁移
- 周期性全量数据刷新
...
- 实时数据复制
- 使 TiDB Cloud Lake 与业务 MySQL 数据库保持同步
- 事件驱动的数据管道
...
2. 实时捕获行级变更
3. 将变更写入 TiDB Cloud Lake 中的原始暂存表
4. 使用主键定期将变更合并到目标表
...
主键用于指定 CDC 期间 MERGE 操作所使用的唯一标识列。当捕获到变更事件时，TiDB Cloud Lake 会使用该键来判断是插入新行还是修改现有行。通常，这应当是源表的主键。
...
启用后（CDC 模式下默认启用），从 MySQL binlog 捕获到的 DELETE 操作会应用到 TiDB Cloud Lake 中的目标表。禁用后，删除操作会被忽略，目标表会保留所有历史记录。这适用于需要保留完整审计轨迹的场景。
