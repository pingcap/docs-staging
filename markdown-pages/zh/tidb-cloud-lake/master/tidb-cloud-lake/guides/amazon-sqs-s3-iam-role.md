...
summary: 了解如何在 TiDB Cloud Lake 中创建 `Amazon SQS (S3) - IAM Role` 数据源。
---
...
`Amazon SQS (S3) - IAM Role` 仅存储 SQS (S3) 导入所需的连接和授权信息。它本身不会消费消息。实际读取 SQS 消息、解析 S3 ObjectCreated 事件并将数据写入 TiDB Cloud Lake 的过程，由 [Amazon SQS (S3) 集成任务](/tidb-cloud-lake/guides/integrate-with-amazon-sqs-s3.md) 执行。
...
2. 选择 **Amazon SQS (S3) - IAM Role** 作为服务类型，然后填写连接详情：
    ...
    | **Role ARN** | Yes | 你的 AWS 账户中允许 TiDB Cloud Lake 扮演的 IAM Role ARN |
    | **External ID** | Yes | 来自 TiDB Cloud Lake 控制台的组织 ID，用于 IAM Role 信任策略 |
...
3. 点击 **Test Connectivity** 验证连接。如果测试成功，点击 **OK** 保存数据源。
    ...
    > **Note:**
    >
    > SQS (S3) 导入使用 AssumeRole 模型。你无需向 TiDB Cloud Lake 提供 AWS Access Key 或 Secret Key。相反，你需要在自己的 AWS 账户中创建一个 IAM Role，并在该角色的信任策略中允许 TiDB Cloud Lake 平台角色通过 `sts:AssumeRole` 获取临时凭证。
...
## Step 4: 创建供 TiDB Cloud Lake Assume 的 IAM Role {#step-4-create-an-iam-role-for-lake-to-assume}
...
> **注意：**
>
> `receive-message` 不会自动删除消息。它只会在可见性超时时间内暂时隐藏该消息。如果你希望 TiDB Cloud Lake 稍后消费这条测试消息，请不要手动删除它。在测试数据源连通性之前，请等待可见性超时过期。

## 提供给 TiDB Cloud Lake 的信息 {#information-to-provide-to-lake}

完成 AWS 侧配置后，在 TiDB Cloud Lake 中创建数据源时填写以下信息：

| 参数 | 说明 |
|-----------|-------------|
| `role_arn` | 你的 AWS 账户中允许 TiDB Cloud Lake 扮演的 IAM Role ARN |
| `external_id` | 来自 TiDB Cloud Lake 控制台的 Organization ID |
