...
TiDB Cloud Lake 为多种编程语言提供了官方驱动，使你能够从应用程序连接到 TiDB Cloud Lake 并与之交互。
...
所有 TiDB Cloud Lake 驱动都使用相同的 DSN（Data Source Name）格式：
...
> **注意：**
>
> `user:pwd` 指的是 TiDB Cloud Lake 中的 SQL 用户。请参见 [CREATE USER](/tidb-cloud-lake/sql/create-user.md) 以创建用户并授予权限。
...
| 参数   | 描述    | TiDB Cloud Lake  | 示例                 |
| ----------- | -------------- | -------------- | ----------------------- |
| `sslmode`   | SSL 模式       | 不使用         | `?sslmode=disable`      |
| `warehouse` | 计算集群名称   | 必填           | `?warehouse=compute_wh` |

> **TiDB Cloud Lake**：[获取连接信息 →](/tidb-cloud-lake/guides/warehouse.md#obtaining-connection-information)
