---
title: 数据清理与回收
summary: 在 TiDB Cloud Lake 中，执行 `DROP`、`TRUNCATE` 或 `DELETE` 命令时，数据不会被立即删除。这使得 TiDB Cloud Lake 的时间旅行功能得以实现，允许你访问数据的先前状态。不过，这种方式也意味着在执行这些操作后，存储空间不会被自动释放。
---
...
在 TiDB Cloud Lake 中，执行 `DROP`、`TRUNCATE` 或 `DELETE` 命令时，数据不会被立即删除。这使得 TiDB Cloud Lake 的时间旅行功能得以实现，允许你访问数据的先前状态。不过，这种方式也意味着在执行这些操作后，存储空间不会被自动释放。
...
TiDB Cloud Lake 提供了三种 VACUUM 命令，它们的**清理范围各不相同**。了解每种命令会清理哪些内容，对于数据管理至关重要。
...
VACUUM 命令族是在 TiDB Cloud Lake 中清理数据的主要方法。
...
> **Note:**
>
> 在正常运行期间通常很少需要使用，因为 TiDB Cloud Lake 会自动处理清理。通常只有在 TiDB Cloud Lake 在查询执行期间发生崩溃时，才需要手动清理。

## 调整数据保留时间 {#adjusting-data-retention-time}

VACUUM 命令会移除早于 `DATA_RETENTION_TIME_IN_DAYS` 设置的数据文件。默认情况下，TiDB Cloud Lake 会保留 1 天（24 小时）的历史数据。你可以调整此设置：
...
| TiDB Cloud Lake（个人版）                | 1 天（24 小时）  | 1 天（24 小时） |
| TiDB Cloud Lake（商业版）                | 1 天（24 小时）  | 90 天          |