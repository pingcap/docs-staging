...
TiDB Cloud Lake 提供了多种 UDF 执行模型。它们在返回形式、语言、托管方式和运维责任方面各不相同。

| UDF 类型 | 实现 | 输出 | 托管 | 典型用途 |
| --- | --- | --- | --- | --- |
| SQL 标量 UDF | SQL 表达式 | 每个输入行一个值 | 由 TiDB Cloud Lake 管理 | 格式化、计算和可重用条件 |
| 脚本标量 UDF | Python 或 JavaScript | 每个输入行一个值 | 由 TiDB Cloud Lake 管理 | 业务规则、验证和结构化数据处理 |
| WebAssembly 标量 UDF | WebAssembly 模块 | 每个输入行一个值 | 由 TiDB Cloud Lake 管理 | 编译为 WebAssembly 的计算密集型逻辑 |
| 聚合 UDF | Python 或 JavaScript | 每个组一个值 | 由 TiDB Cloud Lake 管理 | 自定义有状态聚合 |
| SQL 表 UDF | SQL 查询 | 一个结果集 | 由 TiDB Cloud Lake 管理 | 可重用的参数化查询 |
| 外部标量 UDF | Python UDF Server | 每个输入行一个值 | 由您托管 | 库、模型和专有服务 |
| 外部表 UDF | Python UDF Server | 多列或多行 | 由您托管 | 分词、扩展和记录生成 |

聚合 UDF 和表 UDF 解决的是不同问题。聚合 UDF 会消费多行并返回一个值。表 UDF 则返回一个结果集。TiDB Cloud Lake 不提供同时结合这两种执行模型的 Python 表聚合 UDF。
...
3. 如果是应在 TiDB Cloud Lake 内运行的脚本逻辑，使用内嵌 Python 或 JavaScript 标量 UDF。
...
| UDF 类型 | 主要开销 | 运维责任 |
| --- | --- | --- |
| SQL 标量 UDF | SQL 表达式求值 | 由 TiDB Cloud Lake 管理 |
| Python 或 JavaScript 标量 UDF | 脚本运行时和依赖关系初始化 | 由 TiDB Cloud Lake 管理 |
| WebAssembly 标量 UDF | 模块加载和已编译函数执行 | 由 TiDB Cloud Lake 管理 |
| 聚合 UDF | 脚本运行时和状态序列化 | 由 TiDB Cloud Lake 管理 |
| SQL 表 UDF | 查询执行 | 由 TiDB Cloud Lake 管理 |
| 外部 UDF | 网络传输和外部计算 | 由 TiDB Cloud Lake 和你的 UDF Server 部署共同负责 |
