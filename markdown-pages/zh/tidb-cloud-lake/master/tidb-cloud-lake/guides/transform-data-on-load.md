---
title: 加载时转换数据
summary: "TiDB Cloud Lake 的 `COPY INTO` 命令支持在加载过程中进行数据转换。通过集成基础转换能力，它可以简化你的 ETL 流水线，并免去使用临时表的需求。"
---

# 加载时转换数据

TiDB Cloud Lake 的 `COPY INTO` 命令支持在加载过程中进行数据转换。通过集成基础转换能力，它可以简化你的 ETL 流水线，并免去使用临时表的需求。
