---
title: 加载半结构化格式
summary: 半结构化数据包含用于分隔语义元素的标签或标记，但不遵循严格的数据库结构。TiDB Cloud Lake 使用 `COPY INTO` 命令高效加载这些格式，并可选择在加载过程中进行即时数据转换。
---

# 加载半结构化数据

半结构化数据包含用于分隔语义元素的标签或标记，但不遵循严格的数据库结构。TiDB Cloud Lake 使用 `COPY INTO` 命令高效加载这些格式，并可选择在加载过程中进行即时数据转换。
