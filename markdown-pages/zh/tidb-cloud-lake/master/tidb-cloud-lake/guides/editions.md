---
title: 版本
summary: "TiDB Cloud Lake 提供三个版本：Personal、Business 和 Dedicated，您可以根据不同需求进行选择，以满足广泛的使用场景并确保不同用例下的最佳性能。"
---

# 版本

TiDB Cloud Lake 提供三个版本：**Personal**、**Business** 和 **Dedicated**，您可以根据不同需求进行选择，以满足广泛的使用场景并确保不同用例下的最佳性能。

有关定价信息，请参见 [价格与计费](/tidb-cloud-lake/guides/pricing-billing.md)。有关这些版本之间的详细功能列表，请参见 [功能列表](#feature-lists)。

## 功能列表 {#feature-lists}

以下是 TiDB Cloud Lake 不同版本的功能列表：
...
| **支持使用 AWS PrivateLink 私有连接到 TiDB Cloud Lake 服务**。 | | ✓ | ✓ |
...
| 外部函数，用于将 TiDB Cloud Lake 扩展到其他开发平台。 | ✓ | ✓ | ✓ |
...
| 持续微批加载。 | ✓ | ✓ | ✓ |
| 用于低延时加载流式数据的流式处理。 | ✓ | ✓ | ✓ |
| 用于从 Apache Kafka topic 加载数据的 TiDB Cloud Lake Connector for Kafka。 | ✓ | ✓ | ✓ |
