...
1. Execute the following command to create the role-based access control (RBAC) resources in the `tidb-cluster` namespace based on [backup-rbac.yaml](<https://raw.githubusercontent.com/pingcap/tidb-operator/v1.6.6/manifests/backup/backup-rbac.yaml>):

    
    ```shell
    kubectl apply -f https://raw.githubusercontent.com/pingcap/tidb-operator/v1.6.6/manifests/backup/backup-rbac.yaml -n tidb-cluster
    ```
