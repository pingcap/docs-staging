...
To upgrade the Custom Resource Definitions (CRDs) for TiDB Operator, run the following command. Replace `${version}` with your target TiDB Operator version, such as `v2.0.0`:
...
* `${version}`: the target TiDB Operator version, such as `v2.0.0`.
