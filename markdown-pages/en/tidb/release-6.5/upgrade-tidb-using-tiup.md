...
> + After the upgrade, you want to use v6.5.12's default parameter values for the unmodified configuration items.
...
> **Note:**
>
> Before you upgrade the cluster to v6.5.12, make sure that the parameters you have modified in v4.0 are compatible in v6.5.12. For details, see [TiKV Configuration File](/tikv-configuration-file.md).
...
For example, if you want to upgrade the cluster to v6.5.12:
...
2. Use the `upgrade` command with the `--offline` option to perform the offline upgrade. Fill in the name of your cluster for `<cluster-name>` and the version to upgrade to for `<version>`, such as `v6.5.12`.
