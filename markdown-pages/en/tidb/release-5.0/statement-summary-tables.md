...
- `tidb_stmt_summary_max_stmt_count`: Limits the number of SQL statements that can be stored in statement summary tables. Before v5.0.4, the default value is `200`. Since v5.0.4, the default value is `3000`. If the limit is exceeded, those SQL statements that recently remain unused are cleared.
