...
> **Note:**
>
> For the `LOAD DATA INFILE` statement, TiDB Cloud Dedicated supports `LOAD DATA LOCAL INFILE`, and `LOAD DATA INFILE` from Amazon S3 or Google Cloud Storage, while TiDB Cloud Starter only supports `LOAD DATA LOCAL INFILE`.
...
- The following is an example connection string for TiDB Cloud Starter:
