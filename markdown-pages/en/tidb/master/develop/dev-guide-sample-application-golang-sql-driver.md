...
When using this function, you need to replace `${tidb_host}`, `${tidb_port}`, `${tidb_user}`, `${tidb_password}`, and `${tidb_db_name}` with the actual values of your TiDB. TiDB Cloud Starter and TiDB Cloud Essential require a secure connection. Therefore, you need to set the value of `${use_ssl}` to `true`.
