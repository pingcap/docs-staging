...
- Because this command does not specify the version of the TiDB, PD, and TiKV component, TiUP playground uses the latest version of each component by default. Assume that the latest version is v8.5.7, then this command works the same as `tiup playground:v1.12.3 v8.5.7`.
...
2. Start the TiDB cluster:
    ...
    ```shell
    tiup playground v8.5.7 --tiproxy 1 --db.config tidb.toml
    ```
...
```shell
tiup playground v8.5.7 --pd.mode ms --pd 3 --tso 2 --scheduling 2
```
