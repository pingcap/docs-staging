...
This command requires you to provide the cluster name, the TiDB cluster version (such as `v8.5.5`), and a topology file of the cluster.
...
Save the file as `/tmp/topology.yaml`. If you want to use TiDB v8.5.5 and your cluster name is `prod-cluster`, run the following command:


```shell
tiup cluster deploy -p prod-cluster v8.5.5 /tmp/topology.yaml
```
...
----          ----  -------    ----                                               ----------
prod-cluster  tidb  v8.5.5    /root/.tiup/storage/cluster/clusters/prod-cluster  /root/.tiup/storage/cluster/clusters/prod-cluster/ssh/id_rsa
```
...
For example, the following command upgrades the cluster to v8.5.5:


```bash
tiup cluster upgrade tidb-test v8.5.5
```
...
ID      Time                       Command
--      ----                       -------
4BLhr0  2026-01-15T23:55:09+08:00  /home/tidb/.tiup/components/cluster/v1.12.3/cluster deploy test v8.5.5 /tmp/topology.yaml
4BKWjF  2026-01-15T23:36:57+08:00  /home/tidb/.tiup/components/cluster/v1.12.3/cluster deploy test v8.5.5 /tmp/topology.yaml
4BKVwH  2026-01-15T23:02:08+08:00  /home/tidb/.tiup/components/cluster/v1.12.3/cluster deploy test v8.5.5 /tmp/topology.yaml
4BKKH1  2026-01-15T16:39:04+08:00  /home/tidb/.tiup/components/cluster/v1.12.3/cluster destroy test
4BKKDx  2026-01-15T16:36:57+08:00  /home/tidb/.tiup/components/cluster/v1.12.3/cluster deploy test v8.5.5 /tmp/topology.yaml
```
...
- Deploy a cluster: `tiup cluster deploy <cluster-name> <version> <topo> --ssh=system`. Fill in the name of your cluster for `<cluster-name>`, the TiDB version to be deployed (such as `v8.5.5`) for `<version>`, and the topology file for `<topo>`.
