...
summary: Network policies control who can log in to TiDB Lake based on the client IP. Even if the credentials are correct, a connection request is rejected when its IP does not satisfy the policy, giving you an extra security layer beyond username and password.
---

# Network Policy

Network policies control who can log in to TiDB Lake based on the client IP. Even if the credentials are correct, a connection request is rejected when its IP does not satisfy the policy, giving you an extra security layer beyond username and password.
...
- `BLOCKED_IP_LIST` (optional) lets you carve out explicit deny rules from the allowed ranges. TiDB Lake checks the blocked list first, so an IP that exists in both lists is still denied.
...
- If the server cannot determine a client IP or the IP does not match the allowed list, TiDB Lake immediately returns `AuthenticateFailure`.
