---
title: Ownership
summary: Ownership is a specialized privilege that signifies the exclusive rights and responsibilities a role holds over a specific data object (currently including a database, table, UDF, and stage) within TiDB Cloud Lake.
---

# Ownership

Ownership is a specialized privilege that signifies the exclusive rights and responsibilities a role holds over a specific data object (currently including a database, table, UDF, and stage) within TiDB Cloud Lake.
...
To grant ownership to a role, use the [GRANT](/tidb-cloud-lake/sql/grant.md) command. These examples demonstrate granting ownership of different database objects to the role 'data_owner':
...
-- After u1 logs into TiDB Cloud Lake, role1 has been granted to u1, so u1 can create and own tables under db:
