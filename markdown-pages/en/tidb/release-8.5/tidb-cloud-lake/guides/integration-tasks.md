...
summary: This page provides an overview of integration tasks in TiDB Cloud Lake. Integration tasks define how data flows from external sources into TiDB Cloud Lake, including source settings, target tables, and runtime parameters.
---

# Integration Tasks

An integration task in TiDB Cloud Lake defines how data flows from a source into a target table in TiDB Cloud Lake. Each task references an existing data source and specifies source settings, a target warehouse, a target database / table, and runtime parameters that are specific to the task type.
