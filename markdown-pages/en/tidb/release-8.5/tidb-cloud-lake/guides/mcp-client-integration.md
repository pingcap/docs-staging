...
### 1. Get a TiDB Cloud Lake Connection

We recommend using **TiDB Cloud Lake** for the best experience. You can obtain the DSN in two ways.
...
1. Log in to [TiDB Cloud Lake](https://app.lake.tidbcloud.com).
...
![Use with AI Tools](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/ai-tools.png)
...
1. Create a SQL user in TiDB Cloud Lake and grant the required privileges. See [CREATE USER](/tidb-cloud-lake/sql/create-user.md#example-1-full-access-across-all-databases).
...
| `false` | The MCP server can write to objects allowed by the TiDB Cloud Lake user permissions. | Use only with trusted agents and least-privilege TiDB Cloud Lake users. |

Keep safe mode enabled for most workflows. Disable it only when the agent must modify real production objects and the TiDB Cloud Lake user already has the minimum required permissions.
...
| ------------------------ | ---------------------------------------------- | -------- |
| `DATABEND_DSN`           | TiDB Cloud Lake connection string                | Required |
