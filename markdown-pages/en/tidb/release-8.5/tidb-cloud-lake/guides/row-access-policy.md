...
TiDB Cloud Lake defaults to `SET SECONDARY ROLES ALL` — all granted roles are active in the session immediately after login. The policy CASE is evaluated top-down, so the first matching branch wins.
...
- TiDB Cloud Lake defaults to `SECONDARY ROLES ALL` after login — all granted roles are active in the session.
