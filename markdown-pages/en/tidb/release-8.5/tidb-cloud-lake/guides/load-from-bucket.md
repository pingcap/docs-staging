...
summary: When data files are stored in an object storage bucket, such as Amazon S3, it is possible to load them directly into TiDB Lake using the COPY INTO command. Please note that the files must be in a format supported by TiDB Lake, otherwise the data cannot be imported. For more information on the file formats supported by TiDB Lake, see Input & Output File Formats.
---

# Loading from Bucket

When data files are stored in an object storage bucket, such as Amazon S3, it is possible to load them directly into TiDB Lake using the [COPY INTO](/tidb-cloud-lake/sql/copy-into-table.md) command. Please note that the files must be in a format supported by TiDB Lake, otherwise the data cannot be imported. For more information on the file formats supported by TiDB Lake, see [Input & Output File Formats](/tidb-cloud-lake/sql/input-output-file-formats.md).
