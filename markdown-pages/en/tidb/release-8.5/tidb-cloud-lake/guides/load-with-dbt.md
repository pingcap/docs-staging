...
[dbt-databend-cloud](https://github.com/databendcloud/dbt-databend) is a plugin developed by TiDB Lake with the primary goal of enabling smooth integration between dbt and TiDB Lake. By utilizing this plugin, you can seamlessly perform data modeling, transformation, and cleansing tasks using dbt and conveniently load the output into TiDB Lake. The table below illustrates the level of support that the dbt-databend-cloud plugin offers for commonly used features in dbt:
...
If you're new to dbt, TiDB Lake recommends completing the official dbt tutorial available at <https://github.com/dbt-labs/jaffle_shop>. Before you start, follow [Installing dbt-databend-cloud](#installing-dbt-databend-cloud) to install dbt and dbt-databend-cloud.

This tutorial provides a sample dbt project called "jaffle_shop," offering hands-on experience with the dbt tool. By configuring the default global profile (~/.dbt/profiles.yml) with the necessary information to connect to your TiDB Lake instance, the project will generate tables and views defined in the dbt models directly in your TiDB Lake database. Here's an example of the file profiles.yml that connects to a local TiDB Lake instance:
...
If you're using TiDB Cloud Lake, you can refer to this [Wiki page](https://github.com/databendcloud/dbt-databend/wiki/How-to-use-dbt-with-Databend-Cloud) for step-by-step instructions on how to run the jaffle_shop dbt project.
