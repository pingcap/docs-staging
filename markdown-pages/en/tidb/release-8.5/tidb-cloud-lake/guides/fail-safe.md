...
TiDB Cloud Lake offers the [SYSTEM$FUSE_AMEND](/tidb-cloud-lake/sql/system-fuse-amend.md) table function to enable Fail-Safe recovery. This function lets you restore data from an S3-compatible storage bucket when bucket versioning is enabled.
...
1. Enable versioning for the bucket `databend-doc`.
    ...
    ![alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/bucket-versioning.png)
...
2. Create an external table, storing the table data in the `fail-safe` folder in the `databend-doc` bucket.
    ...
    ![alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/bucket-versioning-2.png)

3. Simulate data loss by deleting all the sub-folders and their files in the `fail-safe` folder.

    ![alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/bucket-versioning-3.png)
