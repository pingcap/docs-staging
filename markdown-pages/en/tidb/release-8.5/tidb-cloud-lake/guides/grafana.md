---
title: Grafana
summary: Grafana is a monitoring dashboard system, which is an open-source monitoring tool developed by Grafana Labs. It can greatly simplify the complexity of monitoring by allowing us to provide the data to be monitored, and it generates various visualizations. Additionally, it has an alarm function that sends notifications when there is an issue with the system.
---
...
TiDB Cloud Lake and TiDB Cloud Lake can integrate with Grafana in two ways:

- **Loki Protocol (Recommended for TiDB Cloud Lake)**: Use Grafana's built-in Loki data source to connect to TiDB Cloud Lake via Loki-compatible API endpoints.
- **Custom Plugin**: Use the [Grafana TiDB Cloud Lake Data Source Plugin](https://github.com/databendlabs/grafana-databend-datasource) for direct SQL access.

## Using Loki Protocol (Recommended)

TiDB Cloud Lake provides a Loki-compatible API that allows you to use Grafana's native Loki data source without installing additional plugins. This is the recommended approach for most use cases.
...
Before connecting to Grafana, configure your TiDB Cloud Lake table for log data visualization. Below are two recommended schema types:
...
![Configure Table](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-configure-table.png)

### Step 2. Get Connection Information

1. Log in to your TiDB Cloud Lake account.
...
![Get Connection Info](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-get-connect-info.png)
...
3. Configure the basic settings:
   - **Name**: Give your data source a descriptive name (e.g., "TiDB Cloud Lake Logs")
   - **URL**: Enter `https://<host>` using the host from Step 2

    ![Configure Loki Data Source - Basic](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-configure-loki-datasource-basic.png)
...
5. Add custom HTTP headers. Under **Custom HTTP Headers**, add the following:
   ...
   - **Header**: `X-Databend-Table`, **Value**: Your table name
    ...
    ![Configure Loki Data Source - Headers](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-configure-loki-datasource-header.png)

6. Click **Save & test** to verify the connection.

![Configure Loki Data Source - Complete](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-configure-loki-datasource-complete.png)

### Step 4. Test Queries

1. Navigate to **Explore** in Grafana.

2. Select your TiDB Cloud Lake Loki data source.
...
![Test Loki Query with Explore](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-test-loki-query-with-explore.png)

## Using Custom Plugin (Alternative)

For advanced use cases requiring direct SQL access or when working with self-hosted TiDB Cloud Lake, you can use the Grafana TiDB Cloud Lake Data Source Plugin.
...
- TiDB Cloud Lake access with connection information for a warehouse (see [Connecting to a Warehouse](/tidb-cloud-lake/guides/warehouse.md#connecting))
...
### Step 3. Install the Grafana TiDB Cloud Lake Data Source Plugin
...
![Plugins](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-plugins.png)
![Plugin detail](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/grafana-plugin-detail.png)
...
3. Input the `DSN` field for your TiDB Cloud Lake instance. For example:
   - Self-hosted: `databend://root:@localhost:8000?sslmode=disable`
   - TiDB Cloud Lake: `databend://cloudapp:******@tnxxxxxxx.gw.aws-us-east-2.default.databend.com:443/default?warehouse=xsmall-fsta`
...
2. Select your TiDB Cloud Lake data source.
