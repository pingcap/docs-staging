...
# Loading TSV into TiDB Cloud Lake
...
> **Note:**
>
> Starting in TiDB Cloud Lake `v1.2.890-nightly`, `TEXT` is supported as an alias for `TSV`. This guide keeps `TYPE = TSV` in examples so they continue to work on older server versions. If you only target `v1.2.890-nightly` or later, you can use `TYPE = TEXT` instead.
