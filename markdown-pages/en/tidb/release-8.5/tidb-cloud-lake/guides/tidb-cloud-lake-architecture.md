...
The metadata service is a multi-tenant service that stores the metadata of each tenant in TiDB Cloud Lake in a highly available Raft cluster. This metadata includes:
...
The architecture of complete separation of storage and compute gives TiDB Cloud Lake a unique computational elasticity.

Each tenant in TiDB Cloud Lake can have multiple compute clusters (Warehouse), each with exclusive computing resources, and can automatically release them when inactive for more than 1 minute to reduce usage costs.

In the compute cluster, queries are executed through the high-performance TiDB Cloud Lake engine. Each query will go through multiple different submodules:
...
- Optimizer: The TiDB Cloud Lake engine provides a rule-based and cost-based optimizer framework, which implements a series of optimization mechanisms such as predicate pushdown, join reorder, and scan pruning, greatly accelerating queries.
- Processors: TiDB Cloud Lake implements a push-pull combination of pipeline execution engines. It composes the physical execution of queries into a series of pipelines in the Processor and can dynamically adapt the pipeline configuration based on the runtime information of the query task, combining the vectorized expression calculation framework to maximize the computing power of the CPU.

In addition, TiDB Cloud Lake can dynamically increase or decrease nodes in the cluster with the change of query workload, making computing faster and more cost-effective.

</div>

<div label="Storage Layer" value="Storage Layer">

The storage layer of TiDB Cloud Lake is based on FuseEngine, which is designed and optimized for inexpensive object storage. FuseEngine efficiently organizes data based on the properties of object storage, allowing for high-throughput data ingestion and retrieval.
