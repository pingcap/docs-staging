...
- User / Internal / External stages: See [What is Stage?](/tidb-cloud-lake/guides/stage-overview.md) to learn about stages in TiDB Lake.
...
These examples showcase data loading into TiDB Lake from various types of stages:
...
These examples showcase data loading into TiDB Lake from various types of external sources:
...
This example connects to Azure Blob Storage and loads data from 'data.csv' into TiDB Lake:
...
This example loads a GZIP-compressed CSV file on Amazon S3 into TiDB Lake:
...
When loading data into a Variant column, TiDB Lake automatically checks the data's validity and throws an error in case of any invalid data. For example, if you have a Parquet file named `invalid_json_string.parquet` in the user stage that contains invalid JSON data, like this:
