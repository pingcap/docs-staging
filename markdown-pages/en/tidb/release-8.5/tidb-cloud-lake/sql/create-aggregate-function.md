...
summary: Creates a user-defined aggregate function (UDAF) that runs inside TiDB Lake's JavaScript or Python runtime.
---
...
Creates a user-defined aggregate function (UDAF) that runs inside TiDB Lake's JavaScript or Python runtime.
...
| `STATE { <state_field_list> }` | Struct definition that TiDB Lake stores between partial/final aggregation steps (for example `STATE { sum DOUBLE, count DOUBLE }`). |
