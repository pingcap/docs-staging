...
summary: "TiDB Lake uses the Fuse Engine as its default storage engine, providing a Git-like data management system with."
---
...
TiDB Lake uses the Fuse Engine as its default storage engine, providing a Git-like data management system with:
...
- **Description:** If an engine is not explicitly specified, TiDB Lake will automatically default to using the Fuse Engine to create tables, which is equivalent to `ENGINE = FUSE`.

### `CLUSTER BY`

- **Description:** Specifies the sorting method for data that consists of multiple expressions. For more information, see [Cluster Key](/tidb-cloud-lake/guides/cluster-key-performance.md).
