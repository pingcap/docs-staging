...
summary: "TiDB Cloud Lake accepts a variety of file formats both as a source and as a target for data loading or unloading. This page explains the supported file formats and their available options."
---
...
TiDB Cloud Lake accepts a variety of file formats both as a source and as a target for data loading or unloading. This page explains the supported file formats and their available options.
...
> **Note:**
>
> - Starting in TiDB Cloud Lake `v1.2.891-nightly`, `TEXT` is supported as an alias for `TSV`.
...
> - If you only target TiDB Cloud Lake `v1.2.891-nightly` or later, prefer `TYPE = TEXT` for new configurations.

TiDB Cloud Lake determines the file format used by a COPY or Select statement in the following order of priority:
...
3. If no file format was defined for the stage during its creation, TiDB Cloud Lake defaults to using the PARQUET format.

> **Note:**
>
> - TiDB Cloud Lake currently supports ORC and AVRO as a source ONLY. Unloading data into an ORC or AVRO file is not supported yet.
> - TiDB Cloud Lake currently supports LANCE as an unload target ONLY. `COPY INTO <location>` writes a Lance dataset directory instead of a standalone file, so it is intended for downstream Lance tooling rather than stage-table reads or `COPY INTO <table>`.
> - For managing custom file formats in TiDB Cloud Lake, see [File Format](/tidb-cloud-lake/sql/file-format.md).
...
TiDB Cloud Lake CSV is compliant with [RFC 4180](https://www.rfc-editor.org/rfc/rfc4180) and is subject to the following conditions:
...
TiDB Cloud Lake TSV (also called `TEXT` in `v1.2.891-nightly` and later) uses the same format and options under both names. This page keeps `TSV` as the primary term for compatibility with older server versions.

TiDB Cloud Lake TSV is subject to the following conditions:
...
> **Note:**
>
> 1. In TiDB Cloud Lake, the main difference between TSV and CSV is NOT using a tab instead of a comma as a field delimiter (which can be changed by options), but using escaping instead of quoting for
...
Compared with CSV, TSV, NDJSON, and Parquet, a Lance export does **not** produce one or more standalone files that TiDB Cloud Lake can read back directly. Instead, TiDB Cloud Lake writes a dataset directory containing `.lance` data files together with dataset metadata such as `_versions/`.
...
| Read back in TiDB Cloud Lake stage query | Not supported |
