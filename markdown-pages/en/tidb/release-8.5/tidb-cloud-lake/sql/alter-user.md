...
- Setting or modifying the default role. If it is not explicitly set, TiDB Cloud Lake will default to using the built-in role `public` as the default role.
...
- When you set a default role for a user using [CREATE USER](/tidb-cloud-lake/sql/create-user.md) or ALTER USER, TiDB Cloud Lake does not verify the role's existence or automatically grant the role to the user. You must explicitly grant the role to the user for the role to take effect.
- `DISABLED` allows you to enable or disable a user. Disabled users cannot log in to TiDB Cloud Lake until they are enabled. Click [here](/tidb-cloud-lake/sql/create-user.md#example-4-create-users-with-special-configurations) to see an example.
