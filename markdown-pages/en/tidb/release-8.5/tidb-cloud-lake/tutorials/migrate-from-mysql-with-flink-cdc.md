...
In this tutorial, we'll walk you through the process of migrating from MySQL to TiDB Cloud Lake using Apache Flink CDC.
...
- Java 8 or 11 is installed on your local machine, as it is required by the [Flink TiDB Cloud Lake Connector](https://github.com/databendcloud/flink-connector-databend).
...
## Step 3: Set Up Target in TiDB Cloud Lake

1. Connect to TiDB Cloud Lake using BendSQL. If you're unfamiliar with BendSQL, refer to this tutorial: [Connecting to TiDB Cloud Lake using BendSQL](/tidb-cloud-lake/guides/connect-using-bendsql.md).
...
2. Download the TiDB Cloud Lake and MySQL connectors into the **lib** folder:
...
![Alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/cdc-dashboard.png)
...
3. Create corresponding tables with MySQL and TiDB Cloud Lake connectors in the Flink SQL Client (replace the placeholders with your actual values):
...
4. In the Flink SQL Client, synchronize the data from the mysql_products table to the databend_products table:
    ...
    ![Alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/cdc-job.png)

    You're all set! If you go back to the BendSQL terminal and query the **products** table in TiDB Cloud Lake, you will see that the data from MySQL has been successfully synchronized:
