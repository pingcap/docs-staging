...
In this tutorial, you will load data from MySQL to TiDB Lake with Debezium. Before you start, make sure you have successfully set up TiDB Lake, MySQL, and Debezium in your environment.
...
## Step 2. Create database in TiDB Lake

Create the corresponding database in TiDB Lake. Please note that you don't need to create a table that corresponds to the one in MySQL.
...
When started for the first time, the tool performs a full synchronization of data from MySQL to TiDB Lake using the specified Batch Size. As a result, the data from MySQL is now visible in TiDB Lake after successful replication.
...
You're all set! If you query the products table in TiDB Lake, you will see that the data from MySQL has been successfully synchronized. Feel free to perform insertions, updates, or deletions in MySQL, and you will observe the corresponding changes reflected in TiDB Lake as well.
