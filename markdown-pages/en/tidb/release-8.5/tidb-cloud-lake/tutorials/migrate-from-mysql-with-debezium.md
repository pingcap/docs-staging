...
# Migrate MySQL with Debezium (CDC)

> **Capabilities**: CDC, Full Load
> **✅ Recommended** for real-time migration with complete change data capture

In this tutorial, you will load data from MySQL to TiDB Cloud Lake with Debezium. Before you start, make sure you have successfully set up TiDB Cloud Lake, MySQL, and Debezium in your environment.
...
## Step 2. Create database in TiDB Cloud Lake

Create the corresponding database in TiDB Cloud Lake. Please note that you don't need to create a table that corresponds to the one in MySQL.
...
Create the file _application.properties_, then start debezium-server-databend.

When started for the first time, the tool performs a full synchronization of data from MySQL to TiDB Cloud Lake using the specified Batch Size. As a result, the data from MySQL is now visible in TiDB Cloud Lake after successful replication.
...
debezium.sink.databend.database.url=jdbc:lake://<your-lake-host>:<port>
...
You're all set! If you query the products table in TiDB Cloud Lake, you will see that the data from MySQL has been successfully synchronized. Feel free to perform insertions, updates, or deletions in MySQL, and you will observe the corresponding changes reflected in TiDB Cloud Lake as well.
