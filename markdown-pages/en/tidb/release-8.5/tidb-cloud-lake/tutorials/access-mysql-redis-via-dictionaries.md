...
summary: In this tutorial, we’ll guide you through accessing MySQL and Redis data using dictionaries in TiDB Cloud Lake. You’ll learn how to create dictionaries that map to these external data sources, enabling seamless data querying and integration.
---

# Access MySQL & Redis via Dictionaries

In this tutorial, we’ll guide you through accessing MySQL and Redis data using dictionaries in TiDB Cloud Lake. You’ll learn how to create dictionaries that map to these external data sources, enabling seamless data querying and integration.

## Before You Start

Before you start, ensure that [Docker](https://www.docker.com/) is installed on your local machine. We need Docker to set up the necessary containers for TiDB Cloud Lake, MySQL, and Redis. You will also need a SQL client to connect to MySQL; we recommend using [BendSQL](/tidb-cloud-lake/guides/connect-using-bendsql.md) to connect to TiDB Cloud Lake.

## Step 1: Setting up Environment

In this step, we’ll launch instances of TiDB Cloud Lake, MySQL, and Redis using Docker on your local machine.

1. Create a Docker network named `mynetwork` to enable communication between your TiDB Cloud Lake, MySQL, and Redis containers:
...
3. Run the following command to start a TiDB Cloud Lake container named `databend` within the `mynetwork` network:
...
5. Verify that the TiDB Cloud Lake, MySQL, and Redis containers are connected to the same network by inspecting the `mynetwork` Docker network:
...
In this step, we’ll add sample data to MySQL and Redis, and TiDB Cloud Lake.

1. In TiDB Cloud Lake, create a table named `users_databend` and insert sample user data:
...
3. Find your Redis container ID on Docker Desktop or by running `docker ps` in the terminal:
    ...
    ![alt text](https://docs-download.pingcap.com/media/images/docs/tidb-cloud-lake/redis-container-id.png)
...
In this step, we'll create dictionaries for MySQL and Redis in TiDB Cloud Lake and then query data from these external sources.

1. In TiDB Cloud Lake, create a dictionary named `mysql_users` in TiDB Cloud Lake that connects to the MySQL instance:
...
2. Create a dictionary named `redis_user_preferences` in TiDB Cloud Lake that connects to the Redis instance:
