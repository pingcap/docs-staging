...
summary: This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for TiDB Cloud Lake and the destination for storing backups.
---

# Backup & Restore with BendSave

This tutorial walks you through how to back up and restore data using BendSave. We'll use a local MinIO instance as both the S3-compatible storage backend for TiDB Cloud Lake and the destination for storing backups.

## Before You Start

Before you start, ensure you have the following prerequisites in place:

- A Linux machine (x86_64 or aarch64 architecture): In this tutorial, we'll deploy TiDB Cloud Lake on a Linux machine. You can use a local machine, a virtual machine, or a cloud instance such as AWS EC2.
    ...
    - If you are on AWS EC2, make sure your security group allows inbound traffic on port `8000`, as this is required for BendSQL to connect to TiDB Cloud Lake.
...
- The TiDB Cloud Lake release package: Download the release from the [TiDB Cloud Lake GitHub Releases page](https://github.com/databendlabs/databend/releases). The package contains the `databend-bendsave` binary in the `bin` directory, which is the tool we'll use for backup and restore operations in this tutorial.
...
2. Set your MinIO credentials as environment variables, then use the AWS CLI to create two buckets: one for storing backups (**backupbucket**) and another for TiDB Cloud Lake data (**databend**):
...
## Step 2: Set up TiDB Cloud Lake

1. Download the latest TiDB Cloud Lake release and extract it to get the necessary binaries:
...
4. Connect to your TiDB Cloud Lake instance from your local machine with BendSQL, then apply your TiDB Cloud Lake Enterprise license, create a table, and insert some sample data.
...
5. Back on your Linux machine, verify that the table data has been stored in your TiDB Cloud Lake bucket:
...
1. Run the following command to back up your TiDB Cloud Lake data to the **backupbucket** in MinIO:
...
2. After the removal, you can verify using BendSQL that querying the table in TiDB Cloud Lake fails:
...
3. Run the following command to restore your TiDB Cloud Lake data to the **databend** bucket in MinIO:
