...
summary: In this tutorial, we'll guide you through setting up a Kafka environment using Docker and loading messages from Kafka into TiDB Cloud Lake with bend-ingest-kafka.
---

# Ingest Kafka with Bend Ingest

In this tutorial, we'll guide you through setting up a Kafka environment using Docker and loading messages from Kafka into TiDB Cloud Lake with [bend-ingest-kafka](https://github.com/databendcloud/bend-ingest-kafka).
...
### Step 3: Create Table in TiDB Cloud Lake

Create the target table in TiDB Cloud Lake:
...
2. Run the following command to ingest messages from the `test-topic` Kafka topic into the target table in TiDB Cloud Lake:
...
3. Connect to TiDB Cloud Lake using BendSQL and verify that the data has been successfully loaded:
