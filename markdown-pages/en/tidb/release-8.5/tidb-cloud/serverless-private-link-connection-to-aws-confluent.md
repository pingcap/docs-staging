...
This document describes how to connect a TiDB Cloud Essential cluster to a [Confluent Cloud Dedicated cluster](https://docs.confluent.io/cloud/current/clusters/cluster-types.html) on AWS using an [AWS Endpoint Service private link connection](/tidb-cloud/serverless-private-link-connection.md).
...
- Region match: the network must reside in the same AWS region as your TiDB Cloud Essential cluster.
- AZ (Availability Zone) availability: the availability zones of the network must overlap with those of your TiDB Cloud Essential cluster.
