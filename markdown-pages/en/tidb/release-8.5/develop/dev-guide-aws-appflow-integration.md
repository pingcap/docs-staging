...
This document describes how to integrate TiDB with Amazon AppFlow and takes integrating a TiDB Cloud Starter instance as an example.

If you do not have a TiDB Cloud Starter instance, you can follow [TiDB Cloud Quick Start](/tidb-cloud/tidb-cloud-quickstart.md) to create one, which is free and can be created in approximately 30 seconds.
...
6. In the **Connect to TiDB-Connector** dialog, enter the connection properties of the TiDB Cloud Starter instance. For TiDB Cloud Starter, you need to set the **TLS** option to `Yes`, which lets the TiDB connector use the TLS connection. Then, click **Connect**.
...
- [TiDB Cloud Starter](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) is **NOT** a production environment.
