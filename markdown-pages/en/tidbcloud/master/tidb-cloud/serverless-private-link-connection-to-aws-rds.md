...
This document describes how to connect a TiDB Cloud Essential instance to an [Amazon RDS](https://aws.amazon.com/rds/) instance using an [AWS Endpoint Service private link connection](/tidb-cloud/serverless-private-link-connection.md).
...
1. In the [TiDB Cloud console](https://tidbcloud.com), navigate to the overview page of your TiDB Cloud Essential instance, and then click **Settings** > **Networking** in the left navigation pane.
...
- Region match: the instance must reside in the same AWS region as your TiDB Cloud Essential instance.
- The [subnet group](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_VPC.WorkingWithRDSInstanceinaVPC.html#USER_VPC.Subnets) of your Amazon RDS instance must have availability zones that overlap with those of your TiDB Cloud Essential instance.
...
> **Note**
>
> Currently, cross-region connectivity is not supported for TiDB Cloud Essential. If you need cross-region connections, use TiDB Cloud Premium and establish connectivity through VPC peering.
...
2. Go to [Load balancers](https://console.aws.amazon.com/ec2/home#LoadBalancers) to create a network load balancer. Provide the following information:
    ...
    - **VPC**: select the VPC where your RDS is located
    - **Availability Zones**: select the availability zones that overlap with your TiDB Cloud Essential instance
