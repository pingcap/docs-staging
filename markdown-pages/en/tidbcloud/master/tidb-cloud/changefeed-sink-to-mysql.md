...
Make sure that your TiDB Cloud Dedicated cluster can connect to the MySQL service.
...
2. [Set up a VPC peering connection](/tidb-cloud/set-up-vpc-peering-connections.md) between the VPC of the MySQL service and your TiDB Cloud Dedicated cluster.
3. Modify the ingress firewall rules of the VPC where MySQL is located.

    You must add [the CIDR of the region where your TiDB Cloud Dedicated cluster is located](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) to the ingress firewall rules. Doing so allows the traffic to flow from your TiDB Cloud Dedicated cluster to the MySQL endpoint.
...
You can connect your TiDB Cloud Dedicated cluster to your MySQL service securely through a private endpoint. If the private endpoint is not available for your MySQL service, follow [Set Up Private Endpoint for Changefeeds](/tidb-cloud/set-up-sink-private-endpoint.md) to create one.
...
Make sure that your TiDB Cloud Premium instance can connect to the MySQL service.

> **Note:**
>
> Currently, this feature is only available upon request. To request this feature, click **?** in the lower-right corner of the [TiDB Cloud console](https://tidbcloud.com), and then click **Support Tickets** to go to the [Help Center](https://tidb.support.pingcap.com/servicedesk/customer/portals). Create a ticket, fill in "Apply for VPC Peering for TiDB Cloud Premium instance" in the **Description** field, and then click **Submit**.

Private endpoints leverage **Private Link** or **Private Service Connect** technologies from cloud providers, enabling resources in your VPC to connect to services in other VPCs through private IP addresses, as if those services were hosted directly within your VPC.

You can connect your TiDB Cloud Premium instance to your MySQL service securely through a private endpoint. If the private endpoint is not available for your MySQL service, follow [Set Up Private Endpoint for Changefeeds](/tidb-cloud/premium/set-up-sink-private-endpoint-premium.md) to create one.
...
The **Sink to MySQL** connector can only sink incremental data from your TiDB Cloud Dedicated cluster to MySQL after a certain timestamp. If you already have data in your TiDB Cloud Dedicated cluster, you can export and load the existing data of your TiDB Cloud Dedicated cluster into MySQL before enabling **Sink to MySQL**.

</CustomContent>
<CustomContent plan="premium">

The **Sink to MySQL** connector can only sink incremental data from your TiDB Cloud Premium instance to MySQL after a certain timestamp. If you already have data in your TiDB Cloud Premium instance, you can export and load the existing data of your TiDB Cloud Premium instance into MySQL before enabling **Sink to MySQL**.
...
2. Use [Dumpling](https://docs.pingcap.com/tidb/stable/dumpling-overview) to export data from your <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent>, then use community tools such as [mydumper/myloader](https://centminmod.com/mydumper.html) to load data to the MySQL service.
...
1. Navigate to the overview page of the target <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent>, and then click **Data** > **Changefeed** in the left navigation pane.
