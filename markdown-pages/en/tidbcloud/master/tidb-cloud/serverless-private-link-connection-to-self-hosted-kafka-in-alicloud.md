...
The document describes how to connect a TiDB Cloud Essential instance to a self-hosted Kafka cluster in Alibaba Cloud, using an [Alibaba Cloud Endpoint Service private link connection](/tidb-cloud/serverless-private-link-connection.md).
...
Ensure that your Kafka cluster is deployed in the same region and AZs as the TiDB Cloud Essential instance. If any brokers are in different AZs, move them to the correct ones.
