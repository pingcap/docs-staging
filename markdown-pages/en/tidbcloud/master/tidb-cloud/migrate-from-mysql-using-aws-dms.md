...
- When you create an AWS DMS migration task, do not use the default `%` wildcard to select databases. Instead, explicitly specify only the databases and tables that you want to migrate. Otherwise, AWS DMS might attempt to migrate system databases such as `INFORMATION_SCHEMA`, `PERFORMANCE_SCHEMA`, `mysql`, and `sys` from the source MySQL database to TiDB, which can cause the migration task to fail.
