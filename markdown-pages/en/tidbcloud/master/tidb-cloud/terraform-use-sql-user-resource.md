...
2. Create a `sql_user.tf` file:
    ...
    - For SQL users in the TiDB Cloud Starter or TiDB Cloud Essential instance, the `user_name` and builtin role `role_readonly` and `role_readwrite` must start with the user prefix, you can get the user prefix by running the `tidbcloud_serverless_cluster` data source.
