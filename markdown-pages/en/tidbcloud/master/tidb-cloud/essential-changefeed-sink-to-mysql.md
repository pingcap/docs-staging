---
title: Sink to MySQL
summary: This document explains how to stream data from TiDB Cloud Essential to MySQL using the Sink to MySQL changefeed. It includes restrictions, prerequisites, and steps to create a MySQL sink for data replication. The process involves setting up network connections, loading existing data to MySQL, and creating target tables in MySQL. After completing the prerequisites, users can create a MySQL sink to replicate data to MySQL.
---
...
> **Note:**
> 
> Currently, the changefeed feature for TiDB Cloud Essential is only available upon request. To request this feature, do the following:
>
...
Make sure that your TiDB Cloud Essential instance can connect to the MySQL service. You can choose one of the following connection methods:
...
The **Sink to MySQL** connector can only sink incremental data from your TiDB Cloud Essential instance to MySQL after a certain timestamp. If you already have data in your TiDB Cloud Essential instance, you can export and load the existing data of your TiDB Cloud Essential instance into MySQL before enabling **Sink to MySQL**.
...
2. Use the [Export](/tidb-cloud/serverless-export.md) feature to export data from your TiDB Cloud Essential instance, then use community tools such as [mydumper/myloader](https://centminmod.com/mydumper.html) to load the data into the MySQL service.
...
1. Navigate to the overview page of the target TiDB Cloud Essential instance, and then click **Data** > **Changefeed** in the left navigation pane.
...
8. In **Start Replication Position**, configure the starting position for your MySQL sink.
    ...
    - If you do not have any data in the upstream TiDB Cloud Essential instance, select **Start replication from now on**.
