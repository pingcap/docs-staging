...
> **Note:**
>
> Currently, the Data Migration feature is in public preview for TiDB Cloud Premium.
...
- The Data Migration feature for TiDB Cloud Premium is in public preview.
...
You can create up to 100 migration jobs on TiDB Cloud Essential instances for each organization. To create more migration jobs, you need to [file a support ticket](/tidb-cloud/tidb-cloud-support.md).
...
- For TiDB Cloud Premium, both logical mode (default) and physical mode are supported. Logical mode exports data from MySQL source databases as SQL statements and then executes them on the target TiDB Cloud Premium instance, which consumes Request Capacity Units (RCUs) during the load. Physical mode uses `IMPORT INTO` on the target TiDB Cloud Premium instance and is recommended for large datasets when you prioritize load throughput and cost efficiency.
- When you use physical mode and the migration job has started, do **NOT** enable PITR (Point-in-time Recovery) or have any changefeed on the TiDB Cloud Premium instance. Otherwise, the migration job stops. If you need to enable PITR or have any changefeed, use logical mode instead to migrate data.
- When you use physical mode, you cannot create a second migration job or import task for the TiDB Cloud Premium instance before the existing data migration is completed.
...
- During incremental data migration, if the table to be migrated already exists in the target database with duplicate keys, an error is reported and the migration is interrupted. In this situation, you need to verify that the MySQL source data is accurate. If it is accurate, click the **Restart** button of the migration job, and the migration job will replace the conflicting records in the target TiDB Cloud Dedicated cluster with the MySQL source records.

</CustomContent>
<CustomContent plan="essential">

- During incremental data migration, if the table to be migrated already exists in the target database with duplicate keys, an error is reported and the migration is interrupted. In this situation, you need to verify that the MySQL source data is accurate. If it is accurate, click the **Restart** button of the migration job, and the migration job will replace the conflicting records in the target TiDB Cloud Essential instance with the MySQL source records.
- During incremental data migration (migrating ongoing changes to your TiDB Cloud Essential instance), if the migration job recovers from an abrupt error, it might enter safe mode for 60 seconds. During safe mode, TiDB Cloud migrates `INSERT` statements as `REPLACE` and `UPDATE` statements as `DELETE` and `REPLACE`, and then applies these transactions to the target TiDB Cloud Essential instance so that all data during the abrupt error reaches the target safely. For source tables without primary keys or non-null unique indexes, this can result in duplicated rows on the target TiDB Cloud Essential instance.

</CustomContent>
<CustomContent plan="dedicated">

- During incremental data migration (migrating ongoing changes to your TiDB Cloud Dedicated cluster), if the migration job recovers from an abrupt error, it might enter safe mode for 60 seconds. During safe mode, TiDB Cloud migrates `INSERT` statements as `REPLACE` and `UPDATE` statements as `DELETE` and `REPLACE`, and then applies these transactions to the target TiDB Cloud Dedicated cluster so that all data during the abrupt error reaches the target safely. For source tables without primary keys or non-null unique indexes, this can result in duplicated rows on the target TiDB Cloud Dedicated cluster.
...
- During incremental data migration (migrating ongoing changes to your TiDB Cloud Premium instance), if the migration job recovers from an abrupt error, it might enter safe mode for 60 seconds. During safe mode, TiDB Cloud migrates `INSERT` statements as `REPLACE` and `UPDATE` statements as `DELETE` and `REPLACE`, and then applies these transactions to the target TiDB Cloud Premium instance so that all data during the abrupt error reaches the target safely. For source tables without primary keys or non-null unique indexes, this can result in duplicated rows on the target TiDB Cloud Premium instance.

</CustomContent>

## Prerequisites

Before migrating, check whether your data source is supported, enable binary logging in your MySQL-compatible database, ensure network connectivity, and grant required privileges for both the source database and the target <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="essential">TiDB Cloud Essential instance</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent> database.
...
For TiDB Cloud Premium, the Data Migration feature supports the following MySQL-compatible source databases, and **MySQL** is the only data source type available in the migration job wizard. For supported connection methods, see [Ensure network connectivity](#ensure-network-connectivity).
...
For TiDB Cloud Premium, the available connection methods are as follows:

| Connection method | Availability | Recommended for |
|:---------------------|:-------------|:----------------|
| Public endpoints or IP addresses | All cloud providers supported by TiDB Cloud Premium | Quick proof-of-concept migrations, testing, or when private connectivity is unavailable |
...
For TiDB Cloud Premium instances hosted on AWS, you can use AWS PrivateLink to connect to your source MySQL instance without exposing the database to the public internet. You can reuse a private endpoint across multiple Data Migration jobs and changefeeds on the same TiDB Cloud Premium instance.
...
You can create the private endpoint either on the **Networking** page of your TiDB Cloud Premium instance or during Data Migration job creation (see [Step 2](#step-2-configure-the-source-and-target-connections)).

To create a private endpoint from the **Networking** page, take the following steps:

1. Log in to the [TiDB Cloud console](https://tidbcloud.com/) and navigate to the overview page of your TiDB Cloud Premium instance.
...
1. [Set up a VPC peering connection](/tidb-cloud/set-up-vpc-peering-connections.md) between the VPC of the MySQL service and your TiDB Cloud Dedicated cluster.

2. Modify the inbound rules of the security group that the MySQL service is associated with.

    <CustomContent plan="dedicated">

    You must add [the CIDR of the region where your TiDB Cloud Dedicated cluster is located](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) to the inbound rules. Doing so allows the traffic to flow from your TiDB Cloud Dedicated cluster to the MySQL instance.

    </CustomContent>
    
    <CustomContent plan="essential">

    You must add [the CIDR of the region where your TiDB Cloud Essential instance is located](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) to the inbound rules. Doing so allows the traffic to flow from your TiDB Cloud Essential instance to the MySQL instance.
...
2. [Set up a VPC peering connection](/tidb-cloud/set-up-vpc-peering-connections.md) between the VPC of your MySQL service and your TiDB Cloud Dedicated cluster.

3. Modify the ingress firewall rules of the VPC where MySQL is located.

    <CustomContent plan="dedicated">

    You must add [the CIDR of the region where your TiDB Cloud Dedicated cluster is located](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) to the ingress firewall rules. This allows the traffic to flow from your TiDB Cloud Dedicated cluster to the MySQL endpoint.

    </CustomContent>
    
    <CustomContent plan="essential">

    You must add [the CIDR of the region where your TiDB Cloud Essential instance is located](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region) to the ingress firewall rules. This allows the traffic to flow from your TiDB Cloud Essential instance to the MySQL endpoint.
...
For testing purposes, you can use the `root` account of your <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="essential">TiDB Cloud Essential instance</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent>.

For production workloads, it is recommended to have a dedicated user for replication in the target <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="essential">TiDB Cloud Essential instance</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent> and grant only the necessary privileges:
...
For example, you can execute the following `GRANT` statement in your target <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="essential">TiDB Cloud Essential instance</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent> to grant corresponding privileges:
...
2. Fill in the source connection profile.
    ...
    - **Connectivity method**: select a connection method for your data source based on your security requirements and cloud provider:
        ...
        - **Public**: available for all cloud providers supported by TiDB Cloud Premium (recommended for testing and proof-of-concept migrations).
...
3. Fill in the target connection profile.
    ...
    - **User Name**: enter the username of the target <CustomContent plan="dedicated">TiDB Cloud Dedicated cluster</CustomContent><CustomContent plan="essential">TiDB Cloud Essential instance</CustomContent><CustomContent plan="premium">TiDB Cloud Premium instance</CustomContent> in TiDB Cloud.
...
> **Note:**
>
> - When you use physical mode, you cannot create a second migration job or import task for the TiDB Cloud Dedicated cluster before the existing data migration is completed.
> - When you use physical mode and the migration job has started, do **NOT** enable PITR (Point-in-time Recovery) or have any changefeed on the TiDB Cloud Dedicated cluster. Otherwise, the migration job stops. If you need to enable PITR or have any changefeed, use logical mode instead to migrate data.
...
To migrate data to TiDB Cloud Premium once and for all, choose **Full + Incremental**, which ensures data consistency between the source and target databases.

You can use **physical mode** or **logical mode** for **existing data migration**:

- The default mode is **logical mode**. This mode exports data from MySQL source databases as SQL statements and then executes them on the target TiDB Cloud Premium instance. In this mode, the target tables can be either empty or non-empty before migration, but performance is slower than physical mode.

- For large datasets, you can choose **physical mode**. This mode uses `IMPORT INTO` on the target TiDB Cloud Premium instance for faster loading. Physical mode requires the target tables to be empty before migration. If precheck detects that the selected target tables are not empty, the migration job automatically falls back to logical mode.
...
> **Note:**
>
> TiDB Cloud Premium automatically manages migration job resources. The worker pool scales up and down based on the number of active migration jobs, so you do not need to choose a specification or perform manual scaling.
...
2. Click the name of your target TiDB Cloud Dedicated cluster to go to its overview page, and then click **Data** > **Data Migration** in the left navigation pane.
