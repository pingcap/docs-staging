...
- In TiDB Cloud, there are three types of projects:
    ...
    - **TiDB X project**: a logical container for TiDB X instances (including TiDB Cloud Starter, Essential, and Premium instances). TiDB X projects are used for grouping resources and applying project-level RBAC, but they do not carry Dedicated-only infrastructure settings.
...
- Dual-layer encryption is [**disabled** by default](/tidb-cloud/ticloud-cluster-create.md#flags) for TiDB Cloud Starter instances and **enabled** by default for TiDB Cloud Essential instances.

- TiDB Cloud Dedicated clusters **always use** dual-layer encryption. If CMEK is not enabled, TiDB Cloud uses an escrow key to encrypt all data in your cluster at rest.
