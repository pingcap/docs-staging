...
The SQL proxy account is automatically created during initialization of a TiDB Cloud Starter instance or TiDB Cloud Dedicated cluster for the TiDB Cloud user who is granted a role with permissions in the instance or cluster.

## How the SQL proxy account is deleted

When a user is removed from [an organization](/tidb-cloud/manage-user-access.md#remove-an-organization-member) or [a project](/tidb-cloud/manage-user-access.md#remove-project-access-for-a-user), or their role changes to one that does not have access to the TiDB Cloud Starter instance or TiDB Cloud Dedicated cluster, the SQL proxy account is automatically deleted.
...
| TiDB Cloud Starter | <= 15 characters | `serverless_unique_prefix + "." + email` |
| TiDB Cloud Starter | > 15 characters | `serverless_unique_prefix + "." + prefix($email, 6)_prefix(base58(sha1($email)), 8)` |
...
> **Note:**
>
> In the preceding table, `{user_name_prefix}` is a unique prefix generated by TiDB Cloud to distinguish TiDB Cloud Starter instances. For details, see the [user name prefix](/tidb-cloud/select-cluster-tier.md#user-name-prefix) of TiDB Cloud Starter instances.
...
SQL proxy accounts are JWT token-based and only accessible to the Data Service and SQL Editor. It is impossible to access the TiDB Cloud Starter instance or TiDB Cloud Dedicated cluster using a SQL proxy account with a username and password.
