...
After deploying your BYOC infrastructure, create a resource pool, create your first TiDB Cloud BYOC instance in the resource pool, and configure secure administrative access.
...
1. Log in to the [TiDB Cloud console](https://tidbcloud.com/), and then follow the instructions in [Create a Resource Pool](/tidb-cloud/byoc/create-resource-pool-byoc.md) to create a resource pool in the target cloud provider and region. 
    ...
    >**Note:**
    >
    > When creating the resource pool, select **Zonal** or **Regional** high availability based on your workload requirements. TiDB Cloud BYOC instances created or restored in the resource pool inherit the high availability mode of the pool.

2. After the resource pool becomes **Active**, follow the instructions in [Create a TiDB Cloud BYOC Instance](/tidb-cloud/byoc/create-tidb-instance-byoc.md) to create a new instance in the resource pool.
...
After preparing your backup file in Amazon S3, you can restore the data to a new TiDB Cloud BYOC instance in an active resource pool.
...
2. Execute the restore process.
    ...
    If the selected resource pool has reached its Pool vCPU Limit, increase or turn off the limit, or select another resource pool before continuing. This is because restoring to a new TiDB Cloud BYOC instance might cause the total provisioned vCPUs to exceed the Pool vCPU Limit and affect the performance of all TiDB Cloud BYOC instances in the same pool.
...
For deployment and verification steps, see [Configure a Bastion Host for TiDB Cloud BYOC](/tidb-cloud/byoc/byoc-configure-bastion-host.md).

## What's next

After you create your first resource pool and TiDB Cloud BYOC instance and configure secure administrative access, continue with [TiDB Cloud BYOC Joint Validation](/tidb-cloud/byoc/joint-validation.md) to validate connectivity, observability, security, and alerting.
