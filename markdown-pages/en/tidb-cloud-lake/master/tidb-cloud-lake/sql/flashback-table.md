...
By the snapshot ID or timestamp you specify in the command, TiDB Cloud Lake flashback the table to a prior state where the snapshot was created. To retrieve snapshot IDs and timestamps of a table, use [FUSE_SNAPSHOT](/tidb-cloud-lake/sql/fuse-snapshot.md).
...
- TiDB Cloud Lake recommends this command for emergency recovery only. To query the history data of a table, use the [AT](/tidb-cloud-lake/sql/at.md) clause.
