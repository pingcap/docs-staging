...
summary: The SETTINGS clause configures specific settings that influence the execution behavior of the SQL statement it precedes. To view the available settings in TiDB Cloud Lake and their values, use SHOW SETTINGS.
---

# SETTINGS Clause

The SETTINGS clause configures specific settings that influence the execution behavior of the SQL statement it precedes. To view the available settings in TiDB Cloud Lake and their values, use [SHOW SETTINGS](/tidb-cloud-lake/sql/show-settings.md).
...
```sql
-- When no timezone is set, TiDB Cloud Lake defaults to UTC, so now() returns the current UTC timestamp
SELECT timezone(), now();
