...
The [TiDB Cloud Lake MCP server](https://github.com/tidbcloud/lake-mcp) exposes TiDB Cloud Lake operations to clients that support the Model Context Protocol (MCP). The `tidbcloudlake-mcp` package supports standard input/output, HTTP, and server-sent events (SSE) transports.
...
> **Warning:**
>
> Keep the bind address restricted to a trusted network. The MCP server can access data with the permissions of the configured TiDB Cloud Lake user.

## Configuration

| Environment variable | Default | Description |
| --- | --- | --- |
| `LAKE_DSN` | Required for TiDB Cloud Lake | Connection string for the database and warehouse. |
