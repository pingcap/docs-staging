...
summary: Learn how to create an "Amazon SQS (S3) - IAM Role" data source in TiDB Cloud Lake.
---
...
`Amazon SQS (S3) - IAM Role` only stores the connection and authorization information required for SQS (S3) ingestion. It does not consume messages by itself. The actual process of reading SQS messages, parsing S3 ObjectCreated events, and writing data into TiDB Cloud Lake is performed by an [Amazon SQS (S3) Integration Task](/tidb-cloud-lake/guides/integrate-with-amazon-sqs-s3.md).

## Use Cases

- Centrally manage the queue URL, Region, IAM Role, and path scope required for SQS (S3) ingestion
- Consume S3 `ObjectCreated` events and write the corresponding object data into TiDB Cloud Lake
...
2. Select **Amazon SQS (S3) - IAM Role** as the service type, then fill in the connection details:
    ...
    | **Role ARN** | Yes | IAM Role ARN in your AWS account that TiDB Cloud Lake is allowed to assume |
    | **External ID** | Yes | Organization ID from the TiDB Cloud Lake console, used in the IAM Role trust policy |
...
3. Click **Test Connectivity** to validate the connection. If the test succeeds, click **OK** to save the data source.
    ...
    > **Note:**
    >
    > SQS (S3) ingestion uses the AssumeRole model. You do not need to provide AWS Access Key or Secret Key to TiDB Cloud Lake. Instead, create an IAM Role in your AWS account and allow TiDB Cloud Lake platform roles to obtain temporary credentials through `sts:AssumeRole` in the role trust policy.
...
4. Create an IAM Role that allows TiDB Cloud Lake platform roles to access it through `sts:AssumeRole`.
...
Prepare the following variables first. `AWS_REGION` must be the Region where both the S3 bucket and SQS queue are located. `EXTERNAL_ID` is the organization ID from the TiDB Cloud Lake platform console.
...
Confirm that `QueueArn` points to the target SQS queue, `Events` includes `s3:ObjectCreated:*`, and `FilterRules` matches the `Object Key Prefix` / `Object Key Suffix` configured in the TiDB Cloud Lake data source.

## Step 4: Create an IAM Role for TiDB Cloud Lake to Assume

Generate `trust-policy.json`. `ExternalId` is the organization ID from the TiDB Cloud Lake (platform) console.
...
> **Note:**
>
> `receive-message` does not delete the message automatically. It only hides the message temporarily during the visibility timeout. If you want TiDB Cloud Lake to consume this test message later, do not delete it manually. Wait for the visibility timeout to expire before testing data source connectivity.

## Information to Provide to TiDB Cloud Lake

After completing the AWS-side configuration, fill in the following information when creating the data source in TiDB Cloud Lake:

| Parameter | Description |
|-----------|-------------|
| `role_arn` | IAM Role ARN in your AWS account that TiDB Cloud Lake is allowed to assume |
| `external_id` | Organization ID from the TiDB Cloud Lake console |
...
- The bucket, prefix, and suffix in the S3 notification should match the TiDB Cloud Lake data source configuration.
