...
-   `Fast Online DDL`作業領域
    ...
    > **ノート：**
    >
    > ディレクトリが存在しない場合、TiDB は起動時に自動的に作成します。ディレクトリの作成が失敗した場合、または TiDB にそのディレクトリ[`Fast Online DDL`](/system-variables.md#tidb_ddl_enable_fast_reorg-new-in-v630)読み取りおよび書き込み権限がない場合、実行時に予期しない問題が発生する可能性があります。
