...
summary: このドキュメントは、TiDBスナップショットのバックアップとリストアのアーキテクチャとプロセスを紹介します。バックアッププロセスでは、BRがバックアップの時点とstorageのパスを取得し、TiKVにデータのバックアップを要求します。復元プロセスでは、BRがデータstorageパスと復元するデータベースまたはテーブルを取得し、TiKVにデータの復元を要求します。バックアップファイルにはSSTファイル、backupmetaファイル、backup.lockファイルが含まれます。
---
