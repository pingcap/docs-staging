...
> **警告：**
>
> TiDB Lightningタイプのエラー ( `lightning.max-error` ) は実験的機能です。本番環境でエラーを解決するためにのみ依存することはお勧めし**ません**。
...
すべてのエラーは、ダウンストリーム TiDB クラスターの`lightning_task_info`データベース内のテーブルに書き込まれます。インポートが完了したら、データベース内のエラーを表示して手動で処理できます。
...
**conflict_error_v1**はすべて[ローカル バックエンドでの一意/主キーの競合](#duplicate-resolution-in-local-backend-mode)を記録します。競合のペアごとに 2 行あります。
