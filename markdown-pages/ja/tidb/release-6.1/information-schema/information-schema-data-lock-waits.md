...
`DATA_LOCK_WAITS`の表は、クラスター内のすべての TiKV ノードで待機中の進行中のペシミスティック ロックを示しています。
...
> -   現在、テーブルは**悲観ロック待ち**情報のみを記録できます。楽観的なトランザクション (自動コミット トランザクションなど) が悲観的なロックによってブロックされている場合、このテーブルには対応するロック待機情報が表示されません。
