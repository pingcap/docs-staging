...
上記のようにパーティションを作成した後、テーブル内の特定の領域に関連するレコードを簡単に追加または削除できます。たとえば、東部地域（東部）のすべての店舗が別の会社に販売されているとします。次に、このリージョンの店舗従業員に関連するすべての行データを`ALTER TABLE employees TRUNCATE PARTITION pEast`を実行することで削除できます。これは、同等のステートメント`DELETE FROM employees WHERE store_id IN (6, 7, 8, 9, 10)`よりもはるかに効率的です。
...
グローバル`dynamic`プルーニング・モードを使用可能にした後でSQLステートメントによって使用される統計が正しいことを確認するには、GlobalStatsを取得するために、テーブルまたはテーブルのパーティションで`analyze`を手動でトリガーする必要があります。
