...
> **ノート：**
>
> リンクの`{version}`は、TiDB のバージョン番号を示します。たとえば、 `amd64`アーキテクチャの`v6.3.0`のダウンロード リンクは`https://download.pingcap.org/tidb-community-server-v6.3.0-linux-amd64.tar.gz`です。
...
> -   TiUP を使用してストアのラベルを管理するには、クラスターを再起動する前に`store label <id> --force`コマンドを実行して、PD に保存されているラベルを空にすることができます。
