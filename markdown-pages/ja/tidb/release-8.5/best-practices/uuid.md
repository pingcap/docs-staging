...
このセクションでは、TiDB で UUID を保存およびインデックス付けするためのベストプラクティスについて説明します。
...
`swap_flag`の効果を示すために、同じ構造を持つ2つのテーブルを示します。違いは、 `uuid_demo_1`への挿入には`UUID_TO_BIN(?, 0)`を使用し、 `uuid_demo_2`への挿入には`UUID_TO_BIN(?, 1)`を使用している点です。
