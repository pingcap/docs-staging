...
分散データベースであるTiDBは、アプリケーションの負荷を異なるコンピューティングノードまたはストレージノードに可能な限り均等に分散し、サーバーリソースをより有効に活用する負荷分散メカニズムを備えています。しかし、特定のシナリオでは、一部のアプリケーションの負荷が適切に分散されず、パフォーマンスに影響を与え、単一の高負荷ポイント（ホットスポット）が形成される可能性があります。
...
インデックスデータには、一意インデックスと非一意インデックスの 2種類があります。
...
- 読み取りホットスポットを判断するには、 **TiKV-Details**監視パネルで**Thread_CPU**を開き、いずれかの TiKV ノードのコプロセッサ CPU メトリック値が特に高いかどうかを確認します。

### TiDB Dashboardを使用してホットスポットテーブルを見つける {#use-tidb-dashboard-to-locate-hotspot-tables}

[TiDB Dashboard](/dashboard/dashboard-intro.md)の**Key Visualizer**機能は、ホットスポットのトラブルシューティング範囲をテーブルレベルに絞り込むのに役立ちます。以下は、 **Key Visualizer**で表示されたサーマルダイアグラムの例です。グラフの横軸は時間、縦軸は各種テーブルとインデックスです。色が明るいほど負荷が大きいことを示します。ツールバーで読み取りフローと書き込みフローを切り替えることができます。
...
この機能を使用するには、 `CREATE TABLE`文の`AUTO_INCREMENT`を`AUTO_RANDOM`に変更してください。この機能は、主キーの一意性のみを保証する必要がある非アプリケーションシナリオに適しています。
...
v8.5.7 以降、PD は読み取りホットスポット向けの CPU-aware hot Region scheduling をサポートしています。TiKV はストアのハートビートでホットリージョンの読み取り CPU 使用率を報告し、PD は CPU 使用率をスケジューリングの次元として使用できます。このメカニズムにより、QPS やバイトスループットは均衡しているように見えても TiKV の CPU 使用率が不均衡な読み取りホットスポットを、たとえば CPU コストが異なるクエリを含むワークロードや、TiKV ノードが異なるワークロードパターンを処理するクラスターのようなケースで、PD が特定しやすくなります。

- 読み取り CPU レポートをサポートするクラスターでは、`balance-hot-region-scheduler` のデフォルトの `read-priorities` 値は `cpu,byte` です。
- 読み取り CPU レポートをサポートしないクラスターでは、PD は自動的に `query,byte` にフォールバックし、クラスターが `query` 次元もサポートしない場合は `byte,key` にフォールバックします。
