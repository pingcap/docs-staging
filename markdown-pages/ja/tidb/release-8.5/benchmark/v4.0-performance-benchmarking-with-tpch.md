...
[TiFlash](/tiflash/tiflash-overview.md) TiDB のハイブリッドトランザクションおよび分析処理 (HTAP) 機能を強化する TiDB v4.0 で導入されたため、このレポートのテスト オブジェクトは次のとおりです。
...
ディスクおよびI/OリソースをめぐるTiKVとTiFlashの競合を回避するため、EC2に設定された2つのNVMe SSDディスクを`/data1`と`/data2`にマウントしデプロイ。TiKVを`/data1`に、 TiFlashを`/data2`にデプロイします。
