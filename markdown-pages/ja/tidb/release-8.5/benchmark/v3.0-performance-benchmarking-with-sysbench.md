...
summary: TiDB v3.0は、すべてのテストにおいてv2.1を上回り、QPSの向上とレイテンシーの低減を実現しました。v3.0における設定の変更が、パフォーマンスの向上に貢献しました。
---
...
このテストはAWS EC2上で実行され、CentOS-7.6.1810-Nitro (ami-028946f4cffc8b916) イメージを使用します。インスタンスのコンポーネントとタイプは次のとおりです。
...
Sysbenchを使用して、**各テーブルに10,000,000行が含まれる16個のテーブル**をインポートします。3つのSysbenchを起動し、3つのTiDBインスタンスに負荷をかけます。同時リクエスト数は段階的に増加します。1回の同時テストは5分間続きます。
...
TiDB v2.1 と v3.0 の両方でプリペアドプランキャッシュを有効にします (最適化の理由により、 `point select`と`read write`はv2.1 では有効になっていません)。
...
### `Read Write`テスト {#read-write-test}
