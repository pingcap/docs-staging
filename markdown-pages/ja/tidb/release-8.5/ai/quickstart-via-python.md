...
TiDB Self-Managedクラスタに接続するための基本的な例を以下に示します。
...
指定されたクエリの最近傍を検索するには、 `table.search()`メソッドを使用できます。このメソッドは、 デフォルトで[ベクトル検索](/ai/guides/vector-search.md)を実行します。
...
この例では、ベクトル検索はクエリベクトルを`chunks`テーブルの`text_vec`フィールドに格納されているベクトルと比較し、類似度スコアに基づいて意味的に最も関連性の高い上位3つの結果を返します。
