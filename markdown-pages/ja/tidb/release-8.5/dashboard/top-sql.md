...
summary: TiDB Dashboard Top SQLは、データベース内のSQLステートメントのCPUオーバーヘッドをリアルタイムで監視および可視化します。CPU負荷の高いステートメントを特定することでパフォーマンスの最適化を支援し、詳細な実行情報を提供します。パフォーマンス問題の分析に適しており、TiDB Dashboardまたはブラウザからアクセスできます。この機能はクラスタのパフォーマンスにわずかな影響を与えますが、現在、本番本番での利用が可能です。
---
