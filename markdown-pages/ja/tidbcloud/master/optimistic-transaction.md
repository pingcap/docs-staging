...
楽観的なトランザクションを有効にする前に、 `COMMIT`のステートメントがエラーを返す可能性があることをアプリケーションが正しく処理することを確認してください。アプリケーションがこれをどのように処理するかわからない場合は、代わりにペシミスティックトランザクションを使用することをお勧めします。
...
2.  クライアントは読み取り要求を発行します。
    ...
    2.  TiDBはTiKVから`start_ts`バージョンのデータを受信します。
...
5.  TiDBは2PCを開始し、トランザクションのアトミック性を保証しながらデータをストアに保持します。
    ...
    2.  TiDBはPDからリージョン分布の情報を受け取り、それに応じてすべてのキーをリージョンごとにグループ化します。
    3.  TiDBは、関係するすべてのTiKVノードにプリライト要求を送信します。次に、TiKVは、競合するバージョンまたは期限切れのバージョンがあるかどうかを確認します。有効なデータはロックされています。
...
分散データベースとして、TiDBは、主にプリライトフェーズで、TiKVレイヤーでメモリ内の競合検出を実行します。 TiDBインスタンスはステートレスであり、相互に認識していません。つまり、書き込みによってクラスタ全体で競合が発生するかどうかを知ることができません。したがって、競合検出はTiKV層で実行されます。
...
さらに、TiKVは、スケジューラーでラッチの待機に費やされた時間の監視をサポートします。

![Scheduler latch wait duration](https://download.pingcap.com/images/docs/optimistic-transaction-metric.png)

`Scheduler latch wait duration`が高く、書き込みが遅い場合は、現時点で書き込みの競合が多いと安全に結論付けることができます。
