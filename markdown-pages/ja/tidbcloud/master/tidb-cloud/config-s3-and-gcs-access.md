...
TiDB Cloud がAmazon S3 バケットのソース データにアクセスできるようにするには、次の手順に従ってTiDB Cloudのバケット アクセスを設定し、Role-ARN を取得します。
...
1.  [TiDB Cloudコンソール](https://tidbcloud.com/)で、ターゲットの TiDB クラスターのTiDB Cloudアカウント ID と外部 ID を取得します。
    ...
    3.  **[データのインポート]**ページで、 <strong>[必要なロール ARN を取得するためのガイド]</strong>をクリックして、 TiDB Cloudアカウント ID とTiDB Cloud外部 ID を取得します。後で使用するために、これらの ID をメモしておいてください。
...
1.  TiDB Cloudコンソールで、ターゲット TiDB クラスターの Google Cloud サービス アカウント ID を取得します。
    ...
    2.  ターゲット クラスターを見つけて、クラスター領域の右上隅にある**[...]**をクリックし、 <strong>[データのインポート]</strong>を選択します。 <strong>[データのインポート]</strong>ページが表示されます。
        ...
        > **ヒント：**
        >
        > または、 **[クラスター]**ページでクラスターの名前をクリックし、 <strong>[インポート]</strong>領域で<strong>[データのインポート]</strong>をクリックすることもできます。
