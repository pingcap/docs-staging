...
-   二重層暗号化は、TiDB Cloud Starter インスタンスではデフォルトで[**無効**](/tidb-cloud/ticloud-cluster-create.md#flags)になっており、TiDB Cloud Essential インスタンスではデフォルトで**有効**になっています。

-   TiDB Cloud Dedicated クラスターでは、**常に**二重層暗号化が使用されます。CMEKが有効になっていない場合、TiDB Cloudはエスクローキーを使用してクラスター内のすべての保存データを暗号化します。
