...
データサービス（ベータ版）では、各データアプリごとに自動生成されたOpenAPIドキュメントを提供しています。ドキュメントでは、エンドポイント、パラメータ、レスポンスを確認し、エンドポイントを試すことができます。
...
5.  (オプション) エンドポイントを試すには、次の手順を実行します。
    ...
    2.  ターゲットエンドポイントを見つけ、必要なパラメータを入力して**「試してみる」**をクリックします。レスポンスは**「レスポンス本文」**領域で確認できます。
