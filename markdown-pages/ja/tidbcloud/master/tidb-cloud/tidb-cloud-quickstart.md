...
AI にさらに多くの SQL文を生成させるには、次の例に示すように、さらに命令を入力します。
...
## ステップ4: TiDB Cloud Starter インスタンスに接続する {#step-4-connect-to-your-starter-instance}

TiDB は MySQL プロトコルとの高い互換性があるため、ほとんどの MySQL ツール、ドライバー、ORM を使用して接続できます。以下の手順では、MySQL CLI クライアント `mysql` を例として使用します。

1. [**My TiDB**](https://tidbcloud.com/tidbs) ページに移動し、対象の TiDB Cloud Starter インスタンスの名前をクリックして概要ページに移動します。
2. 右上隅の **Connect** をクリックします。接続パラメーターを含む接続ダイアログが表示されます。
3. 接続ダイアログの設定が実行環境と一致していることを確認します。
4. **Generate Password** をクリックしてランダムなパスワードを作成します。
5. **Connect With** ドロップダウンリストで、**MySQL CLI** を選択します。
6. 接続コマンドをコピーし、ターミナルに貼り付けて **Enter** を押して実行します。

MySQL CLI クライアントが開き、TiDB Cloud Starter インスタンスに接続されます。
