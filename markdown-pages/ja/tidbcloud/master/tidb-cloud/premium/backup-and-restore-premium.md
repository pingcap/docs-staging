...
TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスは、以下の表に示すように、多層バックアップアーキテクチャを使用してデータを保護します。
...
-   **バックアップサイクル**：TiDB Cloud Premium<CustomContent plan="byoc">またはTiDB Cloud BYOC</CustomContent>インスタンスは、1時間ごとおよび1日ごとの自動バックアップを実行します。
...
> **Note:**
>
> -   自動バックアップのストレージコストは、バックアップデータの量と保存期間によって異なります。
> -   バックアップの保持期間をデフォルトの制限を超えて延長するには、 [TiDB Cloudサポート](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)にお問い合わせください。
...
5. 新しい TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスを復元用に構成します。手順は[TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスの作成](/tidb-cloud/premium/create-tidb-instance-premium.md)と同じです。
...
4. 検証が成功すると、**Restore to a New Instance**ページが表示されます。ページ上部に表示されるバックアップ情報を確認し、[TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスを作成する](/tidb-cloud/premium/create-tidb-instance-premium.md)の手順に従って、バックアップを新しいインスタンスに復元します。
