...
このドキュメントは {{{ .byoc }}} にも適用されます。現在、{{{ .byoc }}} のバックアップおよび復元機能は TiDB Cloud Premium と同等です。
...
2. **Recycle Bin**ページで、<CustomContent plan="premium">**Premium**</CustomContent><CustomContent plan="byoc">**BYOC**</CustomContent> タブをクリックして、<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">{{{ .byoc }}}</CustomContent> インスタンスのごみ箱に移動します。

3. 復元したい<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">{{{ .byoc }}}</CustomContent>インスタンスを見つけて、**>** ボタンをクリックし、そのインスタンスで利用可能なバックアップを展開します。
...
<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">{{{ .byoc }}}</CustomContent> は、クラウドストレージ（Amazon S3 や Alibaba Cloud Object Storage Service（OSS）など）から新しいインスタンスへのバックアップの復元をサポートしています。この機能は、TiDB Cloud Dedicated クラスターまたは TiDB Self-Managed クラスターから生成されたバックアップと互換性があります。
