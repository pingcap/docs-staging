...
このドキュメントでは、TiDB Cloud Premium または TiDB Cloud BYOC インスタンス上のデータをバックアップおよび復元する方法について説明します。TiDB Cloud Premium と TiDB Cloud BYOC は、自動バックアップと手動バックアップの両方をサポートしており、必要に応じてバックアップデータを新しいインスタンスに復元できます。

</CustomContent>

バックアップファイルは、以下のソースから生成される可能性があります。

- アクティブな TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンス
- 削除された TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスのバックアップ用のごみ箱
...
1. [**My TiDB**](https://tidbcloud.com/tidbs)ページで、対象のTiDB Cloud Premium<CustomContent plan="byoc">またはTiDB Cloud BYOC</CustomContent>インスタンスの名前をクリックすると、その概要ページに移動します。
...
TiDB Cloud Premium と TiDB Cloud BYOC は、本番環境向けに強化された自動バックアップ機能を提供します。高頻度スナップショットとログバックアップを組み合わせることで、データの信頼性を確保します。
...
| **Custom Retention Mode** | <ul><li>PITR</li><li>日次バックアップスナップショット</li></ul> | 保持期間は 3日から 33日まで設定できます。PITR と日次スナップショットには、設定した保持期間が適用されます。 | スナップショットサイズに保持期間を乗じて課金されます。各バックアップは個別のオブジェクトとして課金されます。 |
...
| **Custom Retention Mode** | <ul><li>PITR</li><li>日次バックアップスナップショット</li></ul> | 保持期間は 3日から 33日まで設定できます。PITR と日次スナップショットには、設定した保持期間が適用されます。 |
...
1. [**My TiDB**](https://tidbcloud.com/tidbs) ページで、対象の TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンス名をクリックします。
...
5. **Custom Retention Mode** を選択した場合は、以下の設定を構成します。それ以外の場合は、この手順をスキップします。
    ...
    - **Backup Retention**: 3日から 33日の保持期間を選択します。デフォルト値は 7日です。
...
TiDB Cloud Premium<CustomContent plan="byoc">またはTiDB Cloud BYOC</CustomContent>インスタンスの既存のバックアップファイルを削除するには、以下の手順を実行してください。
...
自動バックアップに加えて、TiDB Cloud Premium と TiDB Cloud BYOC は手動バックアップもサポートしています。手動バックアップは、管理された確実な復元ポイントを提供します。システムアップグレード、重要なデータの削除、元に戻せないスキーマや設定の変更など、リスクの高い操作を実行する前に、手動バックアップを作成することを強くお勧めします。
...
新しい TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスにデータを復元するには、以下の手順に従ってください。
...
5. 新しい TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスを復元用に構成します。手順は <CustomContent plan="premium">[TiDB Cloud Premium インスタンスの作成](/tidb-cloud/premium/create-tidb-instance-premium.md)</CustomContent><CustomContent plan="byoc">[Create a TiDB Cloud BYOC Instance](/tidb-cloud/byoc/create-tidb-instance-byoc.md)</CustomContent> を参照してください。

    <CustomContent plan="byoc">

    TiDB Cloud BYOC の場合は、バックアップと同じクラウドプロバイダーおよびリージョンにあるアクティブなリソースプールを選択します。復元されたインスタンスは、選択したリソースプールの高可用性モードを継承します。同じバックアップをゾーンリソースプールまたはリージョンリソースプールに復元できます。適切なリソースプールがない場合は、`Organization Owner` がインスタンスを復元する前に作成できます。その他のロールではリソースプールを作成できません。詳細については、[Create a Resource Pool](/tidb-cloud/byoc/create-resource-pool-byoc.md) を参照してください。

    選択したリソースプールに Pool vCPU Limit が設定されており、現在プロビジョニングされている vCPU がその制限以上である場合、TiDB Cloud は警告を表示し、そのリソースプールにはインスタンスを復元できません。続行するには、Resource Pool の詳細ページに移動して Pool vCPU Limit を増やすか無効にするか、別のリソースプールを選択してください。

    > **Note:**
    >
    > 現在プロビジョニングされている vCPU が Pool vCPU Limit を下回っている場合でも、インスタンスを復元すると、プロビジョニングされる vCPU の合計がその制限を超える可能性があります。これにより、リソースのスケーリングが制約され、リソースプール内のすべてのインスタンスのパフォーマンスが低下する可能性があります。インスタンスを復元する前に、リソースプールに十分な vCPU 容量があることを確認してください。必要に応じて、Pool vCPU Limit を増やすか無効にするか、別のリソースプールを選択してください。
...
ごみ箱から削除した TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスを復元するには、以下の手順を実行してください。
...
2. **Recycle Bin**ページで、<CustomContent plan="premium">**Premium**</CustomContent><CustomContent plan="byoc">**BYOC**</CustomContent> タブをクリックして、<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent> インスタンスのごみ箱に移動します。

3. 復元したい<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent>インスタンスを見つけて、**>** ボタンをクリックし、そのインスタンスで利用可能なバックアップを展開します。
...
TiDB Cloud Premium と TiDB Cloud BYOC は、クラウドストレージ（Amazon S3 など）から新しいインスタンスへのバックアップの復元をサポートしています。この機能は、TiDB Cloud Dedicated クラスターまたは TiDB Self-Managed クラスターから生成されたバックアップと互換性があります。
...
4. 検証が成功すると、**Restore to a New Instance**ページが表示されます。ページ上部に表示されるバックアップ情報を確認し、<CustomContent plan="premium">[TiDB Cloud Premium インスタンスの作成](/tidb-cloud/premium/create-tidb-instance-premium.md)</CustomContent><CustomContent plan="byoc">[Create a TiDB Cloud BYOC Instance](/tidb-cloud/byoc/create-tidb-instance-byoc.md)</CustomContent>の手順に従って、バックアップを新しいインスタンスに復元します。

    <CustomContent plan="byoc">

    TiDB Cloud BYOC では、対象のクラウドプロバイダーとリージョンに一致するアクティブなリソースプールを選択してください。復元されたインスタンスは、選択したリソースプールの高可用性モードを継承します。同じバックアップをゾーンリソースプールまたはリージョンリソースプールに復元できます。適切なリソースプールがない場合は、`Organization Owner` がインスタンスを復元する前に作成できます。その他のロールではリソースプールを作成できません。詳細については、[Create a Resource Pool](/tidb-cloud/byoc/create-resource-pool-byoc.md)を参照してください。

    選択したリソースプールに Pool vCPU Limit が設定されており、現在プロビジョニングされている vCPU がその制限以上である場合、TiDB Cloud は警告を表示し、そのリソースプールにはインスタンスを復元できません。続行するには、Resource Pool の詳細ページに移動して Pool vCPU Limit を増やすか無効にするか、別のリソースプールを選択してください。

    > **Note:**
    >
    > 現在プロビジョニングされている vCPU が Pool vCPU Limit を下回っている場合でも、インスタンスを復元すると、プロビジョニングされる vCPU の合計がその制限を超える可能性があります。これにより、リソースのスケーリングが制限され、リソースプール内のすべてのインスタンスのパフォーマンスに影響する可能性があります。インスタンスを復元する前に、リソースプールに十分な vCPU 容量があることを確認してください。必要に応じて、Pool vCPU Limit を増やすか無効にするか、別のリソースプールを選択してください。
...
2. IAMユーザーに権限を付与します。
    ...
    タスクに必要な権限のみを含むポリシーを作成し、それをIAMユーザーにアタッチします。TiDB Cloud Premium<CustomContent plan="byoc"> または TiDB Cloud BYOC</CustomContent> インスタンスにデータを復元するには、`s3:GetObject`、`s3:GetBucketLocation`、および `s3:ListBucket` 権限を付与します。
...
2. 必要な権限を持つカスタムポリシーを作成します。詳細については、[Create custom policies](https://www.alibabacloud.com/help/en/ram/user-guide/create-a-custom-policy)を参照してください。
    ...
    - **Action** セクションで、必要な権限を選択します。バックアップを TiDB Cloud Premium インスタンスに復元するには、`oss:ListObjects` と `oss:GetObject` 権限を付与します。
