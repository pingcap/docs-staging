...
このドキュメントは TiDB Cloud BYOC にも適用されます。現在、TiDB Cloud BYOC のバックアップおよび復元機能は TiDB Cloud Premium と同等です。
...
<CustomContent plan="premium">TiDB Cloud Premium</CustomContent><CustomContent plan="byoc">TiDB Cloud BYOC</CustomContent> は、クラウドストレージ（Amazon S3 や Alibaba Cloud Object Storage Service（OSS）など）から新しいインスタンスへのバックアップの復元をサポートしています。この機能は、TiDB Cloud Dedicated クラスターまたは TiDB Self-Managed クラスターから生成されたバックアップと互換性があります。
