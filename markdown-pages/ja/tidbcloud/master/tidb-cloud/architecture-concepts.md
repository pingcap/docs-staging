...
- **Resource pool**: 1 つ以上の TiDB Cloud BYOC インスタンスに対する基盤となる物理リソース、ネットワーク、および容量の境界を定義します。各リソースプールには、独自の容量設定、リソースプール CIDR、高可用性モード、および AWS リソースタグがあります。
...
お客様のアプリケーションは、アプリケーション環境から TiDB Cloud BYOC インスタンスに接続します。BYOC の設定に応じて、接続にはプライベートアクセスまたは制御されたパブリックアクセスのいずれかが使用されます。プライベートアクセスについては、BYOC は VPC ピアリング やその他のプライベート接続オプションをサポートします。
