...
>
> - `ti` は、必要な操作をサポートしている場合、TiDB Cloud Starterでの新しい自動化ワークフローに推奨されます。`ti` を使用してTiDB Cloud Filesystem内のファイルシステムを管理することもできます。開始するには、[TiDB Cloud CLI を使い始める](/ai/ti/ti-quick-start.md) を参照してください。
...
`ticloud` CLIはコマンドラインインターフェースであり、ターミナルからTiDB Cloudを操作できます。TiDB Cloud CLIを使用すると、 TiDB Cloud StarterおよびEssentialインスタンスを簡単に管理したり、インスタンスへのデータのインポートやエクスポートを行ったり、その他のCLIワークフローを実行したりできます。

> **Note:**
>
> `ticloud` CLIは現在パブリックプレビューであり、TiDB Cloud Dedicatedクラスターをサポートしていません。
