...
従来のTiDBは既に計算処理（SQLレイヤー）とstorage（TiKV）を分離していますが、TiDB XではSQL層とストレージ層の両方において、さらに分離レイヤーが追加されています。この設計により、オンラインのトランザクション処理ワークロード向けの**軽量な計算**処理と、リソース集約型の**バックグラウンド**タスク向けの高負荷な計算処理が区別されます。
