...
TiDB X インスタンスは、[TiDB Xアーキテクチャ](/tidb-cloud/tidb-x-architecture.md) 上に構築されたサービス指向の TiDB Cloud オファリングであり、TiDB Cloud Starter、Essential、Premium インスタンスなどが含まれます。
...
> **注記：**
>
> このドキュメントは TiDB Cloud Starter、Essential、Premium にのみ適用されます。TiDB Cloud Dedicated については、[Limited SQL Features on TiDB Cloud Dedicated](/tidb-cloud/limited-sql-features.md) を参照してください。

## Statements {#statements}

### Placement と range 管理 {#placement-and-range-management}

| ステートメント | TiDB Cloud Premium | TiDB Cloud Starter and TiDB Cloud Essential |
|:-|:-|:-|
...
| Statement | TiDB Cloud Premium | TiDB Cloud Starter and TiDB Cloud Essential |
|:-|:-|:-|
...
| `ADMIN RESET TELEMETRY_ID` | サポートされています | Telemetry は TiDB Cloud Starter または TiDB Cloud Essential ではサポートされていません。 |
...
| `ALTER INSTANCE RELOAD TLS` | サポートされていません | TiDB Cloud Starter と TiDB Cloud Essential では TLS 証明書が自動的に更新されます。 |
...
| 関数と演算子 | TiDB Cloud Premium | TiDB Cloud Starter and TiDB Cloud Essential |
|:-|:-|:-|
| `SLEEP` | 制限はありません | [`SLEEP()` function](https://docs.pingcap.com/tidbcloud/miscellaneous-functions) 関数は、最大 300 秒のスリープ時間をサポートします。 |

## システムテーブル {#system-tables}

| データベース | テーブル | TiDB Cloud Premium | TiDB Cloud Starter and TiDB Cloud Essential |
|:-|:-|:-|:-|
...
| 変数 | TiDB Cloud Premium | TiDB Cloud Starter and TiDB Cloud Essential |
|:-|:-|:-|
...
[^1]: データ配置の設定は TiDB Cloud Starter、Essential、Premium ではサポートされていません。

[^2]: リソースグループの設定は TiDB Cloud Starter、Essential、Premium ではサポートされていません。

[^3]: TiDB Cloud Starter、Essential、Premium で [Back up and Restore](/tidb-cloud/backup-and-restore-serverless.md) 操作を実行するには、代わりに TiDB Cloud コンソールを使用できます。

[^4]: この機能は [Security Enhanced Mode (SEM)](/system-variables.md#tidb_enable_enhanced_security) では利用できません。

[^5]: TiDB Cloud Starter、Essential、Premium で [Slow Query](/tidb-cloud/tune-performance.md#slow-query) を追跡するには、代わりに TiDB Cloud コンソールを使用できます。

[^6]: TiDB Cloud Starter または TiDB Cloud Essential で [Statement Analysis](/tidb-cloud/tune-performance.md#statement-analysis) を実行するには、代わりに TiDB Cloud コンソールを使用できます。

[^7]: Drainer と Pump は TiDB Cloud ではサポートされていません。

[^8]: プラグインは TiDB Cloud Starter、Essential、Premium ではサポートされていません。

[^9]: TiDB Cloud Starter と TiDB Cloud Essential では強力なパスワードポリシーが適用されます。

[^10]: この変数は TiDB Cloud Starter と TiDB Cloud Essential では読み取り専用です。

[^11]: TiDB Cloud Starter と TiDB Cloud Essential では、[example](https://docs.pingcap.com/tidb/stable/sql-plan-replayer#examples-of-exporting-cluster-information) に示されているように、`${tidb-server-status-port}` を介して `PLAN REPLAYER` がエクスポートしたファイルをダウンロードすることはサポートされていません。代わりに、TiDB Cloud Starter と TiDB Cloud Essential では、ファイルをダウンロードするための [presigned URL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ShareObjectPreSignedURL.html) が生成されます。この URL は生成後 10 時間有効です。